...
| 3C -- split Script Doctor | COMPLETE | -- | `74438ff` -- `run_script_doctor` split into `_diagnosis` + `_edits` passes; +1 LLM call (technical, tagged; sweep 21/21). Rows enriched to 5 fields; `beat_intent`/`target_words` deferred -- not on the ledger (follow-up: stamp them) |
