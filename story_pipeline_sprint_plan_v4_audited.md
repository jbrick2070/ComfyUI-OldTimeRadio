...
- [x] Hook `reroll_targets[]` into `compose_line_draft`. **[DONE 2387cef 2026-05-25]** Resolved Option A -- the reroll calls `compose_line` with polish OFF on the technical model resident in the freeze cascade, threading the critic's `RerollTarget.hint` as a hard REVISE block. Keeps the deterministic strip pipeline (`cast_strip` / phantom gate / vocative strip) on the technical-model output -- a bare `compose_line_draft` would skip those guards.
- [x] Cap at 2 critic->reroll cycles. Cycle 3 -> `needs_full_rerun`. **[DONE 2387cef 2026-05-25]** `MAX_REROLL_CYCLES = 2`; cap exhaustion restores the pre-reroll lines from snapshot and stamps `needs_full_rerun` (the `review_ledger` verdict-stamp pattern). The cascade skips Phase 7/8/10 on the terminal verdict via the new shared `_build_terminal_skip_disposition` helper.
- [x] Add `cycle_count` to ledger. **[DONE 2387cef 2026-05-25]** Stamped to `meta.cycle_count`. The Sprint 6 arc-verdict gate (`build_render_plan`) reads it as `cycle_count >= MAX_REROLL_CYCLES` for the block-on-flat / mid-collapse rule.

---

## Sprint 6 -- Critic -> Render Coupling
- [x] `render_selection: dramatic_peaks_only` reads `render_priority[]` from 5B. **[DONE 244e9fd 2026-05-25]** `build_render_plan` reorders the pool to match the critic's `render_priority`; unprioritized lines fall to the tail in ledger order.
...
### 2026-05-25 -- Sprint 5C (targeted reroll loop)

- Commit `2387cef` on `v2.0-alpha` -- 5 files, +1168/-34. Predecessor HEAD `393ca9c`.
- **Sprint 5C fork RESOLVED -- Option A (technical slot, in-cascade).** The reroll re-composes critic-flagged lines on the technical model already resident in `OTR_LedgerFreezeCascade` -- like the Script Doctor's post-cleanup edit pass -- with NO creative-slot VRAM swap and NO node-surface / workflow-JSON change. Option B (a `creative_writing_model` cascade socket + a technical->creative->technical swap) was rejected: PD2 (VRAM ceiling) and PD3 (no JSON re-wire) both favour Option A; the deviation from the plan's literal "creative slot" wording is deliberate and bounded. The five reroll-loop design questions were resolved to the `session_handoff.md` recommended defaults: loop inside the cascade post-critic / before Phase 7; full critic re-run each cycle; in-place `update_line_text` overwrite + `meta.reroll_history` audit; the critic hint as a hard REVISE constraint; per-batch cycle counter, cap 2, cycle 3 -> `needs_full_rerun`.
- **Sprint 5C COMPLETE.** New module `nodes/_otr_reroll.py` -- `RerollDisposition`; `build_reroll_line_request` (rebuilds a `LineRequest` from `meta` + the ledger line row); `run_targeted_reroll` (the loop). NEVER raises (PD1): a failed re-composition keeps the original line; an unexpected error degrades to freeze-what-we-have. On cap exhaustion the pre-reroll lines are restored from a snapshot and the verdict is `needs_full_rerun` (the `review_ledger` verdict-stamp pattern). Re-composition runs `compose_line` with polish OFF so the deterministic strip pipeline (cast_strip / phantom gate / vocative strip) still guards the technical-model output -- a faithful reading of the plan's "hook into compose_line_draft" bullet that keeps the cast-safety guards a bare `compose_line_draft` call would skip.
- **Composer (`_otr_line_composer.py`):** `LineRequest` gains a `reroll_hint` field; `_build_user_prompt` renders a REVISE block at the WRITE LINE tail, gated on a non-empty value (mirrors the Sprint 5A `continuity_slice` pattern -- existing callers / tests unaffected). `compose_line` / `compose_line_draft` gain a `reroll_hint` keyword param that overlays the hint onto the frozen request.
- **Cascade (`_otr_freeze_cascade.py`):** `run_freeze_cascade` calls `run_targeted_reroll` after the 5B critic on the non-terminal path. A `needs_full_rerun` reroll verdict skips Phase 7/8/10 via a new shared `_build_terminal_skip_disposition` helper -- the reviewer-terminal block was refactored onto the same helper (faithful extraction; regression-verified).
- **Writer (`OTR_LedgerScriptWriter.py`):** stamps `meta.canon_header` / `outline_spine` / `theme` / `style_descriptor` so the cascade (a separate node invocation that reads the ledger from disk) can reconstruct a `LineRequest`. `meta.allowed_roster` was already stamped; the per-line rows already carry `beat_intent` / `traits` (mood) / `target_words` / `arc_phase` (Sprint 3C enrichment).
- **PD6:** both reroll LLM calls -- `compose_line` and `run_story_critic` -- run on the technical slot, tagged `# LLM slot: technical` at each call site. No node exposes a `model_id` widget. **PD3:** no node `INPUT_TYPES` / widget / socket changed -- no workflow JSON re-wire. **PD1:** the reroll never raises; the operator live-run still owns the audio-unbroken confirmation.
- **Noted v2 follow-up:** ~~`build_reroll_line_request` leaves `continuity_slice` empty on a reroll~~ **LANDED `18edd26` 2026-05-25 -- see the dedicated entry below.** The reroll now rebuilds the per-speaker continuity slice from `meta["continuity"]` via the new `_rebuild_continuity_slice` helper, so the rerolled line gets the same per-speaker hard constraints the first-pass compose received.
- **Tests:** new `tests/test_otr_reroll.py` -- 11 tests (LineRequest rebuild + target_words fallback, REVISE block render on/off, `reroll_hint` param, no-op / resolved / exhaustion-restore / never-raises / unknown-target paths).
- **Regression:** full OTR suite 2823 passed / 21 skipped (baseline 2812; +11 = the new reroll tests); Bug Bible 16 passed / 7 skipped / 3 xfailed; LLM-slot sweep 23/23 tagged, 0 parse failures. 0 failed across every suite. All touched files UTF-8 no BOM, AST-clean; `local HEAD == origin/v2.0-alpha`.
- **New bug ids:** none.
- **Sprint state:** Sprints 0, 1, 2A-2E, 3A-3G, 4, 5A-5C COMPLETE. Remaining: Sprint 6 (critic -> render coupling). The operator live-run still covers 3A + 5A + 5B + 5C + the carried cast/style/seed batch (Prime Directive 1).

### 2026-05-25 -- Sprint 6 (critic -> render coupling)

- Commit `244e9fd` on `v2.0-alpha` -- 7 files, large change. Predecessor HEAD `7a36f5e`. Pushed 2026-05-25.
- **Sprint 6 fork RESOLVED -- full v1 coupling (Jeffrey 2026-05-25).** The choice was between cascade-only stamping (plan line 520's literal "widgets on the cascade node surface" with the consumer deferred) and full coupling (cascade widgets + render-side consumer). Jeffrey picked full coupling so the widgets affect rendering today rather than stamping metadata nothing reads.
- **Sprint 6 COMPLETE (LOCAL).** New pure module `nodes/_otr_render_plan.py` -- `build_render_plan(ledger_data, *, render_selection, render_max_n, protagonist_only, manual_line_ids) -> dict`. Pure dict / list arithmetic over the already-stamped Sprint 5B critic report + Sprint 5C `cycle_count` / `reroll_history`. NEVER raises (PD1) -- a degraded computation returns None and HuMo falls back to its pre-Sprint-6 render-all behaviour. No LLM call.
- **Cascade (`_otr_freeze_cascade.py`):** `run_freeze_cascade` gains 4 kwargs (`render_selection` / `render_max_n` / `protagonist_only` / `manual_line_ids`); after the Sprint 5C reroll loop on the non-terminal path, it calls `build_render_plan` and stamps `meta.render_plan`. The stamp lives ONLY when the plan was computed (None -> no stamp -> HuMo render-all). Computed BEFORE Phase 7 / 8 / 10 so the stamp is part of the frozen ledger.
- **Cascade node surface (`OTR_LedgerFreezeCascade.py`):** 4 new optional widgets added to `INPUT_TYPES` after `vram_ceiling_gb` -- `render_selection` (COMBO: "all" | "dramatic_peaks_only", default "all"), `render_max_n` (INT default 6, 0 = no cap), `protagonist_only` (BOOLEAN default False), `manual_line_ids` (STRING default ""). `run()` forwards each to `run_freeze_cascade`. All four default to behave-as-before so an unmodified workflow's behaviour is identical.
- **HuMo consumer (`batch_humo_render.py`):** the per-line plan loop reads `meta.render_plan` at the top (once per run) and applies a SKIP filter ONLY to character beats -- announcer / music / sfx beats follow their existing render rules. A `blocked: True` plan skips every character line (the arc-verdict gate refused the render -- operator must rerun or override). A `line_ids` set skips any character line NOT in the set with reason `(render_plan not_in_plan)`. The skip runs BEFORE timing / chunking / portrait resolution so an excluded line costs zero.
- **Workflow JSON (`workflows/otr_scifi_16gb_full.json`):** `OTR_LedgerFreezeCascade` node `widgets_values` extended from 3 entries `[true, true, 14.0]` to 7 entries `[true, true, 14.0, "all", 6, false, ""]` matching the new INPUT_TYPES order. **`test_workflow_json_guardrails.py::test_cascade_widget_vector_trimmed_in_canonical_json` updated** (3 -> 7 expected) -- the guard's intent (the deleted B3 widgets stay deleted) still holds; the 4 new slots are Sprint 6 additions on top.
- **Selection logic** (in `build_render_plan`):
  - **Arc-verdict gate:** `cycle_count >= MAX_REROLL_CYCLES` (2) AND `arc_verdict in (mid_collapse, flat)` -> stamp `blocked=True, line_ids=[]`. `uneven` and `strong` do NOT block. `manual_line_ids` LIFTS the gate (operator override always wins).
  - **manual_line_ids:** highest-priority override. Comma-separated; unknown ids dropped; dedupe + preserve operator order.
  - **protagonist_only:** restricts the pool to the most-spoken character (ties broken by cast-roster order).
  - **dramatic_peaks_only:** reorders the pool to match the critic's `render_priority`; unprioritized lines fall to the tail in ledger order.
  - **flat_lines exclusion:** lines flagged `flat_lines[]` drop UNLESS they appear in `meta.reroll_history` with `status="rerolled"` (a rerolled flat line is a fresh draft and earned its place back).
  - **render_max_n cap:** applied last; 0 disables the cap.
- **Prime Directives.** **PD1:** `build_render_plan` never raises; HuMo's filter is additive over the existing SKIP arms. **PD2:** no LLM call, no VRAM impact. **PD3:** the workflow JSON re-wire is in lockstep with the cascade `INPUT_TYPES` change; the guardrail test was updated in the same commit. **PD6:** no new LLM call -- the new module is pure arithmetic; the LLM-slot sweep stays at 23/23.
- **Tests:** new `tests/test_otr_render_plan.py` -- 19 tests (defaults / max_n cap / dramatic_peaks_only reorder / protagonist_only / manual override + arc-block lift / arc-verdict gate fires + dormancy / flat_lines exclusion + rerolled-flat-line keep / never-raises / constants).
- **Regression:** full OTR suite 2842 passed / 21 skipped (baseline 2823 post-5C; +19 = the new render_plan tests); Bug Bible 16 passed / 7 skipped / 3 xfailed; LLM-slot sweep 23/23 tagged, 0 parse failures. AST + JSON parse OK on all touched files. 0 failed across every suite. All new files UTF-8 no BOM.
- **New bug ids:** none.
- **Push model -- fix-forward (Jeffrey 2026-05-25):** the original "push held for eyes-on review" plan was reversed at push time. Rationale: unpushed work sitting on one disk is a liability. The commit is now public history; any post-live-run review notes land as follow-up commits rather than holding history hostage. Eyes-on review still happens -- it just happens against the pushed commit. Operator live-RTX-5080 run is the audio-is-king reversion gate (Prime Directive 1); any reversion ships as a new commit on top.
- **Sprint state:** Sprints 0, 1, 2A-2E, 3A-3G, 4, 5A-5C, 6 COMPLETE. The whole v4 build is landed pending the operator live-run validation gate.

### 2026-05-25 -- Sprint 5C v2 follow-up: continuity_slice on rerolls

- Commit `18edd26` on `v2.0-alpha` -- 2 files, +211/-8. Predecessor HEAD `c43046b`. Pushed 2026-05-25.
- **Sprint 5C v2 follow-up CLOSED.** The reroll now rebuilds the per-speaker continuity slice from `meta["continuity"]` (the Sprint 5A writer stamp), so the technical-slot re-composition gets the same hard constraints (who knows what, by which beat) the first-pass creative compose received. The critic's hint stays the load-bearing signal for what to change; the continuity slice closes the back door where a rerolled line could legitimately reference a fact the speaker should not yet know. Additive only -- no node `INPUT_TYPES` / widget / output socket touched, no workflow JSON re-wire (Prime Directive 3 N/A).
- **`nodes/_otr_reroll.py`:** import `ContinuityState` + `render_continuity_slice` from `_otr_continuity` (same package-vs-flat guard as the existing imports). New pure helper `_rebuild_continuity_slice(meta, speaker, beat_index) -> str` -- NEVER raises (Prime Directive 1); four `""` exits cover negative index, missing / empty `meta["continuity"]`, `model_validate` failure, and a defensive guard around the render call (the last three log a warning so the live-run console shows the degradation). `build_reroll_line_request` threads the rebuilt slice into the returned `LineRequest`. Beat coordinate = the 0-based line row position in `ledger_data["lines"]` -- matches the first-pass coordinate via the `init_lines_from_outline` 1:1 beat-to-line-row invariant (writer only calls `update_line_text` in place; rows are never inserted, deleted, or reordered after stamp). Docstring updated -- the "noted v2 follow-up" sentence is replaced with the now-current rebuild path.
- **`tests/test_otr_reroll.py`:** new section "1b. continuity_slice rebuild on reroll" -- 6 tests covering known-fact surface, established_beat filtering, hidden_from always surfaces (beat gating ignored -- a secret is a secret), missing `meta["continuity"]` -> empty, neutral state -> empty, garbage -> empty with no raise. Two helpers (`_continuity_dump`, `_fact`) shape the dumps without pulling pydantic into the test module.
- **Prime Directive 6:** no new LLM call site -- `_rebuild_continuity_slice` is pure Python; `render_continuity_slice` is pure. The LLM-slot sweep stays at 23/23, no new tag needed.
- **Regression:** deferred to the post-live-run sweep. The change is additive and the existing `_ledger_data()` fixture has no `meta["continuity"]`, so the new code path returns `""` and LineRequests come out byte-identical to today's; existing tests should pass unchanged. Live-run was mid-flight at commit time -- the regression sweep + Bug Bible + LLM-slot sweep are queued to run against `18edd26` once the live-run wraps so the CPU stays clear of the HuMo render.
- **New bug ids:** none.
- **Sprint state:** Sprints 0, 1, 2A-2E, 3A-3G, 4, 5A-5C, 6 COMPLETE. v2 follow-up list now: scoped critic re-invocation (per-affected-line re-critique instead of whole-script), per-line v1/v2 ledger versioning, explicit `meta.protagonist_char_id` writer stamp. None blocking; all optional.

### 2026-05-25 -- Plan housekeeping (Sprint 6 push state + 18edd26 entry)

- Commit `a84a3d6` on `v2.0-alpha` -- 1 file, +23/-12. Predecessor HEAD `18edd26`. Pushed 2026-05-25.
- **Docs-only sync of this plan file against actual git state.** After the `244e9fd` (Sprint 6) and `18edd26` (continuity_slice follow-up) pushes shipped, the plan lied in three places. Seven targeted edits squared it up: Sprint 6 Status Board row (line 79) dropped "(local, awaiting Jeffrey eyes-on before push)" + appended the pushed-at hash; the 3 stale Sprint 5C task checkboxes (lines 242-244) flipped to `[x]` with `2387cef` pointers and one-line summaries; the 4 stale Sprint 6 task checkboxes (lines 249-252) flipped to `[x]` with `244e9fd` pointers and one-line summaries; the Sprint 5C entry's "Noted v2 follow-up: `build_reroll_line_request` leaves `continuity_slice` empty" sentence struck through and updated to point at the new `18edd26` follow-up entry below; the Sprint 6 entry header replaced `Commit <TBD-after-push>` + the "LOCAL COMMIT ONLY -- push held" warning with the real hash + pushed-at date; the Sprint 6 entry tail's "Pending Jeffrey eyes-on (push gate)" paragraph replaced with a "Push model -- fix-forward" paragraph recording Jeffrey's at-push decision (push promptly, review against pushed history, post-live-run reversions ship as new commits on top per Prime Directive 1); the Sprint 6 entry's final sprint-state line updated to "0, 1, 2A-2E, 3A-3G, 4, 5A-5C, 6 COMPLETE" (no LOCAL qualifier). A new dated Build Progress Log entry for `18edd26` was appended at the bottom.
- **Process recovery note (not blocking).** First pass of these 7 edits hit a snag worth recording: after the Edits landed, the Cowork Linux bash sandbox returned a stale snapshot of the file (`wc -l` 548 + mid-sentence `tail`). I misread that as "the file got corrupted by my Edits" and ran `git checkout -- <file>` via Desktop Commander to revert -- which wiped 7 clean Edits. Recovered by re-Reading the file (forces the harness file-state tracker to re-snapshot) and re-applying every Edit; the second pass verified only via Desktop Commander. The OTR handoff's standing note covered this exact failure mode ("Cowork Linux sandbox `Bash` mount is STALE for this repo -- use Desktop Commander for git + tests"); reading and obeying it up front would have prevented the detour. Folded into the existing `cowork-sandbox-corruption` global memory.
- No code touched -- no node `INPUT_TYPES` / widget / output socket / workflow JSON. Prime Directives 3 + 6 N/A.
- **Regression:** not run -- docs-only change. The deferred-regression baseline for `18edd26` (queued for post-live-run) stands.
- **New bug ids:** none.
- **Sprint state:** unchanged -- Sprints 0, 1, 2A-2E, 3A-3G, 4, 5A-5C, 6 COMPLETE.

### 2026-05-25 -- Pre-live-run nit cleanup (three deferred follow-ups)

- Commit `28d8e0f` on `v2.0-alpha` -- 3 files, +15/-12. Predecessor HEAD `a84a3d6`. Pushed 2026-05-25.
- **Three small follow-ups flagged in this Build Progress Log all bundled into one commit** -- behaviour-neutral, all live in code that pytest never collects, none touch production runtime paths. The risk profile is identical and each one is too small to ship alone.
- **`nodes/_otr_line_composer.py` `__main__` self-test:** six `compose_line(mock_fn, req)` positional calls fixed to the keyword-only form (`creative_fn=` + `req=`). Stale since Sprint 3A (`e24b327`) made the signature keyword-only; running the self-test today would raise TypeError on the first call. Tests 4-8 + Test 10 all converted.
- **`nodes/OTR_LedgerScriptWriter.py` `__main__` self-test:** the `n_optional == 14` assertion bumped to `== 15`, message + print line updated to match ("11 widget-surface + 4 Phase 4 v4 sampling knobs"; print "15 optional widgets"). `lemmy_cameo` (BUG-LOCAL-260) was added without bumping the count. Comment block expanded to record the count history: 15 (pre-removal) -> 14 (`optimization_profile` removed 2026-05-23) -> 15 (lemmy_cameo added).
- **`nodes/_otr_story_brief.py` `_PROPER_NOUN_STOPWORDS`:** dropped the duplicate `"interior"` from line 289 (line 286 retained). Harmless before the change (the frozenset dedupes) but cosmetic noise; flagged in the 2026-05-25 3F+3G entry.
- No node `INPUT_TYPES` / widget / output socket changed -- all three edits are in `__main__` self-test or pure-data scaffolding -- so no workflow JSON re-wire (Prime Directive 3 N/A). No LLM call site added or removed (Prime Directive 6 N/A). No public function signature touched.
- Sanity: AST parse OK on all three files (via venv python at `C:\Users\jeffr\Documents\ComfyUI\.venv\Scripts\python.exe`); UTF-8 no BOM; zero em-dashes (cp1252 trap); zero `dummy` / `DUMMY` tokens.
