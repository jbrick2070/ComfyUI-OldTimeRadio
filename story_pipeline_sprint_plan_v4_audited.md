...
| 4 -- VRAM hardening | IN PROGRESS | BUG-LOCAL-272 [FIXED] | `6b9300e` -- code-side verify: Zero-Prime Wash / Sovereignty 2.5 GB / 2B-12B caps / bf16+tf32 confirmed present; attn-selector dead code fixed (BUG-272); gate fires for renamed tier. Open: 14B 10.1 GiB cap decision, prompt-cache bullet, operator live-RTX-5080 confirm |
...
- [ ] Per-class VRAM caps in loader: 2B->3.2 GiB, 8-9B->6.8 GiB, 12B->6.8 GiB, 14B->10.1 GiB. **PARTIAL 2026-05-25:** 2B / 8-9B / 12B caps confirmed present; the **14B class is NOT capped at 10.1 GiB** -- it falls into the `total_vram >= 12.0` Sovereignty branch (13.5 GiB budget on a 16 GB card). Open decision for Jeffrey: keep the Sovereignty branch governing 14B, or add the explicit 10.1 GiB cap (touches the live load path -- needs RTX 5080 verification).
...
- [ ] Prompt cache on: `cache_prompt=True`, `n_cache_reuse` tuned. **NOT YET VERIFIED 2026-05-25** -- outside the Sprint 4 verify wave's four-bullet scope; confirm against the loader before closing Sprint 4.
