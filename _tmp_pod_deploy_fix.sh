...
FOLEY="$OTR/workflows/variants/otr_16gb_foley.json"
WANT=c2547657
...
test "$SHORT" = "$WANT" || {
  echo "HEAD_MISMATCH got=$SHORT want=$WANT"
  git rev-parse HEAD
  exit 5
}
...
# Stop the old Comfy so it loads the extractor fix. Do not touch the Foley JSON.
...
cd "$OTR"
: > /workspace/otr-config/foley_mystory_3act.log
...
echo "=== comfy ==="
grep -E 'got prompt|house DEFAULT_IDEA|All 25 nodes|source_bank|my_story|Error|Traceback' /workspace/otr-config/comfy_8188.log | tail -n 30 || tail -n 25 /workspace/otr-config/comfy_8188.log
echo "=== foley json still roll ==="
"$PY" - <<'PY'
import json
from pathlib import Path
p = Path("/workspace/runpod-slim/ComfyUI/custom_nodes/ComfyUI-OldTimeRadio/workflows/variants/otr_16gb_foley.json")
...
writer = next(n for n in data["nodes"] if n["type"] == "OTR_LedgerScriptWriter")
names = [i["widget"]["name"] for i in writer["inputs"] if isinstance(i.get("widget"), dict)]
print(writer["widgets_values"][names.index("source_bank")])
PY
