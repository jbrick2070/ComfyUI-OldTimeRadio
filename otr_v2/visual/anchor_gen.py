...
See ``docs/2026-04-16-2026-04-16-phase-b-anchor-pick/``
...
Design constraints (binding):
    ...
    C7  Audio-side files untouched.  This module writes ONLY under
        ``io/visual_out/<job_id>/anchors/`` and per-shot anchor PNGs.
...
# ``otr_v2.visual.anchor_gen`` AND from a bare ``python anchor_gen.py``
...
def _default_sd15_loader(
    *,
    ...
    # otr_v2/visual/configs/v1-inference.yaml for provenance.
...
def generate_for_shotlist(
    ...
    *,
    ...
    Args:
        ...
        out_root: ``io/visual_out/<job_id>/`` (the per-job output root).
...
def main() -> None:  # pragma: no cover -- exercised by integration only.
    ...
    out_root = job_dir.parent.parent.parent / "io" / "visual_out" / job_dir.name
