"""
otr_v2.visual.backends  --  video stack backend registry
=========================================================
...
resolves a backend by name (from the ``OTR_VISUAL_BACKEND`` env var,
plumbed through by ``bridge.py``) and hands the job directory over.

Contract is intentionally minimal so the existing VisualBridge +
VisualPoll + VisualRenderer trio stays untouched: the backend writes
``STATUS.json`` and per-shot assets into ``io/visual_out/<job_id>/``
...
def _make_video_stack():
    # Composite: flux_anchor still -> wan21_loop motion ->
    # promote loop.mp4 to render.mp4 for VisualRenderer consumption.
    from . import video_stack
    return video_stack.VideoStackBackend()
...
register("video_stack", _make_video_stack)
