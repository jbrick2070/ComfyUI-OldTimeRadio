"""
otr_v2.hyworld.backends._base  --  shared contract for all backends
===================================================================
...
_HYWORLD_DIR = _THIS_DIR.parent
if str(_HYWORLD_DIR) not in sys.path:
    sys.path.insert(0, str(_HYWORLD_DIR))
try:
...
class Backend:
    ...
    ``io/hyworld_in/<job_id>/``; output goes to
    ``io/hyworld_out/<job_id>/``.  The path convention is fixed by
    the HyworldBridge contract and must not change.
    """

    name: str = "base"

    def out_dir_for(self, job_dir: Path) -> Path:
        """Resolve the matching ``io/hyworld_out/<job_id>/`` directory."""
        job_id = job_dir.name
