...
`_otr_ledger_freeze.py:27-28`, `_otr_legacy_to_stage1_adapter.py:527`). Still OUT OF SCOPE: the
SceneSequencer `sfx_audio_clips`/`sfx_offset_ms` overlay (node 3) -- a separate audio feature; the
panel can assess it in a follow-up (it is not fed by sfx_cue).
