...
## R1 FIXES APPLIED (review the CURRENT code, not the original)
- `apply_selection_enable_set` now RETURNS a restore-record `{flag: prior}` and
  `run_real_episode` wraps `run_episode` in try/finally calling
  `_restore_enable_set` -- the env mutation is scoped to ONE render, never leaked
  session-global (both r1 panelists' top finding).
- The dynamic `OTR_LSYNC_BASE_ENGINE` base engine is now included in the derived
  set (it renders off-shot).
- New lock test `tests/test_video_selection_enable_set.py` (5 cases): set-from-
  selection, restore-to-unset, restore-prior-value, already-enabled-untouched,
  unselected-stays-gated, lsync-base-covered. Full suite + these green.
- Rejected as out-of-scope: forced-OOM soak crash + fallback dead-code (pre-
  existing, unrelated); force-map "beats" the dropdown (it is an explicit LOUD
  operator override = explicit consent).
