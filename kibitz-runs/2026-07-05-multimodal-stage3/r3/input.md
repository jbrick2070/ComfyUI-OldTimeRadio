# Multi-Modal Story Schema -- STAGE 3 SUB-PLAN (v3, post-kibitz r2)
...
Status: v3 (kibitz r1+r2 folded; r3 wiring w/ Sonnet fan-out + r4 pending).
...
- Route ALL tail reads through the pack: finish_visual_prompt (:552),
  ...
  char-scene / background-plate grade appends, AND `compose_still_word_prompt`
  (:823-864 -- era tail + IMAGE_GRADE_TAIL reads; r2 M2; its
  typography/backdrop maps stay Python per section 8). Constants move INTO
  the style module (single owner) or survive only as the extraction-test
  fixture.
- De-swallow the seams (r2 M1, section 1): ImportError-only shims; style
  errors propagate; AST pin.
