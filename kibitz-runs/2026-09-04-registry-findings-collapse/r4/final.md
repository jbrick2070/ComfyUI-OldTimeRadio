# Collapse the registry scan by the one-owner rule -- the build plan, closed at r4

**Status: CLOSED, build from this.** Phase 0 has SHIPPED (`2f12a696`).
...
assertion is order-dependent) AND `os.name` monkeypatched to `"nt"` (or the
test passes vacuously on a POSIX runner -- agy Flash, r4) -> True, `os.kill`
never called, exactly one WARNING across many calls. The warning names the
CONSEQUENCE, not just the cause: liveness is unknown without psutil, so lease
reclamation is disabled for this process (Gemini 3.1 Pro, r4). The trade this makes, named: today the ctypes
...
4. **Every migrated module uses the aliased import AT ITS OWN DEPTH**,
   because tests import many modules flat and the bare names collide.
   `ffmpeg.py:28-34` is the SHAPE, not the literal text: it says
   `from .ffprobe import` because it lives INSIDE `_otr_shared`, and a node
   that copy-pasted that line would fail to import (Gemini 3.1 Pro, r4 --
   the best catch of the round). By depth: `nodes/_otr_shared/*.py` uses
   `from . import env as otr_env` then flat `import env as otr_env`;
   `nodes/*.py` and the root use `from ._otr_shared import env as otr_env`
   then `from _otr_shared import env as otr_env`; the engine subpackages use
   `from .._otr_shared import env as otr_env` then the same flat form. The
   flat fallback is identical at every depth because `nodes/` is what goes
   on `sys.path`. `tests/test_master_mux_terminal_knob.py:90-96`
