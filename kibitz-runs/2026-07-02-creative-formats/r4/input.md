...
- visual_format SEMANTICS: !=standard sets all three role-slot
  DEFAULTS to the format row; a per-role pick differing from the
  slot's current default = explicit and preserved. Precedence:
  explicit pick > visual_format (widget/env) > profile
  DEFAULT-OVERRIDE map > registry default. Append-only widget +
  widget-vector tests.
