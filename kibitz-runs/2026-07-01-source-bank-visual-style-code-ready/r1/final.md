...
Do not edit `workflows/otr_scifi_16gb_full.json` during the upstream build.
The canonical workflow is touched only in a later transplant chunk, after the
fresh source brains, prompt profiles, and ledger-writing spec are ready and
tested.
...
8. Workflow and widget guardrails are hard, but deferred until transplant.
   - Do not touch the canonical workflow during upstream architecture work.
   - The later transplant chunk appends `source_bank` and validates widget
     counts.
   - `source_bank` goes into both headless creative whitelists when the
     transplant path is exposed.
   - Workflow JSON changes land only when the code path is ready to wire.
