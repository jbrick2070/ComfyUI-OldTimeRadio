# ROUND 4 -- CONVERGENCE, and the DISCARDED-OBJECTIONS AUDIT

This is the final round of a four-round arc. r1 (arc), r2 (coding) and r3
(wiring) are on record. **Do not re-derive the designs. Do not re-open story
or prose quality -- closed by operator directive.**

## Your PRIMARY job this round

Each hardened plan carries a `discarded_objections` list: fatal-flaw findings
from nine adversarial review lenses that the harden pass REJECTED, each with a
stated reason. **NOBODY HAS EVER AUDITED THOSE REJECTIONS.** A wrongly-rejected
fatal flaw is a must-fix hiding behind a plausible reason -- it is the single
most likely place a real defect is still hiding in this campaign.

There are **19** discarded objections. Audit EVERY one. For each, answer only:
...
2. If true, does it actually **dispose of the objection**, or only part of it?
   A rejection that answers a narrower claim than the one raised is a MISS.
3. If the rejection is wrong, what is the **minimum change** that restores the
   objection, and is it a must-fix or a note?

Rank them. Say plainly which rejections you believe are WRONG.

The driver has already audited D01 and found the cited rng tally does not
reproduce while the conclusion survives -- see the anchor. Verify that finding
independently; do not simply adopt it.

The single highest-stakes rejection in the set is **D13**: all three prior
reviews UNANIMOUSLY said to edit canonical node 87, and the harden overruled
all three on the strength of one probe. If that rejection is wrong, Track 2's
entire live proof is unrunnable as written. Give it real scrutiny.
...
# THE 19 DISCARDED OBJECTIONS, IN FULL
...
**THE OBJECTION THAT WAS RAISED:** Reviewers 2 and 3: the fix for the pinning regression is to call `_plan_gender_distribution(len(unpinned), prior_genders, rng)` with prior_cast only, and interleave the pins into the untouched positions.

**WHY THE HARDEN REJECTED IT:** The DIAGNOSIS is confirmed and load-bearing -- probe: `_plan_gender_distribution(1,['male'])` returns female 400/400, so feeding pins into prior_genders makes the unpinned slot deterministically wrong. But the proposed FIX is rejected on two measured grounds. (1) It still changes the writer's draw count: probed, the call consumes 0 rng calls at count 0 and 1, 2 at count 2 and 3, 4 at count 4 and 5 -- so pinning both slots of the shipped 2-character shape leaves the writer two draws behind the replay, re-creating the very desynchronization Step 5 exists to close. (2) It makes a lone unpinned slot deterministic anyway: `_plan_gender_distribution(1,[])` returns male 400/400. Override-in-place is strictly better on both counts and was probed against the real module: rng call count and post-call stream IDENTICAL on all 200 seeds, and the unpinned HORATIO distribution unchanged at 200 male / 200 female.

### D02

**THE OBJECTION THAT WAS RAISED:** Reviewers 1 and 3: specify the address-form tier as SAME-LINE-ONLY (skipping the line-initial speech prefix), which resolves 8 unknowns with zero contradictions, and add LEONATO/URSULA/VIOLA as regression pins.

**WHY THE HARDEN REJECTED IT:** Their finding that the rule AS WRITTEN produces wrong genders is confirmed and I am keeping it out of the plan -- but the corrected tier is rejected as a STEP, not softened. Reviewer 3 measured its own payoff at +0 pinned slots at the shipped num_characters=2 and +1 at num=3. Worse, it cannot be run correctly at all: the whole-play text it needs is not on disk (config/source_banks/_corpus/ holds 64 Gutenberg PROSE bodies and exactly 1 sidecar, zero Folger plays; all 14 vendored Shakespeare .txt files are sliced scenes), `body` is rebound to the sliced scene at scripts/otr_fetch_public_domain.py:325 one line after the roster parse at :324, and re-vendoring needs the network. A committed 12-entry supplement is offline, deterministic, closes strictly more (including ANTIPHOLUS and DON PEDRO, which no address form reaches), and carries zero risk of contradicting a confirmed roster fact.

### D03

**THE OBJECTION THAT WAS RAISED:** Reviewer 2: HELENA/HERMIA/LYSANDER/DEMETRIUS carry EMPTY descriptions in midsummer__act3_scene2.provenance.json -- a vendor-time `parse_character_roster` defect that is a cheaper root fix than any of Steps 2-7.

**WHY THE HARDEN REJECTED IT:** Not a dropped-entry defect and not cheaper. Verified in the shipped sidecar: all four carry a populated `roster_name` ('HERMIA','LYSANDER','DEMETRIUS','HELENA'), so the parser FOUND them in the cast block -- the fetched Folger text simply did not gender them there. Diagnosing further requires the whole-play text, which is not vendored anywhere in the repo, and any fix requires a network re-vendor of all 14 scenes plus a parser change of unknown blast radius against a module whose 23 tests currently pin its behavior. Four committed supplement lines are the cheaper root fix here.

### D04

**THE OBJECTION THAT WAS RAISED:** Reviewer 3: forbid 'other' on source-owned slots -- restrict the unpinned adaptation allocation to {male, female} so Step 4 does not make the un-castable 'other' gender deterministic on adaptation scenes.

**WHY THE HARDEN REJECTED IT:** The underlying defect is real and I have kept it (Step 6): `assign_voice_for_slot(engine='indextts2', gender='other')` raises VoiceCastingError live, CastLock catches at cast_lock.py:616-620 and continues without stamping, and _otr_voice_node_common.py:109-127 then picks a gender-blind reference. But the premise that Step 4 makes it worse is false under my design. Probed over 400 seeds on the real module at num=3 with two slots pinned: TOBY's gender distribution is {female 133, other 136, male 131} both today and with the pin -- byte-identical, because the allocator is never re-called. Restricting the allocation would change the roll for unpinned slots and destroy the 'never worse than today' property that makes Step 4 safe. The pre-existing hole is fixed where it lives, in the ledger stamp.

### D05

**THE OBJECTION THAT WAS RAISED:** Reviewer 1: Step 5 must pass `source_bank_id` into the replay's `assemble_pre_locked_rows` -- 'the exact asymmetry that created this bug'.

**WHY THE HARDEN REJECTED IT:** The action is accepted (it costs nothing and I have included it); the CHARACTERIZATION is rejected because it would misdirect the next reader. Read at nodes/_otr_casting.py:1270-1276 and :1302, `source_bank_id` affects only the Lemmy exclusion and a LEMMY filter on the source queue, and it consumes ZERO rng. The bug is `pick_first_last(rng, ...)` at :1313 burning draws that `source_queue.pop(0)` at :1311 does not -- proved by probe: forwarding source_character_names alone already restores the writer/replay match exactly at cast_seed 424242 with source_bank_id absent.

### D06

**THE OBJECTION THAT WAS RAISED:** Original plan Step 6(a) and its reviewers' partial defence: rename the cast_hints 'Puck' -> 'Robin' and 'Don Pedro' -> 'Prince' in curated_scenes.sample.json so the hint matches the Folger speech prefix.

**WHY THE HARDEN REJECTED IT:** Dropped entirely, siding with reviewers 1 and 3 against the original plan. Verified: ROBIN carries roster_name='' with gender_source='absent_from_roster' and PRINCE is gender='unknown', so both renames convert a no-join into a join that still returns unknown -- zero gender payoff for a change to a name the audience hears, which brushes the closed story-QUALITY scope. The supplement pins DON PEDRO by its existing hint with no manifest edit at all.
...
**THE OBJECTION THAT WAS RAISED:** Reviewer 2 must_fix: 'Step 7 misses a third copy of the announcer pool -- nodes/story_orchestrator.py:861 holds its own _ANNOUNCER_PRESETS list, mutated at :602. If step 7 is about removing the trap, name all three.' (The prior harden attempt folded this in as a step-7 requirement.)

**WHY THE HARDEN REJECTED IT:** Grounded false. nodes/story_orchestrator.py:857-862 is a BARK preset list -- ('v2/en_speaker_0','Male, authoritative, deep'), ('v2/en_speaker_1', ...), ('v2/en_speaker_4', ...), ('v2/en_speaker_9', ...) -- under the comment 'ANNOUNCER always uses neutral English (en_speaker_*)'. It is a different engine's pool with entirely different values; it shares only a variable name with the kokoro pool. There are exactly TWO copies of the kokoro announcer pool: nodes/_otr_audio_engines/eng_kokoro.py:26 and config/cast_pools.py:304-309. Cross-referencing the bark list into a kokoro drift guard would create a false equivalence in the very comment meant to remove a trap.

### D08

**THE OBJECTION THAT WAS RAISED:** Reviewer 3 must_fix: 'STEP 6 FLOOR=2 IS UNDER-ARGUED; FLOOR=3 DOMINATES ON THE STATED GOAL ... the operator complaint is same voices every night, not wrong timbre.'

**WHY THE HARDEN REJECTED IT:** Over-correction, measured. Counting combos whose first accepted tier is above the gender-only tier across all 24 (engine x gender x writer-word) pairs against the shipped bank: floor 1 = 8/24, floor 2 = 5/24, floor 3 = 2/24. Both floors remove every 100% pin (size-1 first tiers: 3 today, 0 at floor 2, 0 at floor 3). Floor 3's extra spread is bought by making the timbre dimension inert for 22 of 24 combos -- it deletes a working feature rather than fixing a broken one. Floor 2 is the minimal root fix and floor 3 remains one env var away for a live A/B, with the floor folded into stable_cast_seed so the two settings can never claim the same policy string.

### D09

**THE OBJECTION THAT WAS RAISED:** Reviewer 3 must_fix: 'STEP 1 vz_/cb_/dia_ FAMILY WORK IS ALREADY DONE AND IS UNREACHABLE ANYWAY ... Scope step 1 to those two pairs plus the three new operator takes, and drop the mirror-stamping busywork.' (The prior harden attempt adopted this verbatim.)

**WHY THE HARDEN REJECTED IT:** Right premise, wrong scope. The 40 shared ref_paths are confirmed (e.g. models/tts/refs/indextts2/vz_bill_boerst.wav -> vz_/cb_/dia_bill_boerst), so stamping mirrors of IDENTICAL files is indeed redundant. But the mirrors OF THE TWO SAME-HUMAN PAIRS are not redundant: cb_pd_librivox_mark_f_smith and cb_pd_librivox_mark_f_smith_elder are two chatterbox rows with two DIFFERENT ref_paths, both castable in one chatterbox episode, and the same holds for cb_ljspeech vs cb_pd_ljspeech_linda_johnson and for the dia_ mirrors. Casting being single-engine is exactly why each engine needs its own stamp. Correct scope is 12 rows (2 pairs x 2 members x 3 engines), not 6.

### D10

**THE OBJECTION THAT WAS RAISED:** Original design step 4(c) / reviewer 2 must_fix: drop the 'not allow_voice_reuse' clause at nodes/cast_lock.py:535-537 so the announcer voice is always reserved -- 'today a character could be cast the announcer's own voice'.

**WHY THE HARDEN REJECTED IT:** The live-bug framing is false and the change does not belong in this track. The reservation only fires when target_engine == announcer_engine, and the canonical workflow ships indextts2 characters with a kokoro announcer (node 80 ['default','auto_registry',True,'indextts2','kokoro','cuda']), so the branch is unreachable in production. Worse, dropping the clause silently re-bases chatterbox and dia casting whenever allow_voice_reuse is True -- and node 80 ships True. Rejecting it keeps step 1 independently green with a single, provable behaviour change; if it is ever wanted it is latent-path hardening deserving its own commit and its own C7 risk entry.

### D11

**THE OBJECTION THAT WAS RAISED:** Reviewer 1 fatal_flaw 2 / reviewer 3 fatal_flaw 3: re-derive the concentration thresholds (top1 < 12%, top3 < 25%, per-slot < 20%) from a reproducible simulation, or raise criterion (c) to ~25%.

**WHY THE HARDEN REJECTED IT:** Diagnosis accepted, remedy rejected. The two panels' reconstructions of the same stated method produced incompatible baselines (indextts2 top-3 25.0% vs 13.4%), and reviewer 3 is right that a per-slot < 20% assertion is unsatisfiable at floor=2 by construction. But re-deriving a threshold reproduces the original error class -- a number tuned to one simulation's arbitrary gender shape, slot count and seed range, which any future bank edit invalidates. Percentages are the wrong instrument. The plan asserts a structural invariant instead (no first-accepted tier of size 1) which is measured directly against the shipped bank, requires no simulation, fails today on exactly three named combos, and cannot drift.

### D12

**THE OBJECTION THAT WAS RAISED:** Reviewer 3 fatal_flaw 2 / prior harden attempt: 'Steps 5 and 6 are not separable. Land them in ONE commit.' (i.e. the synonym table must ship in the same commit as the tier floor.)

**WHY THE HARDEN REJECTED IT:** True in one direction only, and the plan exploits the asymmetry. Measured size-1 first-accepted tiers: today 3, synonyms alone 5, floor alone 0, floor + synonyms 0. So synonyms BEFORE the floor is a regression (confirmed -- it manufactures four new pins), but the floor before synonyms is strictly an improvement and fully self-contained. Merging them forces one commit that touches the caster, the dataclass, the match semantics and the policy version at once, and it wrongly implies the synonym table is required to fix the defect. It is not: the floor alone removes every pin. Ordering floor-first makes both changes independently green and lets the synonym table be deferred or dropped without losing the fix.
...
**THE OBJECTION THAT WAS RAISED:** Unanimous across all three reviews: step 8/9 must EDIT workflows/otr_canonical.json node 87 to move character_video_model off viz_camera, because 'NOTHING in config/profiles/*.json sets character_video_model, so there is no run-time profile retarget to lean on'.

**WHY THE HARDEN REJECTED IT:** The premise is confirmed but the fix is wrong. Probe: with the canonical trio still_consumer_capabilities is {'announcer_visual': False, 'music_visual': False, 'character_video': False}, roles_requiring_stills is frozenset(), and derive_image_prompts returns {'version':1,'objects':[]} -- so the canonical file as saved really does mint zero stills. But the grep was for the WIDGET name. The PROFILE key is role_overrides.character_visual, and config/profiles/widget_mapping.json maps managed key 'role_overrides.character_visual' to ['OTR_VideoDirector','character_video_model']. I probed all 46 shipped profiles: ~40 set it to a still-consuming engine (otr_w45_still_flat -> still_flat, otr_g4_humo -> humo, otr_w45_ltx_video -> ltx_video, otr_g4_wan_ti2v -> wan_ti2v, all character_video True). scripts/otr_canonical_api_run.py loads the real canonical JSON (with an explicit path assertion at :146-151) and applies the profile through the single applier at :156-158. This is also how the published Tempest episode got portraits at all. Editing the canonical widget would mutate the operator's production configuration to satisfy a test run; naming a profile is the smaller, shipped, section-0-compliant answer.

### D14

**THE OBJECTION THAT WAS RAISED:** Step 3(b) / step 4's schema additions to CanonicalImage and ImageRequest are needed so the extra='forbid' assertion at tests/test_image_platform_c1.py:1132 stays green.

**WHY THE HARDEN REJECTED IT:** Confirmed dead. Grep over the repo (excluding tmp/) finds CanonicalImage and ImageRequest constructed only in nodes/_otr_image_engines/schemas.py:49,73,102 and tests/test_image_platform_c1.py:1126-1132. The dispatcher builds plain dicts at :1170-1194 carrying ten keys CanonicalImage(extra='forbid') would already reject (content_hash, w, h, kind, pool_path, char_id, beat_id, radio_host_style, lettering_style, backdrop_family, mesh_subject_id) and never validates them, and the cited assertion passes nope=1 and EXPECTS a raise -- it was never at risk. The schema edit is dropped entirely rather than kept as decoration; the row build is the field's one owner.

### D15

**THE OBJECTION THAT WAS RAISED:** Name the new request field 'reference_images' (plural) to match eng_google_image._reject_reference_inputs, rather than adding the singular key to that guard.

**WHY THE HARDEN REJECTED IT:** eng_google_image.py:144-151 checks init_image and reference_images with a plain `or`; extending it with a third key is one line. A plural name carrying a single path is a type lie that every downstream adapter would then have to interpret, and _zimage_params / _klein_params would need list handling for no benefit. The guard gap the reviewers found is real and is closed in step 5(b) -- only their proposed remedy is rejected.

### D16

**THE OBJECTION THAT WAS RAISED:** Scale the reference with crop='center' to the OUTPUT dimensions (params width/height) so the face region survives a 16:9 mint (prior harden attempt, step 7).

**WHY THE HARDEN REJECTED IT:** Backwards for this geometry. Portraits are 832x1216 and character scene stills are 16:9 (~1472x832); comfy.utils.common_upscale with crop='center' center-crops the SOURCE to the target aspect first, which on a head-and-shoulders vertical portrait cuts the top of the head -- and it inflates the reference to roughly 4,000 latent tokens on an 8-step turbo mint. ComfyUI's ImageScale supports width=0 and derives the missing side from the actual image (nodes.py:1885-1889), and comfy.sd.VAE.encode calls vae_encode_crop_pixels internally (comfy/sd.py:1253), so width=0 / height=768 / crop='disabled' preserves the whole face at the true aspect for about a third of the tokens. All five ImageScale arguments are still spelled out -- that part of the objection is accepted.

### D17

**THE OBJECTION THAT WAS RAISED:** Correct the plan's loose numbers: flux-2-klein-4b-Q4_K_M.gguf is 2.43 GB not 2.6 GB, and z_image_turbo_nvfp4.safetensors is 4.2 GB not 4.5 GB.

**WHY THE HARDEN REJECTED IT:** A GiB/GB unit confusion in the reviewer, not a drift in the plan. Measured on disk: flux-2-klein-4b-Q4_K_M.gguf = 2,604,311,104 bytes (2.60 GB decimal / 2.43 GiB); z_image_turbo_nvfp4.safetensors = 4,509,509,600 bytes (4.51 GB decimal / 4.20 GiB). The original figures were decimal-GB and correct.

### D18

**THE OBJECTION THAT WAS RAISED:** Gate the four reference node keys on a truthy params['reference_image'] inside _node_candidates (change flux2_klein's signature to _node_candidates(self, params=None)) so resolution stays lazy.

**WHY THE HARDEN REJECTED IT:** The signature change is needed and kept, but params-gating _node_candidates does NOT make resolution lazy and would ship a latent episode killer. EngineRegistry.register stores ONE instance (nodes/_otr_shared/engine_registry_base.py:148-150) and render_image caches the resolved map on it (`classes = getattr(self,'_classes',None) or resolve_graph_classes(...)`; `self._classes = classes`, z_image_turbo.py:318-320, flux2_klein.py:251-253). The episode's first mint is a portrait with no reference, so the cached map would lack the ref keys and every later referenced mint would die with GraphExecutionError 'node ... class ... unresolved' -> ImageRenderError NO FALLBACK. Steps 7-8 use a SEPARATE _ref_classes map merged per call instead.

### D19

**THE OBJECTION THAT WAS RAISED:** Extend the identity-seed pin to jump segments so a multi-clip character beat holds one face (repo-rules review, must-fix #1).

**WHY THE HARDEN REJECTED IT:** Half right, and the wrong half would break a shipped contract. Jump segments DO need the anchor stamp and the reference -- folded into steps 3 and 6, gated on kind in ('scene_character', coverage_plan.JUMP_STILL_KIND) with a truthy char_id. But they must NOT take the pinned seed: tests/test_multiclip_jump_stills.py:234-237 asserts three DISTINCT seeds across a base plus two segments and :540-562 asserts a segment must not reuse the bookend's fixed seed, and otr_image_gen_dispatcher.py:718-729 documents the kind rewrite as the deliberate mechanism that makes a jump CUT a cut. A char-keyed pin would render the same frame twice.


---

# CONTEXT: the plans these rejections belong to

## Cross-track critic (ship order + cuts)
...
# WHAT r4 MUST DELIVER

1. **The audit table** -- all 19, each marked REJECTION SOUND / REJECTION PARTIAL /
   REJECTION WRONG, with `file:line` evidence. This is the deliverable.
2. **Any remaining MUST-FIX** that would break the build, the ledger, or a shipped
   contract. Ledger completeness is a standing operator rule: the new fields are
   `cast_source_contract`, `derived_from_portrait_hash`, `portrait_anchor_mode`,
   `voice_cast_fallback`. Each needs exactly one owner and a defined value on every
   path.
3. **A convergence verdict:** is this queue safe to build in the critic's ship
   order, or is there an ordering hazard nobody has named?

Note these items are CUT and are NOT to be re-opened (except to say a cut breaks
something else): kokoro char_voice (Track 3 Step 5), timbre synonyms (Track 3
Step 8), the drift guard (Track 3 Step 6), speculative klein wiring (Track 2
Step 8), and Track 1 Step 5 which is downgraded to a PROD_BUG_LOG entry with
reproducing seed 424242. D05 and D12 may therefore be moot -- say so if they are.
