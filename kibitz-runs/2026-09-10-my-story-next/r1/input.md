...
**Next action:** complete the newly requested Kibitz R1-R4 arc on these next
sprints, apply the grounded decisions here, then implement Sprint A and run
the real My Story writer/ledger dry run and regressions. The older queue below
...
|---|---|---|
| **A. My Story ledger proof** | Remove unnecessary rejection gates; use existing repairs; preserve the listener's full material and actual act topology. | Focused/full/Bible regression results and real canonical component ledgers, with freeze verdicts. |
...
1. Audit every My Story refusal against the latest operator contract. Keep
   schema/readability, unique identities, valid references, usable voices and
   required fields. Distinguish these from optional planning metadata and
   requested shape preferences. Do not add prose quality or timing gates.
2. Proposed act-count policy: requested acts remain prompt guidance and a
   ...
   disagree about ledger topology. No Python prose merge, trim or fabrication.
3. Keep the existing bounded structured repair ladder for actual ledger-shape
...
4. Do not broadly turn on the local schema binder as a quick fix: the shared
   ...
   to this sprint.
5. Update D1, My Story guide, tests and GO_FORWARD to the new policy together.
   Review the full changed behavior before calling the sprint ready.
...
longer dialogue is not rejected for count or duration; genuine broken
...
Credit the executed creative model, not the technical slot or a virtual picker
handle. Reuse actual cache-entry identity while the existing scheduler has it:
local/GGUF `model_id`, Comfy Credits `slug`, Google `google_model`, OpenRouter's
response-reported resolution keyed by the requested cache-entry slug. Record
distinct creative models if routing changed during a run; missing concrete
identity is disclosed rather than invented. Do not introduce model calls.

Stamp an Original-only model attribution and `credits_source_line` before
wire serialization and terminal save. The existing credits renderer already
consumes that field. Preserve My Story's human byline, and preserve source
authors on other banks. No spoken-story rewrite is needed for this printed
credit request. Existing `resolved_models` stamping occurs after script JSON
serialization; adding the new credit at that late site would miss the wire.

Tests: local and remote actual model identity, creative/technical separation,
alias changes and unreported resolution, no default operator author, same
credit in persisted ledger/output JSON/rendered credits, unaffected My Story
and source-adaptation attribution. No widget or canonical wiring change.
...
still works. Render through the actual credits function with real fonts and
