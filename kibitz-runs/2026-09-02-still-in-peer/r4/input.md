...
## 11. r3 fold (Cursor grok-4.6-high, wiring; judgment in `kibitz-runs/2026-09-02-still-in-peer/r3/judgment.md`) -- seven must-fixes, all grounded, all taken

* The STAGE 3b wire was unspecified and the default copy collides: `run_graph` refuses an
  external id that is also a graph node (`wrapper_bridge.py:546-551`) and the parent's sampler
  graph carries the EmptyLatentImage node (`eng_ghost_signal.py:849-867`). The peer copies the
  parent's graph, DELETES the `latent` node and wires `latent_image` to the external
  `plate_init`.
* `plate_path` is a DIRECTORY (`otr_stills_dir` returns a path and does not create it,
  `_otr_paths.py:312-323`); the engine makes it and names the file.
* The frozen route wins at every reader (`render_driver.py:2104-2112, 5209-5236`), so D11's
  rewrite sits inside ShotLock's replay early-return BEFORE the durable stamp and moves the
  four surfaces together; the coverage contract is a no-op for a sibling pair
  (`frame_contract.py:506-509`) but is re-derived anyway.
* Two audited `run_graph` calls need two fresh `execution_records` lists
  (`wrapper_bridge.py:517-523`); the plate decode needs the batch-1 latent kept alive until it
  returns.
* The evidence-manifest generator emits version 1 against a live version-7 JSON and refuses to
  overwrite it (`build_video_evidence_manifest.py:270, 376-387`); the haunted sentence lives
  only in the JSON (`:27`). The peer's sentence is appended to the JSON by hand in the
  registration commit. The generator's inability to reproduce its artifact is noted for the
  plan as a separate defect.
* The guarded import goes between `eng_ghost_signal_official` (`__init__.py:333`) and the
  roster audit (`:359`).
* Also folded: no custom menu label (`_label_for` derives it, `otr_video_director.py:116-137`;
  D1's label is CUT); G3.7 greps the module that DEFINES `render_clip`, so the peer's module
  reads `negative_prompt` itself; the request pruner keeps `plate_prompt` / `plate_path`
  (`render_driver.py:1888-1951`) and the trace copies receipts wholesale
  (`otr_video_render_batch.py:329-334`), so the receipt projection is the only path for the
  plate fields; `plate_identity` uses the checkpoint's `(size, mtime_ns)`, never a per-shot
  2 GB hash; node 91 ignores unlisted PNGs (`otr_image_gen_dispatcher.py:1036-1053`), closing
  the verify-at-build item; the lab profile bakes no denoise -- grid legs set the env per leg.

## 12. The wiring contract (what r4 converges on; one window builds sections 10 + 12 together)

**Graph wiring, exactly.**
* STAGE 2 (parent's encode graph + one node): `NODE_PLATE_POSITIVE = "plate_positive"`
  (CLIPTextEncode{text: request.plate_prompt, clip: Wire('clip')}); positive and negative as
  the parent; the negative cond is SHARED by both samplers.
* STAGE 3a (`run_graph`, `audit_node_ids={"plate_sampler"}`, `execution_records=[]`, fresh):
  `plate_latent` = EmptyLatentImage{512, 288, 1}; `plate_sampler` = KSampler{model:
  Wire('base_model') (external, the prepare handle), seed: request_seed, steps: PLATE_STEPS,
  cfg: PLATE_CFG, sampler_name: PLATE_SAMPLER, scheduler: PLATE_SCHEDULER, positive:
  Wire('plate_cond'), negative: Wire('negative_cond'), latent_image: Wire('plate_latent'),
  denoise: 1.0}; terminal `plate_sampler` -> `plate_out[0]` is the batch-1 LATENT dict, KEPT.
* Python: `assert plate_out[0]["samples"].shape[0] == 1`; `init = dict(plate_out[0])`;
  `init["samples"] = samples.repeat(U, 1, 1, 1)`; `noise_mask` / `batch_index` replicated by
  the live `RepeatLatentBatch` rule (`ComfyUI/nodes.py:1317-1330`, copied at build, one rule).
* STAGE 3b (`run_graph`, `on_result=_register_patched` as the parent): the parent's sampler
  graph MINUS the `latent` node; `sampler.inputs["latent_image"] = Wire("plate_init", 0)`;
  `sampler.inputs["denoise"] = resolved`; `external_results = {"base_model", "positive_cond",
  "negative_cond", "plate_init": (init,)}`; patchers detached before decode as the parent.
* STAGE 4: the beat decode as the parent; then `plate_decode` (`run_graph`, `audit_node_ids=
  {"plate_decode"}`, `execution_records=[]` fresh) = VAEDecode{samples: Wire('plate_kept'),
  vae: Wire('vae')} with `external_results={"plate_kept": plate_out, "vae": owners["vae"]}`;
  PNG written under `request.plate_path` (`makedirs(exist_ok=True)`) as
  `<sanitised shot_id>_<plate_identity sha16>.png` via temp sibling + `os.replace`;
  `plate_sha256` = sha256 of the file bytes.
* `finally` (every exit): drop plate cond, `plate_out`, `init`, the decoded plate image;
  `reclaim_idle_models` after 3a and after the plate decode. All heavy imports inside methods.
* Render-time node instances on the fresh path: 11 (pinned).

**Request and driver.** `VideoRequest.plate_prompt: str = ""` (causal via
`sampler_inputs`) and `VideoRequest.plate_path: str = ""` (non-causal, a directory).
`build_request_from_shot` fills both after the frozen-route resolution and after `_vstyle`
(`render_driver.py:2104-2123, 2878-2918`) only when `getattr(eng, "wants_plate_prompt",
False)`; `plate_path = str(otr_stills_dir(episode_id) / "ghost_plates")`. The composer
`compose_plate_prompt(vstyle, ledger_meta)` lives beside `_compose_background_plate_prompt`'s
pattern (`otr_meta_brief_image_prompt.py:1839-1865`) and is imported lazily by
`render_driver`.

**Receipt.** `build_actual_receipt` projects `clip["qc"]["plate_sha256" / "plate_name" /
...
gains the id; `docs/evidence/video_evidence_manifest.json` gains the `admission_unenforced`
sentence (by hand); `docs/ENGINE_MATRIX.md` regenerated; `config/profiles/
otr_stillin_lab_5080.json` (`status: draft`, no denoise) + `build_variants --all/--check`;
`tests/fixtures/still_plan_head_parity.json` regenerated if the roster test asks.
