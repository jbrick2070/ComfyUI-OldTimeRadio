...
- Node 90 `OTR_ShotLock`: OUT `patched_ledger_json` -> link 256.
- Node 92 `OTR_VideoRenderBatch`: IN `patched_ledger_json`=link 260,
  `master_audio_path`=link 264 (from node 7 `OTR_EpisodeAssembler.output_path`
  fan [263,264]); OUT `clip_manifest_json`=[261,271,275].
- `last_node_id`=95, `last_link_id`=276.
...
## Option A -- dedicated producer node (what the plan literally says)
New `OTR_AudioMotionProfile` (id 96) inserted ShotLock -> [96] -> VideoRenderBatch:
re-point link 260's source through 96, add a master_audio_path fan to 96, output
patched ledger to 92. `OUTPUT_NODE=False`, no custom IS_CHANGED (cache by input
hashes) OR IS_CHANGED keyed on master_sha + beat timings.
