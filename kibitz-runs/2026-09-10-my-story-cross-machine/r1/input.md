# My Story cross-machine repairs: R1 review input

Review snapshot at 2499b411. GO_FORWARD_PLAN.md remains the only work queue.
...
|---|---|---|
| M1 | Constrained generation retains per-request token history on a resident model | Confirmed code defect relevant to Mac memory pressure; not yet proven sole cause of the OS kill. `_otr_constrained_generate.py:123-142` caches parser/prefix functions by schema. LMFE prefix functions own `TokenEnforcer.prefix_states` and `allowed_token_cache`; full token prefixes accumulate. Cleanup also creates local schema classes per row. |
...
| D2 | Mac cannot download another model with current free space | Correct storage refusal. No cache deletion, invented fallback, or raised budget. First fix M1 and assess the existing model/workload; preserve the resource evidence. |
