...
- In `tests/test_freeze_policy_readonly.py`, add a real-cascade test parametrized on `original` and `scifi_news_pro`, using the exact fresh fixture recipe below. Do not mock `_run_inline_safety_cleanup`, policy resolution or phase dispatch. Pass a callback that increments a counter then raises; assert the counter is zero, not merely that the exception did not escape. Verify original text, retired/not-applicable receipt, successful freeze with no post-audit errors, and a saved ledger. This is CPU validation of the real deterministic cascade, not full canonical rendering.
