...
Status: PRE-BUILD, Kibitz R1-R3 complete; R4 pending
Branch: `v2.0-alpha`
...
4. Exercise `run_exchange_prepass` with production-shaped cast dictionaries
   ...
   `(messages, *, temperature, max_new_tokens)` and its parsing of the
...
6. Prove the live mixed-group path rather than merely any exchange with a
   ...
   - Index `ledger.lines[]` by `beat_id`; those rows carry the usable
     `dialogue_slot_id` and speaker.
   - Walk the complete `ledger.beats[]` order. For each beat, join its line row.
     Missing line, invalid/non-`d###` slot, empty speaker, or reserved
     `ANNOUNCER` is a run break exactly as `_groupable` is used by
     `run_exchange_prepass`; do not filter the breaker away.
   - For each uninterrupted valid run, construct slots carrying beat id,
     dialogue slot id, and speaker, then call the real
   ...
   does, rerun; do not call the mixed-exchange leg qualified. Record the
