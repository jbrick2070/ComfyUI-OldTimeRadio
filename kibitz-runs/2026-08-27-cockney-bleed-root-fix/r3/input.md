...
Status: PRE-BUILD, Kibitz R1-R2 complete; R3-R4 pending
Branch: `v2.0-alpha`
...
`compose_line(creative_fn=creative, ...)` and inspect index 0 of the messages it
captures, asserting that entry has `role == "system"`:
...
4. Exercise `run_exchange_prepass` with production-shaped cast dictionaries
   ...
   no calls; assert against the captured role=`system` entry, never the joined
   user roster. This is a forward scope invariant; the direct mixed-group test
   above is the test that kills the current grouped implementation.
...
6. Prove the live mixed-group path rather than merely any exchange. Build
   candidate slots from the full ordered ledger beat/line sequence using the
   same non-empty-speaker, valid-`d###`, reserved-speaker breaks as
   `run_exchange_prepass`; then call the real
   `group_voiced_beats(min_size=2, max_size=3)`. Only afterward retain groups
   whose complete beat-id set is a subset of
   `meta.exchange_prepass_audit.beat_ids`. Grouping the accepted IDs alone can
   bridge a failed/singleton hole and invent a group that never ran. At least
   one retained group must include `LEMMY` and another speaker. If none does,
   rerun; do not call the mixed-exchange leg qualified. Do not add new ledger
   schema solely for this receipt.
