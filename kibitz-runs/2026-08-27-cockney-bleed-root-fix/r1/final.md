...
Status: PRE-BUILD, Kibitz R1 complete; R2-R4 pending
Branch: `v2.0-alpha`
...
1. Replace `roster_has_lemmy` with a private active-speaker predicate accepting
   `Sequence[str]`. Delete the dictionary/`char_id` roster behavior; no
...
3. Reject `str` and `bytes` as the top-level container before iteration: Python
   strings are sequences and would otherwise be inspected one character at a
   time. Inspect only a real sequence of speaker-name strings and fail loudly
   with `TypeError` if any element is not a string. This makes scalar names and
   future cast-row/object calls test failures instead of silent scope misses.
...
5. Preserve `system_prompt or ""` and return it unchanged when no active
   speaker is Lemmy.
6. Preserve `_COCKNEY_ORTHOGRAPHY_RULE` as the one canonical module-level text
   constant. The single static block is intentionally shared by Lemmy-only and
   mixed exchange calls; the other-character sentence is harmless on a
   one-speaker turn and avoids a second prompt branch.
...
5. A scalar string/bytes container raises `TypeError`; it must not silently
   iterate the characters of `"LEMMY"`.
6. A cast dictionary or `_CastShim`-like object raises `TypeError`; the API must
...
4. Exercise `run_exchange_prepass` with production-shaped cast dictionaries
   ...
   introduced after `_normalize_cast` and grouping. This is a forward scope
...
3. Qualify at least one `media_archive` episode. Prefer Gemma 4 E2B to match the
   two named `media_archive` evidence ledgers in P1.5; if that exact model is
   unavailable, record the resolved replacement rather than pretending the
   comparison is model-identical.
...
6. Prove the live mixed-group path rather than merely any exchange: reconstruct
   accepted groups from ordered ledger beats/lines, the audit's accepted
   `beat_ids`, and the real `group_voiced_beats(min_size=2, max_size=3)` rule.
   At least one accepted group must include `LEMMY` and another speaker. If the
   episode topology does not produce one, rerun; do not call the mixed-exchange
   live leg qualified. Do not add new ledger schema solely for this receipt.
