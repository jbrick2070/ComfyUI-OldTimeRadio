...
| E-12 | P0 | **MusicGen wiring IN sprint at C5g.** L-4 override. `get_story_brief_music_mood` is wired into `nodes/musicgen_theme.py`. Helper no longer ships "implemented-but-not-wired"; it ships wired. R-04 disposition flips from RESOLVED-as-deferred to SUPERSEDED. E-09 no-touch gate is amended (one-commit exception at C5g). Audio C7 baseline resets at C5g. **REVISED at C5g execution time (Option 2 path):** structural pytest wiring + 11 active tests ship in C5g; the actual audio C7 baseline reset b3sum captures (pre-C5g forensic + new canonical) and the E-16 isolation test are deferred to Sprint A's empirical-verification pass. Three runtime-gated `@pytest.mark.skipif(not OTR_REGRESSION_RUNTIME)` tests stage the assertions at C5g; Sprint A unlocks them after the GPU captures. SPRINT.md §C-final QA wording (E-27 / RR-B12) already commits the sprint to "pytest-only structural pass; no audio render quality verified in Sprint C" -- MusicGen audio reset is the same class of deferred artifact. | Specified at C5g (revised); affects L-4, R-04, E-09, §4 hard rule 1, §7 acceptance rows 38/39. |
...
| RR-A2 | A | HIGH | C5g audio reset blesses regression | ACCEPT → E-16. **REVISED at C5g execution time (Option 2 path):** E-16 isolation test ships as a runtime-gated `@pytest.mark.skipif(not OTR_REGRESSION_RUNTIME)` placeholder in C5g; Sprint A's empirical-verification pass captures the pre-C5g forensic b3sum + new canonical baseline + flips the isolation test live. | C5g (revised) + Sprint A |
...
## C5g — MusicGen integration (REVISED to pytest-only structural wiring per operator directive 2026-05-15 Option 2)

**Review.** L-4 override (§1.1). E-12 revised (§1.2). R-04 superseded (§1.3). §4 hard rule 1 amended audio C7 boundary. §A.6 MusicGen coupling. Refinement §6.3 mood-vocab spec. C5b helper `get_story_brief_music_mood` (already shipped at C5b). The MusicGen wiring at C5g brings story-flavor into the final audio so brief, FLUX env, FLUX portraits, LTX, HuMo, AND MusicGen all consume `meta.story_brief*`. Every downstream artifact carries the story's unique flavor.

**SCOPE REVISION at C5g execution time (Option 2 path):** the SPRINT.md spec below describes the original full-scope plan (structural wiring + audio C7 baseline reset b3sum captures + E-16 isolation test). At C5g execution time, operator directive 2026-05-15 split the work: the **structural wiring + 11 active pytest tests** ship in C5g; the **audio C7 baseline reset b3sum captures (E-12 step 1+3) and the E-16 isolation test** are deferred to Sprint A's empirical-verification pass. Three runtime-gated `@pytest.mark.skipif(not OTR_REGRESSION_RUNTIME)` tests stage the deferred assertions in `tests/test_story_brief_musicgen_c5g.py::TestRuntimeOnly`; Sprint A captures both fixture files (`audio_c7_baseline_pre_c5g.wav.b3sum` against parent commit `c86db57`, `audio_c7_baseline.wav.b3sum` post-C5g) and unblocks the gated tests. §C-final QA wording (E-27 / RR-B12) already commits the sprint to "pytest-only structural pass; no audio render quality verified in Sprint C" -- MusicGen audio is the same class of deferred artifact.
...
>
> Audio baseline note: Audio C7 baseline reset deferred to Sprint A per C5g revised scope (Option 2 path, operator directive 2026-05-15). C5g shipped the structural MusicGen wiring + 11 active pytest tests; the actual b3sum captures (`tests/fixtures/audio_c7_baseline_pre_c5g.wav.b3sum` + `tests/fixtures/audio_c7_baseline.wav.b3sum`) and the E-16 isolation test live behind `OTR_REGRESSION_RUNTIME=1` skip gates and are unblocked by Sprint A's first runtime-verification commit. Sprint A captures both fixture files (pre-C5g forensic against parent commit `c86db57`, new canonical post-C5g) and flips the three runtime-gated tests live.

**Wire / Pytest.** Wide pytest walk: confirm baseline + new test count.

**Commit gate.** All acceptance rows green (rows 38, 39 explicitly marked DEFERRED to Sprint A per C5g revised scope). Audio C7 held against existing baseline C2a → C5g (no audio-pipeline code change in any Sprint C commit; the MusicGen mood-prefix code path is status-gated and inactive when `meta.story_brief_status != "ok"`, so legacy ledgers and brief-failure runs preserve byte-identity). Runtime-status line in final QA. MusicGen rename-immune gate green AND semantic-gate visual inspection complete. Post-C contract for Sprint A documented (now includes the b3sum-capture handoff bullet). Branch pushed.

**Commit subject.** `C-final: Sprint C close -- meta.story_brief shipped (8 keys, 6 consumers including MusicGen wiring); legacy retired at C2a/C2b/C3/C3b; audio C7 baseline reset CAPTURED in Sprint A (runtime-gated tests staged at C5g, fixture files unblocked by OTR_REGRESSION_RUNTIME=1); runtime quality NOT PROVEN`
...
| 38 | Audio C7 baseline reset event: pre-C5g forensic b3sum + new canonical b3sum BOTH captured at `tests/fixtures/`. **DEFERRED to Sprint A per C5g revised scope (Option 2 path).** Runtime-gated test `test_story_brief_musicgen_c5g.py::TestRuntimeOnly::test_audio_c7_new_baseline_captured` stages the assertion; Sprint A's empirical-verification pass captures the fixture files. | C5g + Sprint A |
| 39 | Absent-brief isolation test (E-16 / RR-A2): force `story_brief_status="absent"`, output byte-identical to pre-C5g forensic b3sum -- proves core audio engine intact. **DEFERRED to Sprint A.** Runtime-gated test `test_story_brief_musicgen_c5g.py::TestRuntimeOnly::test_c5g_audio_matches_legacy_when_brief_absent` stages the assertion. | C5g + Sprint A |
