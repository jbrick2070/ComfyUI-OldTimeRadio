...
#: The English row's printed templates, kept here so an unreadable registry
#: still prints exactly the sentences this function always printed.
_ENGLISH_CREDIT_TEMPLATES = {
    "credit_adapted_from": "adapted from {src} ({tag})",
    "credit_adapted_generic": "adapted from a {tag} work",
    "credit_adapted_plain": "adapted from {src}",
    "tag_public_domain": "public domain",
    "tag_cc0": "CC0 / public domain",
    "credit_research_only": "adapted from {src} (research use only)",
    "credit_research_only_generic": "adapted from a source cleared for research use only",
    "credit_licensed": "adapted from {src}, used under {label}",
    "credit_used_under": "used under {label}",
    "credit_synthetic": "an original story generated by machine for this broadcast",
}


def printed_credit_line(provenance: Any, *, episode_meta: Any = None) -> str:
    """The printed ``credits_source_line`` for the credits roll. Never raises.

    Templates are the episode row's ``credits`` block; the English row is
    today's sentences character for character, and no meta means English.
    """
    prov = provenance if isinstance(provenance, dict) else {}
    status = str(prov.get("status") or "")
    src = str(prov.get("source_label") or "").strip()
    ...
    try:
        ...
        if not t:
            raise KeyError("empty credits block")
    except Exception as exc:  # noqa: BLE001 -- a printed credit never fails a roll
        log.warning("[provenance] credits templates unavailable (%s); "
                    "printing the English line", exc)
        t = _ENGLISH_CREDIT_TEMPLATES
    if status in ("public_domain_us", "cc0"):
        tag = t["tag_public_domain"] if status == "public_domain_us" else t["tag_cc0"]
        return (t["credit_adapted_from"].format(src=src, tag=tag) if src
                else t["credit_adapted_generic"].format(tag=tag))
    if status == "research_only":
        return (t["credit_research_only"].format(src=src) if src
                else t["credit_research_only_generic"])
    if status in ("licensed_commercial", "licensed_noncommercial"):
        if src and label:
            return t["credit_licensed"].format(src=src, label=label)
        if label:
            return t["credit_used_under"].format(label=label)
        if src:
            return t["credit_adapted_plain"].format(src=src)
    if status == "synthetic":
        return t["credit_synthetic"]
    return ""
