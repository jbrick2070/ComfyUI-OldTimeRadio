...
# THE MODELS ROOT IS NOT OWNED HERE ANY MORE.
#
# ``ModelsRootUnresolved`` and ``_models_root()`` moved to
# ``nodes/_otr_models_root.py`` unchanged -- same precedence, same refusal, same
# __file__ depth so step 4's sibling walk lands where it always did. They are
# re-exported here so the existing call sites keep working while they are
# repointed one at a time.
#
# WHY THEY LEFT: every subsystem in the pack asks where the weights live, and
# that answer had no business living inside one optional writer backend. While
# it did, this module could not be retired without taking the shared answer with
# it.
#
# THIS RE-EXPORT IS TEMPORARY. It exists so the extraction is one small green
# commit instead of a sweep of sixteen files plus a deletion in a single change,
# and it goes when this module does.
from ._otr_models_root import (  # noqa: F401  -- deliberate re-export
    ModelsRootUnresolved,
    _models_root,
)
