...
Architecture:
  1. Master Clock: audio duration × FPS = exact frame count
...
def _analyze_audio(audio_np, sample_rate, total_frames, fps):
    ...
    freq_data    : list[np.ndarray(32)]  — normalised 0-1 FFT bins per frame
...
class _CRTRenderer:
    ...
    def render(self, fi, total, fps, vol, freq, wave):
        ...
        # ── 1. Title bar (minimal — just show title + timestamp) ─────
        ...
        for i in range(n_bars):
            ...
            # Color: green → amber as energy increases
        ...
        # ── 3. ROTATING LISSAJOUS / ORBITING PARTICLES ──────────────
        # Phase-shifted sine particles orbit the ring
        n_particles = 12
        for p in range(n_particles):
            ...
            py = cy + int(orbit_r * math.sin(angle) * 0.6)  # elliptical
            # Particle size pulses with corresponding freq bin
            size = max(2, int(3 + freq[p % len(freq)] * 8))
            # Color cycles through green → cyan → amber
        ...
        # ── 4. GEOMETRIC GRID (background layer) ────────────────────
        # Subtle grid that warps with volume — like a CRT test pattern
        grid_step = max(40, self.w // 24)
        grid_alpha = max(8, int(15 + vol * 25))
        grid_col = (0, grid_alpha, int(grid_alpha * 0.4))
        # Vertical lines with slight wave distortion
        ...
        # ── 5. MIRRORED WAVEFORM (full width, large) ────────────────
        ...
        # ── 6. FREQUENCY BARS (wide, bottom section) ────────────────
        ...
        draw.text((pad, by - pad // 6),
                  f"OTR v1.0  ·  {self.w}×{self.h}  ·  {fps}fps",
        ...
        # Audio-reactive noise (stronger on louder frames)
        if vol > 0.3:
    ...
    def _waveform_mirror(self, draw, wave, x, y, w, h, vol, t):
        """Draw mirrored oscilloscope waveform — top and bottom halves."""
        mid = y + h // 2
        n = len(wave)
        pts_top = []
        pts_bot = []
    ...
    def _freq_bars_wide(self, draw, freq, x, y, w, h, vol):
        """Draw wide frequency bars with colour gradient."""
        n = min(32, len(freq))
        bw = w // n - 1
        for i in range(n):
            ...
            bx = x + i * (bw + 1)
            by = y + h - min(bh, h)
            # Color gradient: green (low freq) → amber (mid) → red (high)
            ratio = i / max(1, n - 1)
            if ratio < 0.5:
                r = int(ratio * 2 * 255)
                g = 255
...
def _encode_mp4(frames_iter, total_frames, audio_path, output_path,
                width, height, fps):
    """Pipe raw RGB frames to ffmpeg and mux with audio."""
    ffmpeg = _find_ffmpeg()
    if not ffmpeg:
        raise RuntimeError(
...
class SignalLostVideoRenderer:
    ...
    Takes the finished audio — produces a length-perfect MP4 with
    pure audio-reactive procedural art visuals.
    No AI video generation, no script sync: pure math + PIL + ffmpeg.
    """
        ...
        return {
            "required": {
                ...
                "script_json": ("STRING", {
                    ...
                    "tooltip": "Parsed script JSON (kept for pipeline compat, not displayed)"
                }),
                "production_plan_json": ("STRING", {
                    "multiline": True, "default": "{}",
                    "tooltip": "Production plan JSON (kept for pipeline compat)"
                }),
                "news_used": ("STRING", {
                    "multiline": True, "default": "[]",
                    "tooltip": "News JSON (kept for pipeline compat)"
                }),
            },
            "optional": {
                "fps": ("INT", {
