...
def copy_conditioning(out):
    ...
    ``n = [t[0], t[1].copy()]``), ``LTXVImgToVideo`` passes both through
    untouched, ``process_conds`` re-lists with ``conds[k][:]``
    (``samplers.py:1040``), and both ``calculate_start_end_timesteps`` and
