...
def wrap_cjk_chars(text: str, max_chars: int = MAX_CHARS_PER_LINE) -> list[str]:
    """Wrap CJK (and any non-spaced script) by character count, not spaces."""
    compact = str(text or "").replace("\n", "")
    if not compact:
        return [""]
    width = max(1, int(max_chars))
    return [compact[i:i + width] for i in range(0, len(compact), width)]


def _grapheme_clusters(text: str) -> list[str]:
    """Stdlib grapheme-ish clusters: combining marks + Devanagari virama."""
    import unicodedata
    virama = "\u094d"
    clusters: list[str] = []
    buf = ""
    for ch in text:
        if not buf:
            buf = ch
            continue
        if unicodedata.combining(ch) or ch == virama:
            buf += ch
            continue
        if buf.endswith(virama):
            buf += ch
            continue
        clusters.append(buf)
        buf = ch
...
def wrap_unicode_grapheme(text: str, max_chars: int = MAX_CHARS_PER_LINE) -> list[str]:
    """Wrap Hindi / Devanagari on grapheme clusters, not ASCII spaces."""
    clusters = _grapheme_clusters(str(text or "").replace("\n", ""))
    if not clusters:
        return [""]
    width = max(1, int(max_chars))
    lines: list[str] = []
    ...
    for cluster in clusters:
        if cluster.isspace() and not cur:
            continue
        if len(cur) + 1 <= width:
            cur.append(cluster)
        else:
            lines.append("".join(cur))
            cur = [] if cluster.isspace() else [cluster]
    if cur:
        lines.append("".join(cur))
    return lines or [""]


def wrap_text(text: str, max_chars: int = MAX_CHARS_PER_LINE,
              wrap_policy: str = "word_split") -> list[str]:
    """Dispatch the row's wrap_policy. Latin keeps wrap_words."""
    policy = str(wrap_policy or "word_split").strip() or "word_split"
    if policy == "cjk_chars":
        return wrap_cjk_chars(text, max_chars)
    if policy == "unicode_grapheme":
        return wrap_unicode_grapheme(text, max_chars)
    return wrap_words(text, max_chars)
...
def chunk_into_cues(text: str, max_chars: int = MAX_CHARS_PER_LINE,
    ...
    """
    phys = wrap_text(text, max_chars, wrap_policy)
    cues: list[str] = []
...
def _title_style_line(font_name: str | None = None) -> str:
    ...
    on Consolas. English keeps ``mono_font()`` -- today's card.
    """
    face = str(font_name or "").strip() or mono_font()
    return (
        f"Style: {TITLE_STYLE_NAME},{face},48,&H0041FF00,&H000000FF,"
        f"&H00000000,&H00000000,1,0,0,0,100,100,0,0,1,{TITLE_OUTLINE_W},"
        f"{TITLE_SHADOW},7,0,0,0,1"
    )


def _ass_header(style: dict, margin_v: int, title_font: str | None = None) -> str:
    s = style
    style_line = (
...
def build_ass_from_ledger(ledger_path, style: str = "sdh_standard",
    ...
    if isinstance(ledger, dict):
        try:
            ...
            cap = _EPLANG.row_from_meta(ledger.get("meta") or {}).captions
            wrap_policy = str(cap.get("wrap_policy") or "word_split")
            font_policy = str(cap.get("font_policy") or "")
            if font_policy == "devanagari":
                st["font"] = "Nirmala UI"
                title_font = "Nirmala UI"
            elif font_policy == "cjk":
    ...
    for li, ln in enumerate(dlines):
        ...
        cues = chunk_into_cues(label + body, wrap_policy=wrap_policy)
    ...
    ass_text = _ass_header(st, mv, title_font) + "\n".join(events + title_events) + "\n"
