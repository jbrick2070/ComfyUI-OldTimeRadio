"""
worker.py  --  sidecar worker (runs in main ComfyUI venv)
=========================================================
This script is spawned by OTR_VisualBridge as a subprocess.
It reads the contract files from io/visual_in/<job_id>/, dispatches
to a backend in ``otr_v2/visual/backends/`` (or falls back to the
Ken Burns stub when no explicit backend is selected), and writes
results to io/visual_out/<job_id>/.

IMPORTANT: The worker runs in the main ComfyUI venv (torch 2.10,
CUDA 13.0).  Do NOT import any OTR node code or ComfyUI modules --
the worker must stay a self-contained script.

Usage (called by bridge.py, not by humans):
    python otr_v2/visual/worker.py <path_to_job_dir>

Status protocol:
    The worker writes STATUS.json to io/visual_out/<job_id>/ with:
        {"status": "RUNNING"|"READY"|"ERROR"|"OOM", "detail": "...", ...}
    The poll node reads this file to determine completion.

Execution path (selected at runtime, no model contention with audio):
    1. Explicit backend dispatch -- OTR_VISUAL_BACKEND env var names
       a backend registered in ``otr_v2/visual/backends/`` (e.g.
       ``flux_anchor``, ``wan21_loop``, ``video_stack``).  This is the
       primary entry point for the v2 video stack.
    2. Ken Burns stub -- default when no backend is selected.  ffmpeg
       CPU pan/zoom over per-shot PNGs driven by the shotlist camera
       adjective.  Safe to run alongside Bark TTS on the GPU.
    3. Still stub (solid PNG) -- last-resort when ffmpeg is absent.

The retired Visual 2.0 / WorldMirror path (visual2 conda env) has
been stripped -- see the v2.0-alpha video stack backends for the
current real-inference code.
"""
...
# is launched as ``python otr_v2/visual/worker.py <job_dir>`` (no
# package context) or imported as ``otr_v2.visual.worker``.
...
try:
    ...
    # of otr_v2.visual), fall back to a no-op stand-in.  Today the
...
#
#   set OTR_VISUAL_ANCHOR=sd15   (PowerShell: $env:OTR_VISUAL_ANCHOR="sd15")
#
...
_ANCHOR_BACKEND = os.environ.get("OTR_VISUAL_ANCHOR", "").strip().lower()
...
# ``otr_v2/visual/camera_path.py`` as a deterministic trajectory
...
try:
    ...
    # env missing the rest of otr_v2.visual.  Matches the _atomic and
...
def run_stub(job_dir: Path) -> None:
    ...
    Used as the default when no explicit backend is selected via
    OTR_VISUAL_BACKEND.  Real generative inference lives in
    ``otr_v2/visual/backends/`` (see ``video_stack``, ``flux_anchor``,
    ``wan21_loop``, etc.).
    """
    # Derive paths
    job_id = job_dir.name
    otr_root = job_dir.parent.parent.parent  # io/visual_in/<job_id> -> repo root
    out_dir = otr_root / "io" / "visual_out" / job_id
    ...
    if _USE_ANCHOR_GEN and _anchor_gen_module is not None:
        ...
        try:
            ...
            # still gets the short form; full detail goes to io/visual_in/<job>/anchor_error.log
            import traceback
            try:
                # out_dir is io/visual_out/<job_id>; in_dir is io/visual_in/<job_id>
                err_log = out_dir.parent.parent / "visual_in" / out_dir.name / "anchor_error.log"
...
def _run_named_backend(name: str, job_dir: Path, out_dir: Path) -> bool:
    ...
    Ken Burns stub.  See ``backends/__init__.py``.
    """
    ...
    # script from any env that has the repo on PYTHONPATH.
    try:
        try:
            from otr_v2.visual import backends as _backends  # type: ignore
        except ImportError:
            sys.path.insert(0, str(_THIS_DIR.parent))  # add otr_v2/
            from visual import backends as _backends  # type: ignore
...
def main() -> None:
    ...
    # Explicit backend dispatch (v2.0 video stack entry point).  When
    # OTR_VISUAL_BACKEND is set to a registered backend name we skip
    # the Ken Burns stub entirely.  Unknown names fall through.
    backend_name = os.environ.get("OTR_VISUAL_BACKEND", "").strip().lower()
    if backend_name:
        try:
    ...
    # No backend selected -> Ken Burns stub.  Real generative inference
    # lives in the backends registry; the stub is the safety net for
    # workflows that haven't opted into the video stack yet.
    try:
        run_stub(job_dir)
    except Exception:
        _write_status(out_dir, "ERROR", traceback.format_exc()[-500:])
