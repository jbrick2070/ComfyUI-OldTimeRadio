...
**Prior last entry:** BUG-LOCAL-227 [FIXED S31.5 B1 2026-05-14] -- 25 LFC test failures latent at S30 B8 surfaced by wide `pytest tests/` walk during S31 B1 verification. Triage at S31.5 B1: 16 Bucket A deletions (15 in `test_lfc_phase_3_polish_in_cascade.py` for the deleted Phase 3 + 1 back-compat assertion in `test_lfc_w4_writer_polish_fn.py`), 9 Bucket B refactors (4 b1 + 4 c4 + 1 freeze_cascade widget test, all updated for the post-S30 B3 `technical_model` signature), 0 Bucket C skips. Plus 10 additional S31 symbol-relocation collateral surfaced at the wide walk re-run (3 vram_calibration retargeted to `_otr_model_loader.py`, 4 bark_ledger + 3 sequencer_ledger mock targets switched from `story_orchestrator._unload_llm` to `_otr_model_loader.unload_llm`). Final wide walk: 2080 passed / 8 skipped / 0 unexpected failures.
**Prior last entry:** BUG-LOCAL-226 [FIXED S31 B4 2026-05-14] -- S30 sprint-plan §2b audit claimed `nodes/story_orchestrator.py::_load_llm` is dead-runtime; a fresh grep at B0 kickoff caught a live caller chain `OTR_LedgerScriptWriter._resolve_news_seed -> _fetch_rss_seed_or_die -> _so._fetch_science_news -> _llm_rank_news_candidates / _llm_rerank_with_bodies -> _generate_with_llm -> _load_llm`. Fixed at S31 B4: all 4 legacy symbols (`_load_llm`, `_unload_llm`, `_LLM_CACHE`, `_generate_with_llm`) DELETED. The RSS news path was refactored at S30 B4b through `request_slot`; S31 B3 refactored the remaining internal callers onto `request_slot + make_generate_fn`; S31 B4 closed the deletion.
