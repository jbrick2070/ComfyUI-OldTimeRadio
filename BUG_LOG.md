...
### BUG-LOCAL-226: S30 sprint-plan §2b audit claim "_load_llm is dead-runtime" was wrong [LOGGED 2026-05-14, fix pending B4b]
...
- **Fix:** B0 narrowed to `__init__.py` forensic-comment scrub only (one stale "registered as an alias below" comment for OTR_LedgerScriptReviewer rewritten to say the alias is DEAD per S29). B4b inserted between B4 and B5: rewires `_fetch_science_news` / `_llm_rank_news_candidates` / `_llm_rerank_with_bodies` to use `_otr_model_loader.request_slot('technical', model_id)` instead of `_load_llm`; THEN deletes `_load_llm` / `_LLM_CACHE` / `_generate_with_llm`. Sprint count grows from 14 to 15 commits.
- **Verify:** `findstr /N "_load_llm _LLM_CACHE _generate_with_llm" nodes\story_orchestrator.py` returns 0 hits after B4b lands. The canonical workflow's RSS path still produces a valid news article (test: `pytest tests/test_news_history_ttl.py -v` + end-to-end audio fixture).
