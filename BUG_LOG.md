...
**Last entry:** BUG-LOCAL-225 (2026-05-13) -- general lesson surfaced by S28 Phase 5: a final QA gate that runs only the targeted-subset regression battery cannot catch pre-existing repo-wide static-quality issues (BOM signatures, AST-parse failures). The Bug Bible regression must run before the sprint-close commit lands; inheriting a pre-existing failure into the close commit is the same category error as BUG-LOCAL-223's `EXPECTED_FAILED_NODEIDS`-vs-actual-fail gap.
**Stack head when last updated:** S28 cleaner-break final QA commit `dfcc406` (s28-cleaner-break branch)
