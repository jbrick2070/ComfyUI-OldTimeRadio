...
**Last entry:** BUG-LOCAL-255 (2026-05-22) -- `override_announcer_close` matches the private `_speaker_role` key (stripped at `set_lines`), so `news_close_brief` is silently dropped every episode -- the announcer outro is generic composer text, not the journalistic close. Found while mapping the announcer dedicated-pass design; fix folded into that work (round-robin in progress). See body `### BUG-LOCAL-255`.
...
### BUG-LOCAL-255: `override_announcer_close` keys on the private `_speaker_role` -- `news_close_brief` silently dropped every episode [FOUND 2026-05-22 -- fix pending]
...
- **Fix:** pending -- folded into the announcer dedicated-pass work (design round-robin in progress 2026-05-22). The planned `compose_announcer_outro` LLM pass replaces the verbatim-stamp overlay outright, retiring `override_announcer_close`. Standalone hotfix if wanted sooner: add the `_speaker_role or speaker_role` fallback at `_otr_news_wiring.py:71` to mirror L108.
- **Verify:** an episode with a non-empty `news.news_close_brief` has its closing announcer line `text` equal to (or an in-voice rewrite of) that brief, not the generic outline-intent composer output.
