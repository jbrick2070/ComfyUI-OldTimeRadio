...
### BUG-LOCAL-259: Outline Stage 2 hard-crashes the run on an off-cast beat speaker [OPEN -- forward plan locked 2026-05-23]
...
- **Fix:** forward plan locked in ROADMAP `## HOTFIX -- Outline cast-drift crash` (synthesized from a round-robin consult; full context in `docs/2026-05-23-outline-cast-drift-problem-statement.md` and `-claude-analysis.md`). Ordered: (1) singleton-cast bypass -- skip the Stage 2 LLM when the cast has one character; (2) deterministic no-crash fallback for multi-character casts at the `skeleton is None` point in `generate_outline()`; (3) invert + lower the Stage 2 retry temperatures; (4) remove the "vary speakers" prompt contradiction; (5) minimal speaker normalization, no broad fuzzy matching; (6) GBNF dynamic cast-enum hardening, later; (7) maybe route Stage 2 to the technical slot, last. Steps 1-2 are the hotfix.
- **Verify:** pending implementation. Done when an operator run completes the writer outline with valid in-cast speakers and zero `OutlineFailedError`, for both a 1-character and a multi-character cast; plus `tests/` coverage for the singleton bypass, the deterministic fallback (no raise, valid skeleton), the new temperature schedule, and speaker normalization.
- **Tags:** writer, outline, llm-drift, hard-crash, cast-contract, pending-fix
