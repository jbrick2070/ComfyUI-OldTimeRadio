...
## 4a. Bug Bible Automation (Claude's Duty)

Claude is the Bug Bible curator for this project. When AntiGravity (or any agent) fixes a bug, Claude's QA review includes a Bug Bible triage step.

### The Pipeline

```
FIX_COMPLETE logged → Claude QA review → Bug Bible triage → Draft YAML entry → Push to survival guide repo → Update README entry count → Add regression test if verifiable
```

### Triage Filter

Not every bug belongs in the Bug Bible. Apply this filter:

| Add to Bug Bible | Skip |
|---|---|
| Bug class affects any custom node developer | Unique to OTR codebase only |
| Root cause is a ComfyUI/platform gotcha | Root cause is a typo or logic error |
| Fix requires non-obvious knowledge | Fix is self-evident from the error |
| Has appeared 2+ times (in OTR or elsewhere) | First occurrence, not yet a pattern |
| Verify step can be automated (static analysis) | Verify requires full runtime only |
...
- id: "<phase>.<next_seq>"
  ...
  symptom: |
    <what you see — from the operator's FIX_COMPLETE log>
  cause: |
    <why it happens — root cause, not the surface error>
  fix: |
    <how to resolve — concrete, actionable, copy-paste ready>
  verify: |
    <how to confirm — ideally machine-executable for the regression suite>
...
| Phase | Area |
|---|---|
| 01 | Bootstrap and Discovery |
| 02 | Environment and Dependencies |
| 03 | Registration and Loading |
| 04 | INPUT_TYPES and Widgets |
| 05 | Execution Model |
| 06 | Caching and IS_CHANGED |
| 07 | Tensors, Audio, Video |
| 08 | I/O and Output Nodes |
| 09 | Subprocess and Network |
| 10 | Safety, Pools, RNG |
| 11 | LLM-Specific |
| 12 | Regression, Git, Handoff |

### Three-File Contract (MANDATORY)

Every Bug Bible change must update all three files in sync:
1. `BUG_BIBLE.yaml` — the new or updated entry
2. `README.md` — entry count, coverage table if new phase/area
3. `tests/bug_bible_regression.py` — new test if verify step is automatable

Push all three in one commit. Never push a YAML entry without the README update.

### Bug Bible Repo

```
Local: C:\Users\jeffr\Documents\ComfyUI\comfyui-custom-node-survival-guide\
Remote: https://github.com/jbrick2070/comfyui-custom-node-survival-guide
```

### Logging

After pushing a Bug Bible entry:
```
[TIMESTAMP] [Claude] BUG_BIBLE_ENTRY: <id> — <one-line summary>
  SOURCE: OTR IT-NNN FIX_COMPLETE
  COMMIT: <sha> on comfyui-custom-node-survival-guide
```
