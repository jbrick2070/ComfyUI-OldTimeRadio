...
def test_the_operator_ratio_is_applied_exactly_once(tmp_path):
    ...
    assert stats["lanes"] == {"ltx25_foley_16gb": 1}
...
def test_stems_SPLAT_at_their_own_offsets_and_OVERLAPS_add(tmp_path):
    ...
    gain = fs.FOLEY_LANE_GAINS["ltx25_foley_16gb"][0]
...
def test_the_offset_is_integer_frames_not_a_float_multiplication(tmp_path):
    ...
    assert np.allclose(
        mixed[:, edge], 0.5 * fs.FOLEY_LANE_GAINS["ltx25_foley_16gb"][0],
        atol=1e-3)
...
def test_mime_zeroes_ONLY_its_own_windows_never_the_whole_episode(tmp_path):
    ...
    mixed, stats = fs.mix_foley_under_master(
        master, RATE, [_row(bed, 10 / FPS, 10, engine="ltx25_mime_16gb")],
        fps=FPS, lane_ids={"ltx25_mime_16gb"})

    assert stats["lanes"] == {"ltx25_mime_16gb": 1}
...
def test_a_mixed_episode_gives_each_lane_its_OWN_gains(tmp_path):
    ...
    mixed, stats = fs.mix_foley_under_master(
        master, RATE,
        [_row(foley, 0.0, 10), _row(mime, 20 / FPS, 10, engine="ltx25_mime_16gb")],
        fps=FPS, lane_ids={"ltx25_foley_16gb", "ltx25_mime_16gb"})

    assert stats["lanes"] == {"ltx25_foley_16gb": 1, "ltx25_mime_16gb": 1}
...
def test_the_global_master_gain_applies_to_beats_with_no_bed_at_all(tmp_path):
    ...
    master = np.full((1, 100 * STEP), 0.5, dtype=np.float32)
    mixed, stats = fs.mix_foley_under_master(
        master, RATE, [], fps=FPS, lane_ids={"ltx25_foley_16gb"})
...
def test_an_UNPOSITIONED_beat_is_skipped_loudly_not_fatal(tmp_path):
    ...
    rows = [
        ...
        {"foley_path": orphan, "start_s": None, "frame_count": 10,
         "start_s_space": "master_mix", "engine_id": "ltx25_foley_16gb"},
    ]
...
def test_every_beat_unpositioned_still_delivers_a_master(tmp_path):
    ...
    mixed, stats = fs.mix_foley_under_master(
        master, RATE,
        [{"foley_path": orphan, "start_s": None, "frame_count": 10,
          "engine_id": "ltx25_foley_16gb"}], fps=FPS)
...
def test_the_route_is_decided_by_ONE_function_for_both_audio_stages():
    ...
    assert fs.is_foley_route(policy(music="ltx25_foley_16gb"))
    ...
    assert fs.is_foley_route(policy(character="ltx25_mime_16gb"))
    assert fs.is_foley_route(policy(character="ltx25_mime_16gb (16:9)"))
    assert fs.route_lane_ids(
        policy(character="ltx25_mime_16gb", music="ltx25_foley_16gb")
    ) == {"ltx25_mime_16gb", "ltx25_foley_16gb"}
    ...
    assert fs.is_foley_route(policy(music="ltx25_foley_16gb (16:9)"))
    ...
    assert fs.is_foley_route(
        policy(announcer="ltx25_video", music="ltx25_foley_16gb",
    ...
    assert MUX._foley_route(policy(music="ltx25_foley_16gb")) is True
...
def test_cloud_foley_mixes_at_the_same_gains_as_local(tmp_path):
    ...
    assert (fs.FOLEY_LANE_GAINS["cloud_ltx25_foley_plus"]
            == fs.FOLEY_LANE_GAINS["ltx25_foley_16gb"]
            == (0.50, 0.50))
    master = np.full((2, 100 * STEP), 0.5, dtype=np.float32)
    bed = _stem(tmp_path, "bed.wav", 100, 0.25)
    ...
    assert sl["lanes"] == {"ltx25_foley_16gb": 1}
