...
Validates ``otr_v2.visual.postproc.vhs`` without requiring ffmpeg
...
def test_module_imports_cleanly():
    ...
    mod = importlib.import_module("otr_v2.visual.postproc.vhs")
    ...
    assert "torch" not in sys.modules or "otr_v2.visual.postproc.vhs" in sys.modules


def test_postproc_package_imports():
    pkg = importlib.import_module("otr_v2.visual.postproc")
    assert "vhs" in getattr(pkg, "__all__", [])


def test_default_params_shape():
    from otr_v2.visual.postproc.vhs import DEFAULT_VHS_PARAMS
...
def test_public_constants():
    from otr_v2.visual.postproc import vhs
...
def test_filter_chain_deterministic():
    from otr_v2.visual.postproc.vhs import build_vhs_filter_chain
...
def test_stub_mode_copies_input_byte_identical(tmp_path, monkeypatch):
    from otr_v2.visual.postproc.vhs import apply_vhs_filter
...
def test_force_stub_flag_overrides_env(tmp_path, monkeypatch):
    from otr_v2.visual.postproc.vhs import apply_vhs_filter, STUB_REASON_FORCED
...
def test_stub_mode_env_reason_recorded(tmp_path, monkeypatch):
    from otr_v2.visual.postproc.vhs import apply_vhs_filter, STUB_REASON_ENV
...
def test_stub_mode_activates_when_ffmpeg_missing(tmp_path, monkeypatch):
    ...
    from otr_v2.visual.postproc.vhs import (
        apply_vhs_filter,
...
def test_batch_apply_finds_motion_loop_render(tmp_path, monkeypatch):
    from otr_v2.visual.postproc.vhs import apply_vhs_to_job_dir, VHS_SUFFIX
...
def test_batch_apply_handles_mixed_shot_outputs(tmp_path, monkeypatch):
    ...
    from otr_v2.visual.postproc.vhs import apply_vhs_to_job_dir
...
def test_fps_defaults_to_renderer_fps():
    ...
    from otr_v2.visual.postproc.vhs import DEFAULT_VHS_PARAMS
    from otr_v2.visual import renderer
