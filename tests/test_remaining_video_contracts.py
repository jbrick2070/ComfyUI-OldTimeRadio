...
def test_wan_8gb_profile_declares_the_render_ceiling():
    profile = cp.load_profile("otr_8gb_wan")
    assert profile["video"]["max_render_frames"] == 17
    # The ceiling is NOT render.frame_budget: that is the soak/single harness
    # per-clip count (every 16GB tier declares 25 and must not be capped to it).
    assert profile["render"]["frame_budget"] == 17
...
def test_applied_8gb_variant_pins_17_and_other_tiers_stay_unpinned():
    ...
    assert _director_ceiling("otr_8gb_wan") == 17
