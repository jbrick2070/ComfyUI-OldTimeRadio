...
def test_the_plan_snaps_to_the_adapters_REAL_ladder(monkeypatch):
    ...
    it would have gone on passing while the plan promised an engine a 50-frame
    render that its adapter was quietly snapping to its own ladder behind its
    back. Two policies over one state, which is the defect this build exists
    to remove.

    What is true now, and worth pinning: 50 visible frames on ltx_8gb's 8n+1
    ladder (min 9) has no exact single render, so the planner renders the
    SMALLEST LEGAL length at or above it (57) and trims the excess (7). The
    ...
    so deleting the engine's contract moves both sides together and the test
    goes on passing against an open ladder. A mutation run caught exactly that,
    in this test, before it shipped. 50 visible frames must render 57 -- say 57.
    """
    beats = _beats(n=3)
    _groups, shots = sl.build_execution_plan(beats, _budget(beats), {}, _policy())
    contract = fc.frame_contract_for(vreg.get_engine("ltx_8gb"))
    for shot in shots:
        ...
        seg = plan.segments[0]
        # 9 + 8*6 = 57, the smallest length on ltx_8gb's ladder at or above 50.
        assert seg.render_frames == 57
...
def test_a_beat_past_the_cap_is_COVERED_by_more_clips_not_refused():
    ...
    and it monkeypatched a capped contract onto an engine to assert a refusal.
    Then that engine got a REAL cap, and the refusal stopped being
    ...
    No monkeypatch here. ltx_8gb's own declared ceiling, an ordinary beat
    ...
    # ltx_8gb locks frame 0 to its init image, so this one earns a real chain
...
def _ledger_with(plan_dict, target=50, engine="ltx_8gb"):
    return {
        "episode_id": "ep_cov",
        "video": {
...
def _engine_plan(frames=50):
    """A plan built from ltx_8gb's OWN declared contract.

    Chunk 7a, 2026-07-26: these boundary tests used to partition against
    ``fc.SINGLE_ONLY`` while stamping a real engine id on the ledger. That
    worked only while the engine declared nothing, so the two contracts
    happened to be the same object. Once it declares a real ladder, a
    SINGLE_ONLY plan is simply an ILLEGAL plan for that engine -- so a test
    ...
    return cp.partition_beat(
        frames, fc.frame_contract_for(vreg.get_engine("ltx_8gb")))


def test_render_boundary_accepts_a_sound_plan():
    assert rd.assert_coverage_plans(_ledger_with(_engine_plan().to_dict())) == 1
...
def test_render_boundary_rejects_a_plan_the_LIVE_contract_now_refuses(monkeypatch):
    ...
    """
    plan = _engine_plan()                    # LEGAL under ltx_8gb as declared
    assert rd.assert_coverage_plans(_ledger_with(plan.to_dict())) == 1
    engine = vreg.get_engine("ltx_8gb")
    # The adapter's floor moved above the stamped 57-frame render.
    narrow = fc.FrameContract(min_frames=65, max_frames=161, quantum=8)
...
def test_the_legacy_path_validates_the_plan_against_the_FINAL_engine(monkeypatch):
    ...
    forced = vreg.get_engine("ltx25_video")
    narrow = fc.FrameContract(min_frames=65, max_frames=161, quantum=8)
    ...
    assert ledger["video"]["shots"][0]["engine_id"] == "ltx25_video"
...
def test_a_row_carrying_real_jump_still_requests_validates():
    ...
    requests = cp.jump_still_requests(plan, "b001", role="character_video",
                                      engine_id="google_veo_video", char_id="c1")
