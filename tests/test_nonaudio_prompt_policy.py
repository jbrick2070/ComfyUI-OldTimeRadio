...
def test_a_public_menu_id_resolves_instead_of_being_refused():
    ...
    c = sl.derive_creative_directives(
        _beats(), led["meta"], led, llm_fn=_directive_llm(),
        video_policy=_policy("ltx25_foley_16gb (16:9)"))[0]["b1"]
...
def test_the_foley_suffix_names_the_sound_of_the_action():
    assert ltx25.finish_joint_av_positive("ltx25_foley_16gb", _CORE) == \
        _CORE_FINISHED
...
def test_foley_and_mime_compose_the_IDENTICAL_PICTURE_too():
    ...
    assert (ltx25.finish_joint_av_positive("ltx25_foley_16gb", foley)
            == ltx25.finish_joint_av_positive("ltx25_mime_16gb", mime))
...
def test_foley_and_mime_receive_the_IDENTICAL_string():
    ...
    assert ltx25.finish_joint_av_positive("ltx25_mime_16gb", _CORE) == \
...
def test_an_action_with_no_cue_still_NAMES_sounds():
    ...
    out = ltx25.finish_joint_av_positive(
        "ltx25_foley_16gb", "a figure stands looking out")
...
def test_at_most_three_sounds_are_named():
    ...
    out = ltx25.finish_joint_av_positive("ltx25_foley_16gb", crowded)
...
def test_the_composer_seats_sounds_inside_the_action_golden_shape():
    ...
    fin = ltx25.finish_joint_av_positive("ltx25_foley_16gb", core)
    ...
    assert ltx25.finish_joint_av_positive("ltx25_foley_16gb", fin) == fin
...
def test_finishedness_is_frame_present_plus_clause_last():
    ...
    assert ltx25.finish_joint_av_positive("ltx25_foley_16gb", good) == good
...
def test_a_weapon_beat_names_NO_weapon_sound(capsys):
    ...
    out = ltx25.finish_joint_av_positive("ltx25_foley_16gb", core)
...
def test_a_bare_no_voice_clause_is_NOT_treated_as_finished():
    ...
    out = ltx25.finish_joint_av_positive("ltx25_foley_16gb", bare)
    assert out != bare
...
def test_a_properly_finished_prompt_IS_left_alone():
    ...
    once = ltx25.finish_joint_av_positive(
        "ltx25_foley_16gb", "hands on the console")
    assert ltx25.finish_joint_av_positive("ltx25_foley_16gb", once) == once
...
def test_the_receipt_records_WHICH_sounds_were_named(_text_only_lane):
    ...
    req = rd.build_request_from_shot(_open_shot("ltx25_foley_16gb"),
                                     _open_ledger())
...
def test_the_identity_guard_is_quiet_on_todays_prompt():
    ...
    for engine, fn in (("ltx25_mime_16gb", ltx25.compose_ltx25_mime),
                       ("ltx25_foley_16gb", ltx25.compose_ltx25_foley_plus)):
...
def test_the_finisher_is_idempotent():
    once = ltx25.finish_joint_av_positive("ltx25_foley_16gb", _CORE)
    twice = ltx25.finish_joint_av_positive("ltx25_foley_16gb", once)
    ...
    assert twice == once
    assert ltx25.finish_joint_av_positive("ltx25_foley_16gb", twice) == once
...
def test_trailing_punctuation_is_trimmed_before_the_join():
    assert ltx25.finish_joint_av_positive(
        "ltx25_foley_16gb", _CORE + " ,.;: ") == _CORE_FINISHED
...
@pytest.mark.parametrize("engine,tail", [
    ("ltx25_foley_16gb", "No speech, no voices."),
    ("ltx25_mime_16gb", "No speech, no voices."),
])
...
def test_the_ordinary_idempotent_case_still_holds():
    ...
    for engine in ("ltx25_foley_16gb", "ltx25_mime_16gb"):
        once = ltx25.finish_joint_av_positive(engine, _CORE)
...
def test_a_foley_open_composes_a_real_prompt_and_finishes_it(_text_only_lane):
    """TWO defects in one request. Before this change `ltx25_foley_16gb`
...
def test_a_mime_open_IGNORES_the_briefs_mood_terms(_text_only_lane):
    ...
    with_moods = rd.build_request_from_shot(
        _open_shot("ltx25_mime_16gb"), _open_ledger(meta))["text_prompt"]
    without = rd.build_request_from_shot(
        _open_shot("ltx25_mime_16gb"), _open_ledger())["text_prompt"]
...
def test_a_mime_open_is_finished_with_named_sounds(_text_only_lane):
    req = rd.build_request_from_shot(_open_shot("ltx25_mime_16gb"), _open_ledger())
...
@pytest.mark.parametrize("spelling, internal", [
    ("ltx25_audio_in_16gb", "ltx25_audio_in_16gb"),
    ("ltx25_audio_in_16gb (16:9)", "ltx25_audio_in_16gb"),
...
def test_an_audio_in_lane_keeps_its_framing_under_any_spelling(spelling,
    ...
    spelling (`ltx25_audio_in_16gb (16:9)`) or a public id
...
def test_a_menu_spelling_on_the_shot_row_still_gets_finished(_text_only_lane):
    ...
    from nodes._otr_shared.public_engines import resolve_engine_id
    menu = "ltx25_foley_16gb (16:9)"
    assert resolve_engine_id(menu) == "ltx25_foley_16gb"
...
def test_a_menu_spelling_of_mime_is_also_finished(_text_only_lane):
    from nodes._otr_shared.public_engines import resolve_engine_id
    menu = "ltx25_mime_16gb (16:9)"
    assert resolve_engine_id(menu) == "ltx25_mime_16gb"
...
def test_the_empty_ltx25_negative_is_unchanged(_text_only_lane):
    ...
    for engine in ("ltx25_video", "ltx25_foley_16gb", "ltx25_mime_16gb"):
...
def test_a_character_beat_on_foley_gets_no_dialogue_end_to_end():
    ...
    creative = sl.derive_creative_directives(
        _beats(), led["meta"], led, llm_fn=_directive_llm(),
        video_policy=_policy("ltx25_foley_16gb"))[0]["b1"]
    assert _LINE not in creative["text_prompt"]
    shot = {"shot_id": "shot_b1", "source_line_ids": ["b1"],
            "role": "character_video", "engine_id": "ltx25_foley_16gb",
            "group_id": "grp_character_video", "target_frame_count": 50,
            "creative": creative}
