...
def test_slot_scheduler_transitions_match_dag(monkeypatch):
    """Defense-in-depth: the writer's _SlotScheduler must report
    transitions matching the DAG of slot calls. When slot1 != slot2
    the canonical writer-phase trace produces 2 transitions; when
    slot1 == slot2 it produces 0.
    """
    import nodes.OTR_LedgerScriptWriter as W
    import nodes._otr_model_loader as ML

    # Force-import so monkeypatch.setattr targets the real module.
    _OTRML = ML
    ...
    def fake_request_slot(slot, model_id, policy=None, load_config=None):
        # S1: mirrors the production signature (policy + load_config threaded
        # by the scheduler); the stub ignores them.
        return _StubCache({
            "model":       MagicMock(),
            "tokenizer":   MagicMock(),
            "device":      "cuda",
    ...
    monkeypatch.setattr(
        W, "_build_truncating_generate_fn",
    ...
    monkeypatch.setattr(
        _OTRML, "make_polish_generate_fn",
        lambda cache_entry: (
            lambda messages, *, temperature, max_new_tokens: "<fake-polish>"
        ),
    )

    # Same model -> 0 transitions.
    s_same = W._SlotScheduler(
        creative_id="mistralai/Mistral-Nemo-Instruct-2407",
        technical_id="mistralai/Mistral-Nemo-Instruct-2407",
        top_p=0.95, min_p=0.05, repetition_penalty=1.03,
    )
    s_same.for_slot("creative")([], temperature=0.8, max_new_tokens=10)
    s_same.for_slot("technical")([], temperature=0.4, max_new_tokens=10)
    s_same.for_slot("creative")([], temperature=0.8, max_new_tokens=10)
    assert s_same.transitions == 0

    # Split models -> writer DAG (creative, tech, creative, creative,
    # creative, polish, creative) = 2 transitions.
    s_split = W._SlotScheduler(
        creative_id="mistralai/Mistral-Nemo-Instruct-2407",
        technical_id="google/gemma-4-E2B-it",
        top_p=0.95, min_p=0.05, repetition_penalty=1.03,
    )
    creative = s_split.for_slot("creative")
    technical = s_split.for_slot("technical")
    polish = s_split.for_polish()
    creative([], temperature=0.8, max_new_tokens=10)
    technical([], temperature=0.4, max_new_tokens=10)   # +1
    creative([], temperature=0.8, max_new_tokens=10)    # +1
    creative([], temperature=0.8, max_new_tokens=10)
    creative([], temperature=0.8, max_new_tokens=10)
    polish([], temperature=0.4, max_new_tokens=10)
    creative([], temperature=0.8, max_new_tokens=10)
    assert s_split.transitions == 2
