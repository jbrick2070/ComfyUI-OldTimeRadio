...
def test_engine_without_a_measured_row_is_reported_unenforced(monkeypatch):
    """The important one: no row must not look like a passed check."""
    monkeypatch.setattr(mc, "free_vram_mb", lambda: 14000.0)
    shot = {"engine_id": "humo"}
    assert not mc.has_measured_cost_row("humo")
    record = rd._assert_beat_affordable(shot, _prebuilt([49, 49]))
    assert record["enforced"] is False
    assert record["checked"] == []
    assert "no measured cost row" in record["reason"]
    assert "humo" in record["reason"]


def test_unaffordable_planned_segment_raises(monkeypatch):
    """A measured row that cannot fit the planned length must REFUSE.
    ...
    with pytest.raises(mc.MotionBudgetError):
        rd._assert_beat_affordable(shot, _prebuilt([177]))


def test_free_vram_unreadable_is_reported_not_guessed(monkeypatch):
    """On a box with no NVML/torch the guard says so instead of inventing one."""
    monkeypatch.setattr(mc, "free_vram_mb", lambda: 0.0)
    shot = {"engine_id": "wan_ti2v"}
    record = rd._assert_beat_affordable(shot, _prebuilt([177]))
    assert record["enforced"] is False
    assert "unreadable" in record["reason"]


def test_affordable_beat_records_every_segment(monkeypatch):
    monkeypatch.setattr(mc, "free_vram_mb", lambda: 14000.0)
    shot = {"engine_id": "wan_ti2v"}
    record = rd._assert_beat_affordable(shot, _prebuilt([17, 17, 17]))
