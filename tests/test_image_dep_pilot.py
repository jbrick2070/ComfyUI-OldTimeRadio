...
def test_enumerate_covers_all_registered_engines():
    ...
    assert {"flux_gen1", "z_image_turbo", "lumina_image"} <= names
...
def test_isolation_classification():
    ...
    for n in ("z_image_turbo", "lumina_image"):
...
def test_commercial_clean_reported_honestly():
    by_name = {e["engine"]: e for e in pilot.enumerate_engines()}
    assert by_name["flux_gen1"]["commercial_clean"] is False
    assert by_name["z_image_turbo"]["commercial_clean"] is True
...
def test_run_pilot_counts():
    ...
    # flux2_klein UNREGISTERED 2026-09-17): z_image_turbo, lumina_image.
    assert report["in_stack_count"] >= 2
