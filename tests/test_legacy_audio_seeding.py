"""R0a (d): the four legacy audio nodes seed every RNG from the FROZEN
script_json (no parse) before their forward, so a render-twice is reproducible
(I-2). Source-proxy guard (the real bit-identity check is operator GPU, step f),
plus a functional check that seed_all_rngs is deterministic."""
import random
...
CASES = {
    "batch_bark_generator.py": "bark_legacy_v1",
