"""
test_hyworld_phase_a.py  --  Phase A regression tests
=====================================================
Covers the Phase A robustness work landed on v2.0-alpha:
  ...
  - HyworldPoll detects a dead sidecar PID without waiting for the
    full poll-timeout (`poll.py` integration).
  - HyworldBridge rejects malformed script_json with a PARSE_ERROR
...
# Make `otr_v2.hyworld.*` importable when pytest is run from the repo root.
_REPO_ROOT = Path(__file__).resolve().parent.parent
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

from otr_v2.hyworld._atomic import atomic_write_json, atomic_write_text
from otr_v2.hyworld.vram_coordinator import VRAMCoordinator, _pid_alive
...
# ---------------------------------------------------------------------------
# HyworldBridge schema validation
# ---------------------------------------------------------------------------

class TestBridgeValidation:
    def _validator(self):
        from otr_v2.hyworld.bridge import _validate_script_lines
...
# ---------------------------------------------------------------------------
# HyworldPoll WORKER_DEAD detection
# ---------------------------------------------------------------------------
...
class TestPollWorkerDead:
    def test_pid_helpers_match_coordinator(self):
        ...
        """
        from otr_v2.hyworld.poll import _pid_alive as poll_alive
        from otr_v2.hyworld.vram_coordinator import _pid_alive as coord_alive
    ...
    def test_read_sidecar_pid_missing(self, tmp_path: Path, monkeypatch):
        ...
        from otr_v2.hyworld import poll as poll_mod
    ...
    def test_worker_dead_terminal_status(self):
        ...
        from otr_v2.hyworld.poll import _TERMINAL_STATUSES
