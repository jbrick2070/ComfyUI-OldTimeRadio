...
LOG=/workspace/otr-config/foley_mystory_3act.log
...
if [ -f /tmp/_otr_my_story.py ]; then
  cp /tmp/_otr_my_story.py "$OTR/nodes/_otr_my_story.py"
  echo PATCHED_SPOKEN_LINE_KEYS
fi
...
tail -n 60 "$LOG" || true
echo "=== comfy ==="
grep -E 'got prompt|house DEFAULT_IDEA|All 25 nodes|my_story|Error|Traceback|NVIDIA' /workspace/otr-config/comfy_8188.log | tail -n 40 || tail -n 25 /workspace/otr-config/comfy_8188.log
echo "=== queue ==="
