...
Find your listed machine in **What works on what machine**. Apple Silicon and CPU-only systems still have experimental rows rather than a stranger-facing machine key; find those in the tier details.

## Engine proof by hardware

Only receipt-backed rows appear here. **PROVEN** means a full OTR episode published on the named hardware. **LAB-PROVEN** means an isolated recipe produced valid media there; it does not promote the full OTR workflow. A setup, model load, queued prompt, reserve clamp, or run still in progress is unqualified and does not appear.
...
| `still_motion` (the four still_* lanes (motion / pan / flat / word)) | **PROVEN** | Mac mini M4, 16 GB unified, Apple Silicon (MPS) | full published OTR episodes covering all four still_* lanes across two runs, every still supplied locally by sd15; three video dropdowns exist so no single run can cover four lanes | otr/obs/ 20260908_005546 (arch/stmo) covers still_motion; 20260908_042821 (cart/stwo) covers still_pan + still_flat + still_word; apple/MAC_PORTABILITY_GUIDE.md section 9 |
| `viz_camera` (procgen visualizer lanes) | **PROVEN** | Mac mini M4, 16 GB unified, Apple Silicon (MPS) | full published OTR episodes on the zero-download press-Run path: viz_mxc_cpu / viz_green / viz_camera video, Qwen3.5-4B writer on Metal, Kokoro voices, Stable Audio 3 music; no image weights invoked | otr/obs/ 20260907_201810 (rfrc/vcam) and 20260907_214731 (cart/vcam); apple/MAC_PORTABILITY_GUIDE.md sections 1-4 |
...
| `shipping` | the workflow is considered runtime-ready on a preloaded machine. It is neither hardware proof nor a complete clean-install claim. |
...
## 8 GB  --  8 experimental workflow(s), 7 shipping
...
## 10-15 GB  --  4 experimental workflow(s), 4 shipping
...
## 16 GB+  --  7 experimental workflow(s), 6 shipping
