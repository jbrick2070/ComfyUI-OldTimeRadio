...
**GENERATED by `scripts/otr_dropdown_matrix.py`. Do not hand-edit; regenerate.** Selectability is derived from each engine's `CAPABILITIES` row against the shipped workflow named in its column; sizes and gating are read out of the fetch manifests. Only the memory verdicts and receipts are curated, in `apple/dropdown_matrix.json`.

You never need all the weights in this workflow. One workflow ships; the dropdowns decide what it loads, and therefore what you have to fetch.

## The machines

| column | machine | reproduce with |
|---|---|---|
| 8 GB NVIDIA | RTX 4060 / 3070 / 2080 class | `workflows/otr_8gb_video.json` |
| 16 GB+ NVIDIA | RTX 5080 / 4080 / 3090 class | `workflows/otr_16gb_video.json` |
| Mac 16 GB | Apple Silicon, unified memory | `workflows/otr_mac16_video.json` |
| AMD ROCm | Windows or Linux -- and read "What the words mean" at the foot of this page before trusting any AMD cell | `workflows/otr_amd_still.json` |
| CPU only | no GPU at all | `workflows/otr_cloud_low.json` |
...
These need no weights and no VRAM -- only a credential -- and the engines themselves are registered and selectable. But no shipping workflow selects one, so they were never added to the provisioner's remote route, and `scripts/otr_provision.py` REFUSES a `--profile` (its own flag name) that names one rather than guessing. Using one today means selecting it in the canonical workflow by hand, not through a shipped one. Worth closing before shipping.
