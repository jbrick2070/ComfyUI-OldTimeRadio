...
**GENERATED by `scripts/otr_dropdown_matrix.py`. Do not hand-edit; regenerate.** Selectability is derived from each engine's `CAPABILITIES` row against the shipped profile named in its column; sizes and gating are read out of the fetch manifests. Only the memory verdicts and receipts are curated, in `apple/dropdown_matrix.json`.
