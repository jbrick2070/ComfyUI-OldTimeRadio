...
Some lanes resolve node classes that live in ComfyUI CORE, not in any node pack, so an older ComfyUI fails them no matter what you install. Since 2026-09-25 the queue-time gate checks every selected engine's node classes BEFORE any download, so the refusal comes in seconds and says "Update ComfyUI: these are built-in ComfyUI nodes that this version does not have" (until then it arrived at render time as WrapperNodeMissing, typically seventeen minutes in, and looked nothing like a version problem).
...
ComfyUI-LTXVideo is NOT needed: every class the LTX lanes ask for (`ltx_8gb` and the `ltx25_*` lanes) ships with ComfyUI itself, and `otr_8gb_video` published on a wiped 8 GB box without it (2026-09-26). A missing LTX class means an old ComfyUI -- update it.


`scripts/otr_provision.py` clones AnimateDiff-Evolved at its pin. If you install by hand, clone into `<comfy>/custom_nodes/` and restart ComfyUI -- node classes are registered at startup.
