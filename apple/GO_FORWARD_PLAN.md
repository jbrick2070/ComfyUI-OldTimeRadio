...
- **"What each graph uses" table in `apple/MACHINES.md` (DECIDED, operator
  2026-09-25: "the matrix should say what music model is being used").**
  Today no generated doc shows, per shipped graph, which engines it picks:
  `MACHINES.md` is machine -> graph plus per-engine feasibility, and
  `MACHINE_MATRIX.md` is per-engine proof. Add one table to `MACHINES.md`,
  right after "Which graph do I open?", one row per shipped graph (25:
  canonical first, then the matrix rows in matrix order), columns:
  `graph | video | image | voices | music | writer`. Source is the matrix
  (`config/workflow_matrix.json` deltas, read the way `build_variants.py`
  reads them): video = `role_overrides.{announcer,character,music}_visual`
  (one name when all three match -- the one-lane rule -- else
  `announcer / character / music` spelled out); image = the three
  `*_image` picks the same way; voices = `slot_overrides.char_voice_engine`
  and `announcer_voice_engine` (one name when equal); music =
  `slot_overrides.music_engine`; writer = `llm.creative_model`, with
  `technical_model` after a slash when it differs. The canonical's row
  reads its own saved widgets (it has no matrix row). Generated by
  `scripts/otr_dropdown_matrix.py`, which already writes `MACHINES.md`
  (`:1022`), so `build_variants.py --all` refreshes it; if `--check` does
  not already diff `MACHINES.md` against a fresh render, add that parity
  assertion the way `tests/test_dependency_doc_is_generated.py` does for
  DEPENDENCIES.md. Engine ids as they are at build time -- after the 0b2
  rename lands, they read `ltx25_foley_16gb` etc. with no edit here.
