...
- **Per-graph music column: ALREADY EXISTS, nothing to build.** Operator
  2026-09-25: "the matrix should say what music model is being used" --
  it does. `apple/MACHINE_MATRIX.md` (generated by
  `scripts/otr_machine_matrix.py`) carries one row per matrix graph, all 24,
  ...
  local graph -- the four Mac ones and the AMD one included -- selects
  `stable_audio_3`; the five cloud graphs select `sonilo`. The feasibility
