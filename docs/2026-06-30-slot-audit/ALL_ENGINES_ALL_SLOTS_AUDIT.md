...
## CONFIRMED DEFECT 3 -- the soak does NOT drive the CANONICAL JSON (the root of the drift)
The coverage soak (`scripts/_otr_cov_runner.py` -> `_otr_soak_capstone.run_leg` + `build_profile`)
SYNTHESIZES a capability profile and resolves engines via `engines_for_role` (the stale whitelist) --
a PARALLEL path that does NOT load `workflows/otr_scifi_16gb_full.json` and does NOT go through the
real OTR_VideoDirector role_compat validation. That parallel path is WHY the soak drifted from
production (Defect 1) and why it mis-reports (still_flat where production would render the engine).
This VIOLATES CLAUDE.md S0 ("EVERY API/headless/soak run MUST LOAD this real JSON -- never a generated
.gen.json, an ad-hoc graph, or a synthetic profile").

WORKSTREAM (operator 2026-06-30): REBUILD THE SOAK FROM THE GROUND UP on the canonical JSON. The soak
LOADS `otr_scifi_16gb_full.json`, sets the per-slot engine pick on node-87 OTR_VideoDirector via the
REAL director/role_compat/profile-applier path (the exact production resolution), and runs the real
graph. Then the soak's eligibility == production's by construction (no build_profile, no
engines_for_role). This SUBSUMES `build_profile`. Reuse the baked-combo design already converged at
`docs/2026-06-29-coverage-soak/COMBO_SOAK_CONVERGED_PLAN.md` (bake story+audio from the canonical JSON,
vary the node-87 picks per leg, start at the directors, render stills+video+upscale->obs). The two
plans MERGE into one: the canonical-JSON-driven combo soak that proves every engine in every slot.
