...
- 2C @78bee5d5: widget slot 25 (registry-live choices, fail-loud INPUT_TYPES, non-runnable listed = honest error); require_runnable_bank FIRST in run(); threading run()->_resolve_inputs->3 writer sites->compose_line/draft (incl. recursion)->resolve_creative_system_prompt(source_bank_id); meta.source_bank; whitelists; BUG-LOCAL-416 root-cause fix + regression test; guardrails/scaffold/openrouter-s2/api-companions pins updated same commit; +15 new tests (tests/test_source_bank_widget_2c.py); lane-enablement checklist -> STAGE2_SUBPLAN section 4b. Suite 6247/0, Bug Bible 16, no BOM, AST clean, JSON round-trip green.
Current step: STAGE 3 (visual_style schema, per BUILD_PLAN) -- Stage 2 done.
Next: Stage 3 planning/sub-plan (own kibitz gate); 6 commits UNPUSHED on v2.0-alpha (operator gates the push this arc).
Commits: 78bee5d5 (local, v2.0-alpha).
