...
The measured record behind `apple/MAC.md` -- harness run ids, per-leg logs,
MPS allocation numbers and tracebacks, from the 16 GB Mac mini M4.

This is a NOTEBOOK, not a guide. It lived inside `apple/MAC.md` until
2026-09-13, where it was 367 of that page's 461 lines -- so the page a Mac user
is told to read first was four-fifths internal log, including a handoff note
about a Codex session running out of credits. The guide keeps the findings;
this keeps the evidence. `docs/` is excluded from the published package, which
