...
**Status: PARKED AT r1 (operator, 2026-09-04). No code written, and this plan as
written is REJECTED.** Feasibility is confirmed YES -- the capability is there --
but both r1 lanes returned "no" and so did the driver on re-reading. Resume at r2
whenever; nothing is lost.

**Read `kibitz-runs/2026-09-04-h3-fl2v/r1/judgment.md` FIRST.** It carries the
grounded findings, the three errors in THIS document, and the one shape that
survived. The two blockers r2 must answer before anything else:

* **C1 -- the pinned frame is not always published.** H3 renders at 24 fps and
  publishes at 25, and the conversion FLOORS. Executed against the pack's own
  functions: on model rungs 141, 158 and 209 the last canvas frame maps to a
  model frame BEFORE the one `last_frame` pins. You would buy the conditioning
  and the viewer would never see the pinned frame.
* **C2 -- the fear cape owns that seam** on any beat of 4+ segments, and breaks
  it deliberately. Not a reason to remove the cape (the operator offered; it was
  declined): the two can be mutually exclusive per beat, and removing it would
  not touch C1 at all.

The sections below are kept as the record of what was proposed, INCLUDING the
parts the panel corrected -- do not read section 3 or the node-order diagram in
section 3 as fact without reading the judgment.
