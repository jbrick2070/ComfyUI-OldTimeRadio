...
|---|---|---|---|
| Visualization helper duplication | CONSOLIDATE | Centralize only exact pure `_ref_path`, `_canvas_dims`, and `_build_render_request` behavior. | Do not merge differing `_clip_from_raw` policies or remove explicit `fallback_engine=None`. |
| Six local image `_role_of` helpers | CONSOLIDATE | One cold-import-clean pure role helper. | Do not create a new inheritance hierarchy. |
...
| Test path/stubs | SCOPED, ORDER 11 | Remove only exact redundant root inserts after runner-root proof; keep protocol-specific fixtures. | No universal LLM stub and no global `sys.path` rewrite. |
...
| 7 | Land exact small consolidations: visualization pure helpers, local image role helper, then TTS sidecar adoption. | These reduce duplicated mechanisms without crossing failure-policy boundaries. | At least two live callers use each surviving abstraction in its landing commit; caller behavior is unchanged. |
| 8 | Consolidate ffprobe resolution/raw probing for node/runtime callers while retaining caller-specific wrappers. Leave standalone script callers unchanged until their order-11 owner audit proves a cold-import-safe adoption. | It is broad but mechanically bounded once dead/public surfaces are gone, without pre-empting script reachability work. | Every migrated caller retains its named error/fallback contract and focused failure tests; standalone tools still run independently. |
