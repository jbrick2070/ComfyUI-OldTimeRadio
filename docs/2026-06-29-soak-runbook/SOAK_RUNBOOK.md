...
**The harness now auto-derives acts from words (2026-06-29 fix -- this is the
root fix for the all-nighter):** leave `OTR_SOAK_ACT_COUNT` UNSET and each leg
uses `max(1, min(6, words//120))` with words floored at 60, so the budget node
can NEVER reject a leg. The legacy `420w -> 3 acts` default is preserved exactly.
An explicit `OTR_SOAK_ACT_COUNT` still wins -- use it only to force a specific
shape, and the Section-0 gate catches a manual mismatch on leg 1.
