...
1. **TASK 0 (above) first.** Then **16GB tier -- Path A smoke (no download).** Boot
   ...
   - This is **Phase 1 (bare /prompt graph, SaveWEBM)** -- the fast motion eyeball.
     Assert-in-trace does NOT apply here (no dispatcher); that moves to Phase 2 (the
     engine leg, task 4 / the real `render_clip` path), where you FAIL LOUD if
     `wan_i2v` is not the final_engine (the CS-1 fallback-PASS trap).
...
## Temperature / seeds / determinism + I/O contract (planner review 2026-06-12)
- **Wan is TEMPERATURE-FREE.** Diffusion has no temperature -- only seed / steps /
  cfg / sampler / scheduler / denoise. Temperature lives ONLY in the text writer,
  which a motion smoke does NOT exercise. So the smoke must use a **FIXED
  motion-prompt STRING + a FIXED seed, NO LLM in the loop** -- the only variable
  across runs is the seed. (Do not let a writer/LLM generate the smoke prompt.)
- **Seed path (verified in `eng_wan_i2v`):** `seed_bundle.request_seed` ->
  `plan["seed"]` -> `KSampler` seed. Log the seed in the clip filename so each
  eyeball clip is reproducible.
- **Determinism (V-7) is SEED-KEYED INPUTS, not bit-identical pixels.** Render twice
  at the same seed and expect a STABLE result, but GPU attention (Wan / Sage / flash)
  is not guaranteed bit-identical. So compare a frame hash + log any drift -- do NOT
  hard-fail the eyeball gate on a sub-pixel diff. The contract OTR actually holds is
  "same seed in -> same plan/inputs out," which the engine satisfies.
- **Input contract.** The engine reads the still from `asset_refs.init_image`; canvas
  w/h default 832x480; the motion text is `text_prompt`. **Supply `init_w`/`init_h`**
  -- if either is 0, `_aspect_plan` returns `None` and NO pad/crop runs (Wan
  internal-scales to canvas; a non-landscape still would distort). Give a real
  landscape console still + its dims so the aspect policy applies (uniform scale,
  never stretch, N9).
- **TWO smoke modes -- keep them DISTINCT (this corrects the task list):**
  - **Phase 1 = FAST eyeball, BARE `/prompt` graph.** Clone `otr_ltx_motion_smoke.py`
    -> `otr_wan_smoke.py` (SaveWEBM). Tests the MODEL + node wiring + MOTION only.
    NO OTR engine, NO "trace," NO audio. This is the quick "does Wan 2.2 actually
    move?" gate. **Assert-in-trace does NOT apply here** (there is no dispatcher).
  - **Phase 2 = ENGINE leg.** Drive `eng_wan_i2v.render_clip` via the real path
    (`otr_run_leg.ps1` / `coverage_sweep --only ...`). THIS is where **assert
    `wan_i2v` is the final_engine in the trace** (no fallback-PASS, CS-1),
    **render-phase NVML <=14.5GB** (CS-2), and **byte-identical audio mux** apply.
- **Output clip contract (engine, Phase 2):** silent **mp4** (h264 / yuv420p /
  bt709), `fps 25`, `frame_count`, **`has_audio` MUST be False** (V-1: only
  `OTR_MasterAudioMux` emits audio -> the frozen spine stays byte-identical). The
  SaveWEBM output is Phase-1-only; episodes consume the engine's mp4.
