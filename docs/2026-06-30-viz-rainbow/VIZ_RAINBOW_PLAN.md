...
## CREATIVE DIRECTION -- OTR MYSTIQUE, NOT PARTY RAINBOWS (operator, hard)
The rainbow must be filtered through the OLD-TIME-RADIO aesthetic -- it CANNOT be crazy/happy
rainbows. It is an OTR-themed, rainbow-COLORED viz with vintage-radio mystique. Anchor motifs (pick
during the creative roundtable): a glowing vacuum-tube / radio-dial / magic-eye tuning indicator that
pulses with the audio; the rainbow as a RADIO SIGNAL SPECTRUM sweep (ties to the "Signal Lost" theme);
oscilloscope / CRT phosphor traces in spectrum colors over a DARK noir field; muted/desaturated rainbow
(not neon), graded with the existing OTR cinematic lens -- 35mm film grain, heavy vignette, dim
amber/cyan rim, subtle scanlines (the same suffixes the flux bookend uses). Mysterious + retrofuturistic,
period-radio, NOT a music-festival visualizer. This aesthetic applies to BOTH tiers.
