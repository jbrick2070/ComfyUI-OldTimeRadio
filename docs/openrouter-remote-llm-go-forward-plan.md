...
| S6 — Smoke proofs (disabled + enabled) | DONE-autonomous (2026-05-31); ENABLED smoke + full README rewrite = operator | — | disabled smoke green; in-app error + dropdown tooltips wired; setup doc verified current; README OpenRouter pointer promoted. REMAINING (operator): the ENABLED live smoke (real OpenRouter call, spends credits, needs env+GPU) + the broad newbie-voice README rewrite (release/versioning + voice = Jeffrey's call) |
...
### 2026-05-31 · Session 1 (W1–W4 executed)
- **W1** (`a117efd`): S1 `_otr_openrouter_backend.py` (FC1 backend; cost guard before the call, bounded retries, key-from-env; 22 mocked-HTTP tests incl. cost-abort via mocked counter) + S2 catalog virtual rows (`provider` field + `openrouter_http` literal; enabled-gated; 12 toggle tests). Full OTR 3238 pass.
- **W2** (`152f76e`): S3 wired the remote branch into `request_slot` (skips local steps; C2 no-evict, object-identity test) + all three generate-fn factories branch on `provider`; 8 wiring tests. Full OTR 3246 pass.
- **W3** (`2ce4f77`): S4 fail-closed technical JSON (remote `make_constrained_generate_fn` → schema→response_format → existing `structured_call` ladder; 6 tests) + S5 `openrouter_meta_for` provenance stamp (empty for local; no key leak; 8 tests). Full OTR 3260 pass.
- **W4** (this commit): S6 autonomous half — disabled-smoke proof (6 tests), in-app error pointing to `docs/openrouter-setup.md` when a handle is picked while disabled, dropdown tooltips naming the env vars, README OpenRouter pointer promoted to a real entry, setup doc verified current. Full OTR **3266 pass / 12 skip / 0 fail**; Bug Bible 23 pass / 1 skip / 2 xfail. 51 new OpenRouter tests total across W1–W4.
- **C1–C9 all honoured.** Remote default-off; offline baseline untouched (every full-suite run had remote disabled); no-evict; fail-closed JSON; cost guard; PD6 slot tags; no secrets.
- **Two items left for the operator (Jeffrey), as planned:** (1) the **ENABLED live smoke** — a real OpenRouter call that spends credits on a GPU episode, needs `OTR_ENABLE_OPENROUTER=1` + `OPENROUTER_MODEL_A/B` set + ComfyUI restarted; run the disabled-then-enabled A/B per S6. (2) the **broad newbie-voice README rewrite** — left for Jeffrey because it turns on release/versioning intent (the v1.7 download badge may be correct for `main`) and on his public-on-ramp voice; the OpenRouter section is done.

### 2026-05-31 · Session 2 (provider routing — speed/cost control)
- **Context:** the build above is complete and live-proven (BUG-LOCAL-294 confirmed the enabled path works on `anthropic/claude-opus-4.8`). The story-quality baseline (`docs/2026-05-31-otr-story-quality-baseline.md`) closed transport/wiring and named the remaining quality lift as a prompt problem (F1 speaker-attribution collapse, F2 headline-dump outro) — NOT touched here.
- **Gap closed (operator-requested):** the backend sent the bound slug verbatim with no provider routing, so the writer's many internal LLM calls could not be biased toward the fastest (or cheapest) upstream. Added **`:nitro` (fastest / `sort=throughput`)** and **`:floor` (cheapest / `sort=price`)** support, plus `latency`, via three inputs (most-specific-first): a `:nitro`/`:floor` suffix on the slug, the per-slot `OPENROUTER_A_ROUTE`/`OPENROUTER_B_ROUTE`, or the global `OPENROUTER_SORT`. The suffix is stripped and normalized to one `provider.sort` value that merges with the existing `require_parameters` JSON guard (one provider object, no clobber). Default = no `provider.sort` (OpenRouter load-balancing) — byte-identical to before.
- **Scope:** `nodes/_otr_openrouter_backend.py` only (`_normalize_sort`, `_split_slug_suffix`, `resolve_route`; `load()` stores `provider_sort`; `generate()` merges it; `openrouter_meta_for` stamps `llm_<slot>_route`). New `tests/test_openrouter_routing.py` (33 tests). `docs/openrouter-setup.md` Step 4b + troubleshooting. C1–C9 honoured (default-off, no-evict, cost guard still gates, no node surface/widget/workflow change, no new LLM call site, no secrets). Regression: full OTR **3303 passed / 12 skipped / 0 failed**; Bug Bible 23 pass / 1 skip / 2 xfail; core + audio byte-identical + LLM-slot sweep green; AST/BOM clean.
- **Operator note:** for a quality win, point the **creative** slot at a strong model (e.g. `anthropic/claude-3.5-sonnet` or Opus) — NOT the `mistralai/mistral-nemo` used for the parity baseline — and add `:nitro` (or `OPENROUTER_SORT=throughput`) for speed. The F1/F2 prompt-quality work and the "script-first, parse-second" design (`docs/2026-05-31-openrouter-remote-creative-json__*`) remain the next quality levers.
