...
| A leg was killed but the GPU stays busy and the next leg sits at `status=pending` forever | Killing the leg CLIENT does not cancel the PROMPT it already queued; ComfyUI keeps executing it and the new leg queues behind it | `GET /queue` and compare `queue_running` against the prompt id your leg printed. `POST /interrupt` cancels the running one -- but DRAIN FIRST: with a leg already queued behind it, the interrupt can land on the newly promoted prompt instead and kill the leg you were trying to rescue (observed 2026-09-03: the next leg died at node 62 `OTR_LedgerFreezeCascade was INTERRUPTED`). Clear pending, interrupt, then re-queue. A client kill is not a render kill -- the server owns the work |
