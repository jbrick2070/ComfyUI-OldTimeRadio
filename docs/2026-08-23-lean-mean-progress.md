...
| 8 ffprobe consolidation | **APPROVED by the operator, 2026-08-23** -- after a Fable explainer distinguished it from the order-7 ruling ("shares the tape measure, not the judgment calls": one shared binary-resolver/raw-probe for the ~30 in-pipeline callers, every caller KEEPING its own failure policy -- GraphExecutionError / -1 / best-effort / CreditsDataError / CORRUPT_OUTPUT). His words: *"do it with everything else that is safe in a new window and lets regression test."* Queued for the NEXT window, not this one. |
| 9 Writer / StoryOrchestrator splits | QUEUED FOR THE NEXT WINDOW under the operator's "everything else that is safe" -- MEASURED for whoever takes it: `_resolve_inputs` is 398 lines (module level), `WriterTailContext` 38, `_run_writer_tail` 911 lines AND A METHOD inside the class, in the 7,417-line file that writes every episode. That is a fresh-window campaign with hash proofs per seam, not a session-tail job. |
