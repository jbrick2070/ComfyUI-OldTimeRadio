...
**The undershoot is mechanical:** 14 beats x `words_per_beat_range` (20-35) caps a *character* total at ~280-490 words no matter what `target_words` says. With `target_words=864` the budget can never be met by this shape -- which is exactly what the soak shows (none reached 864).
