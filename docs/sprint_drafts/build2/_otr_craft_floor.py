"""docs/sprint_drafts/build2/_otr_craft_floor.py -- Build 2 draft (2026-05-28).

DETERMINISTIC Tier-A integrity gate for the Story Room slot-formatted
output. STAGED DRAFT: lives under docs/sprint_drafts/build2/ so it does
NOT auto-import into ComfyUI. The integration session moves it to
nodes/_otr_craft_floor.py and wires it per WIRING_SPEC.md.
...
STAGED-DRAFT NOTE: this module intentionally does NOT carry
`from __future__ import annotations`. The repo convention is to include
it, and the integration session SHOULD add it back when promoting this
file to nodes/_otr_craft_floor.py (where the normal package import path
avoids the issue). It is omitted here only because the staged test loads
this file by path under pytest's import hook, which races the Python
3.10 dataclasses string-annotation resolver during @dataclass class
creation. Real (non-string) annotations sidestep that resolver. No
runtime behavior depends on the future import.
"""
