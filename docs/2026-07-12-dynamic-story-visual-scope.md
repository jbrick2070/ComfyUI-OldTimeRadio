...
kibitz-runs/2026-07-12-dynamic-story-visual/{r1,r2,r3,r4}/. The arc converged at
r4: no panelist proposed a different architecture -- every r4 finding was a
precision fix inside the agreed design, and the residue is section 11's
...
- **`field_evidence` keys are a CLOSED ENUM** of literal key names: the 11
  ...
  pointers or schema paths.
- **Row-shaped items carry their evidence INLINE only** (`motifs[]`,
  `clue_visual`) -- never duplicated into `field_evidence`. Inline evidence
  carries the same `{id, quote?}` shape and its own `kind`.
- An `evidence` item is `{id, quote?}`. A `factual` entry MUST carry a `quote`
  (<= 120 chars) that is a VERBATIM SUBSTRING of the cited item's TARGET TEXT --
  machine-checked in the `post_validator`. ID existence alone does not prove
  support. A `rationale` entry needs only the `id`.
- **Target-text mapping (r4 antigravity M2 -- rev 4 never defined it for non-line
  ids):** `line:<line_id>` -> `lines[line_id].text`; `cast:<char_id>` ->
  `cast[char_id].character_description`; `brief:<key>` -> the brief value (the
  joined element strings when list-valued); `meta:<key>` -> the meta value;
  `title` -> `meta.episode_title`.
- **Every EXECUTABLE field must have at least one `factual` anchor** -- all 11
  `look.*` keys and every `shots[]` row. Without that rule an executable field
  could evade grounding by declaring itself `rationale` (r4 codex M5).
  `style_language`, `era_cues` and `motifs[]` may be `rationale`.
- **`clue_visual` may be empty** (r4 antigravity M3): an episode with no clue
  mechanism sets `evidence: []` and a `treatment` that states the absence. It is
  not an executable field and carries no mandatory factual anchor.
...
```json
{
  ...
  "model_receipt": {
    ...
    "constrained": true,
    "runtime_policy": { "...": "an EXPLICITLY SERIALIZED snapshot of
                        policy_from_meta -- it returns a frozen dataclass with a
                        tuple-valued allowlist, not JSON (r4 codex M7)" },
    ...
    "token_budget": { "advertised_cap": 0, "configured_safe_cap": 8192,
                      "count_mode": "exact | conservative",
                      "pa_base_call": 0, "pa_repair_call": 0,
                      "pb_batches": [ { "batch": 0, "base_call": 0,
                                        "repair_call": 0 } ] },
  ...
  "story_binding": {
    "episode_id": "<the ledger's episode_id -- VERIFY-AT-BUILD: read it from its
                   real home, which is NOT necessarily meta (r4 codex M3)>",
  ...
  "style_pack": {
    "style_id": "dynamic_story", "schema_version": "v2",
    "...": "the COMPLETE v2 field set and NOTHING ELSE -- ASSEMBLED from the
            vetted safety base + the draft's `look` -- validated by the SAME
            _validate_row rules (nodes/_otr_visual_styles.py:184+)"
  },
...
outside the semantic hash. `shots` is sorted into SOURCE-LINE ORDER before hashing
-- the P-B batches complete in whatever order they complete, and without an
explicit re-sort the semantic hash would depend on batch ordering (r4 antigravity
S2).

**`is_dynamic` is NOT a pack key (r4 codex M6 -- the worst defect rev 4
introduced).** `VisualStyle` is a `@dataclass(frozen=True)` with an EXACT field
list (nodes/_otr_visual_styles.py:138-166) and v2 validation rejects unknown keys,
so a persisted `style_pack.is_dynamic` would fail the artifact's OWN validation
step. `is_dynamic` is a RUNTIME-ONLY attribute: a new dataclass field defaulting
to `False`, set to `True` by the resolver ONLY after the assembled payload passes
...
`content_mutations` and `source_recheck` are both CUT as stored fields: a STORED
artifact can only ever say "clean" (a failed recheck REFUSES the stamp), so the
field is tautological. The CHECK stays (5.2 step 6); its outcome is a structured
log line and, on failure, a named exception.
...
| Collection | EXACT fields (closed set) | normalization |
|---|---|---|
| `episode` | episode_id | str |
| `meta` | episode_title, style, source_bank, visual_style | str: `""` |
| `brief` | the brief's look-bearing fields, as they ACTUALLY exist on the ledger | per-type; see below |
| `cast[]` | char_id, name, gender, traits, character_description | str: `""`; list: `[]` (traits' str-vs-list is pinned, not inferred) |
...
would corrupt the boolean `skip` and the list-valued brief fields).

**VERIFY-AT-BUILD (r4 codex M3):** the brief's real storage shape and
`episode_id`'s real home must be read off the LIVE ledger before coding -- the
panel reports `story_brief` as a STRING with flat palette/key-object fields plus a
nested `story_brief_terms`, and `episode_id` as TOP-LEVEL rather than in `meta`. I
have not personally verified those paths, so this design states the REQUIREMENT --
ONE canonical representation, exact source paths, closed types, no duplicated
lighting field -- and Codex pins the paths against the real ledger. A DTO built on
a guessed path is a silent-drift bug of exactly the class this document exists to
prevent.
...
- `DirectionSourceView_PA`: `episode` + `meta` + `brief` + full `cast[]` + the
  COMPLETE line spine (`line_id`, `char_id`, `speaker_role`, `boundary`, `skip`,
  and the FULL `text`).
...
**NO TRUNCATION, ANYWHERE (r4 codex M1).** Rev 4 allowed a deterministic head
truncation of P-A's line text when the budget demanded it. That contradicts both
this document's own premise ("an LLM reads the final accepted/frozen story") and
its own must-fit law ("RAISES on overflow, NEVER truncates"). P-A receives the
complete spine or the PREFLIGHT ABORTS (4.4). If a 720-word episode cannot fit the
configured safe cap, that is a real constraint the operator must see -- it argues
for a larger-context model or a chunked P-A, not for a silent head-slice.
...
**The `shots[]` universe is a NAMED, LINE-BACKED PROJECTION of the still-target
set** -- not "every line" (r3 antigravity M4), and not the raw target set either
(r4 codex M8). `derive_scene_still_targets(lines, fps)`
(nodes/otr_meta_brief_image_prompt.py:1026-1118) DELIBERATELY EMITS the synthetic
opening-music target `b000_music_open` -- even optimistically on a pre-audio ledger
(:1072-1086) -- so a naive "shots set == target set" gate could never pass.

Definition: `SHOT_TARGETS = derive_scene_still_targets(lines)` MINUS every target
whose `beat_id` is not a real `line_id` (i.e. the synthetic `scene_open`), MINUS
any line whose role resolves to a mesh-fodder / plate-only kind that consumes no
note (7.4). Both the direction node and MetaBrief compute it with the SAME pure
helper, so the two sides cannot drift. Exactly one row per target, unique
`line_id`, re-sorted into source-line order before hashing.

The opening-music scene keeps its pack-level treatment (it has no line, so it has
no evidence and no note). Consumption of every remaining row is MANDATORY and
per-kind (7.4) -- an authored note that nothing consumes is dead scope.
...
**ONE exact parent/key table, shared by all five representations** (r4 codex M4 --
rev 4's prose omitted P-A's top-level `era_cues`, and its test battery put an
`era_tail_2` in P-B, which has no era field at all). The table names, for EVERY
object and collection in BOTH passes, its exact parent and its exact key:

| Pass | Top-level keys it owns | Collections it owns, and their ONLY legal parent |
|---|---|---|
| P-A | `style_language`, `era_cues`, `motifs`, `clue_visual`, `look`, `field_evidence` | `motifs[]` -> ROOT; `field_evidence{}` -> ROOT; `evidence[]` -> inside a `motifs` row / `clue_visual` / a `field_evidence` entry -- NEVER inside `look` |
| P-B | `shots` | `shots[]` -> ROOT; `evidence[]` -> inside a `shots` row |

Schema, base prompt, worked fixture, validator and repair directive all quote THAT
table. A key legal in one pass is a schema error in the other.
...
(:207-238). **Google has NO branch.**

**CLOSED (r4): the Google lane runs UNCONSTRAINED**, with the typed-repair ladder
as its sole schema defense, and `make_constrained_generate_fn` returns the plain
Google closure for `provider == "google_api"` rather than failing on a missing
tokenizer. Adding a Google schema branch is separate work and does not belong in
this feature's blast radius. `model_receipt.constrained` records which mode each
leg actually ran in.
...
2. EXACT preflight of P-A:
   `max(base_call_tokens, repair_call_tokens) <= configured_safe_cap`.
...
**The two calls, measured (r4 codex M2 -- rev 4's formula double-counted):**

```
base_call   = count_tokens(prompt) + effective_max_new_tokens(reserve)
repair_call = count_tokens(prompt) + ECHO + count_tokens(directive)
              + effective_max_new_tokens(reserve)
```

where `ECHO` is BOUNDED and small: the real repair composer echoes only
`failed_output[:_FAILED_OUTPUT_ECHO_CHARS]` (400 chars,
nodes/_otr_repair_prompts.py:128-152) -- it does NOT re-send a whole failed
generation. Measure the directive; hard-code nothing. Record the ADVERTISED /
native cap and the CONFIGURED SAFE cap separately, plus whether the count is exact
or conservative; if either is unknown, FAIL (Lesson 5: "a model's native context
and the project's configured safe context are separate facts; measure both").

`batch_size` and the output reservations are DERIVED FROM THE MEASURED MAXIMA of
that arithmetic, with a deterministic source-order partition. (The figures 15 /
~900 tokens in earlier revisions were a PLACEHOLDER, not a specification: 15 legal
240-char notes plus evidence and JSON overhead already exceed 900 tokens.)

**Every bound is explicit** (enforced in `post_validator`, never as `max_length` --
2.1): `subject_note <= 240` chars (mirrors the motion budget,
nodes/_otr_visual_styles.py:121-123); `quote <= 120` chars; `mood` = ONE word from
a closed register vocabulary; `era_cues` / `motifs[]` / `evidence[]` all carry
explicit maximum cardinalities; every authored `look.*` string carries an explicit
maximum length; assembled artifact `<= 64 KB` canonical (a STORAGE bound, not a
context budget).
...
7. **`finally`: LLM teardown** (5.5). If teardown RAISES: log a structured failure
   receipt and RE-RAISE -- do NOT proceed to GPU image work with a resident LLM.
   (It cannot be STAMPED: at this point `meta.visual_direction` does not exist
   yet, and MetaBrief/ShotLock have no ledger output at all -- r4 codex M7. A
   teardown failure is a NAMED EXCEPTION plus a log line, not an artifact field.)
...
A bare `VisualStyle` would force consumers to re-read unvalidated `meta` for the
notes and the digest. (`artifact_meta` is CUT -- no consumer uses it.)
...
**LOOK authority, not CONTENT (r4 codex M9).** `get_story_brief_ltx` supplies the
SEMANTIC SCENE CONTENT of a shot -- what is in frame -- not merely a palette.
Suppressing its LOOK/palette authority must NOT delete the narrative core: the
dynamic lane passes the resolved style INTO the finisher and keeps the brief's
scene content. Deleting the core would leave a beautifully-graded shot of nothing.

D10 CLOSED: the dynamic lane is signalled by the `is_dynamic` BOOLEAN on the
resolved `VisualStyle` (a runtime-only dataclass field, 2.3), not by a
`style_id == "dynamic_story"` string compare scattered through the helper family.

### 5.4a Talking-lane safety, stated precisely (r4 codex M9)

Earlier revisions claimed the talking lane was "pinned to the safety base". That is
true of the fields that actually protect lip-sync, and FALSE as a blanket claim:
`finish_visual_prompt(meta, text_prompt)` runs on EVERY talking-head path
(nodes/otr_shot_lock.py:634), so the pack's `era_tail` / `positive_tail` /
`image_grade_tail` DO reach talking prompts -- on the dynamic lane those tails are
LLM-authored. The honest law:

- **Pinned to the safety base, never LLM-authored:** `portrait_look_talking` (the
  S4b bright/frontal/warm law), `announcer_subject_face`,
  `announcer_subject_ltx_mouth`, `announcer_subject_object`, `motion_registers`.
  These are the subject / mouth / framing / motion vocabulary -- they are what keep
  lip-sync and engine behavior safe, and no per-episode LLM output can alter them.
- **Authored tails DO reach talking prompts** -- exactly as a NAMED pack's tails do
  today. That is not a regression; it is the existing contract.
- **What protects the talking lane from a hostile tail** is therefore the
  PYTHON-OWNED anti-geometry lint over authored fields (2.4): an authored tail
  containing framing / headroom / crop / close-up / mouth-visibility vocabulary is
  a validation FAILURE, not a rendered prompt.
- A HOSTILE-TAIL test is mandatory (9.1.22).
...
| talking-head video prompt | ShotLock `finish_visual_prompt` | no in v1 (stated explicitly) | no in v1 |
| synthetic opening-music scene | `derive_opening_music_beat` | no -- it has no line, so no row exists (2.6) | no |

**The last two "no" rows are the reason the shots universe is a PROJECTION, not the
raw target set** (2.6): a line whose role resolves to a mesh-fodder/plate-only kind
consumes NEITHER field, so authoring a note for it would be dead output. Such lines
are excluded from `SHOT_TARGETS` at derivation time, by the same pure helper both
sides call. Consequently every row that exists IS consumed, and nothing authored is
dead.

Notes are ADDITIVE clauses appended at a SPECIFIED insertion point with a SPECIFIED
separator (the clause template, punctuation and position are pinned in the code so
prompts -- and therefore `prompt_hash` -- are deterministic). They can never replace
pack-level fields, are bounded (4.4), and are linted against the safety-base
`forbidden_terms` + the Python geometry vocabulary at validation time.

Engine-safety continuity is structural: the mouth / subject / motion / typography
fields all come from the vetted base (2.4), so no per-episode LLM output can degrade
lip-sync or engine motion; `render_driver`'s `motion_registers` reads never vary with
LLM output. The authored TAILS do reach talking prompts (5.4a) -- the anti-geometry
lint is what keeps them safe there.
...
2. **Registration in the LITERAL `_NODE_MODULES` dict in `__init__.py`** (:119-325;
   ...
   **But AST presence is NOT registration (r4 codex M11):** the loader loop CATCHES an
   import failure and silently omits BOTH mappings (__init__.py:351-367), so a node that
   fails to import is simply absent at runtime while every AST-based test stays green.
   A COLD PACKAGE-IMPORT test is therefore mandatory (9.1.23).
...
{ "id": 96, "type": "OTR_DynamicStoryDirection",
  ...
  "outputs": [
    { "name": "patched_ledger_json", "type": "STRING", "links": [252, 255],
      "slot_index": 0 },
    { "name": "direction_report", "type": "STRING", "links": [], "slot_index": 1 },
    { "name": "done", "type": "STRING", "links": [], "slot_index": 2 } ],
  "properties": { "Node name for S&R": "OTR_DynamicStoryDirection" },
  "widgets_values": [] }
```

   `order = 4` (it must execute after 62, whose order is 3, and before 89/90); every
   existing node with `order >= 4` is incremented by one.
   ...
   **Re-baseline procedure, in order:** `OTR_WorkflowValidator`
   ...
   (`_expected_slot_count`, :140-155) -> link/output-fan-out reconciliation -> compute
   the canonical receipt hash -> REGENERATE and verify every committed stamped variant
   (`scripts/build_variants.py`). VERIFY-AT-BUILD: the canonical node 63's stamp state
   must be read before assuming "no widget change" and "master hash stamp" are
   compatible (r4 codex M10).
...
22. **Hostile tail (5.4a):** an authored `positive_tail` / `era_tail` carrying framing,
    headroom, crop, close-up or mouth-visibility vocabulary FAILS validation and never
    reaches a talking-head prompt. This is the test that makes the talking-safety claim
    true, because the tails themselves DO reach those prompts
    (nodes/otr_shot_lock.py:634).
23. **Cold package-import registration (8.2):** import the package fresh and assert
    `OTR_DynamicStoryDirection` is present in BOTH `NODE_CLASS_MAPPINGS` and
    `NODE_DISPLAY_NAME_MAPPINGS`, with the concrete class object and the exact display
    label -- then pin `INPUT_TYPES`, `FUNCTION`, `CATEGORY`, `RETURN_TYPES`,
    `RETURN_NAMES` and the return tuple arity. An AST-based test cannot catch an
    ImportError; the loader swallows it.
24. **Is-dynamic resolution (2.3):** a named pack resolves with `is_dynamic == False`;
    a dynamic payload resolves `True` ONLY after strict v2 validation; a payload
    carrying a stray `is_dynamic` key inside `style_pack` FAILS `_validate_row`;
    `_STYLES` is unchanged after a dynamic resolve.
25. **Shots universe (2.6):** the synthetic `b000_music_open` target is EXCLUDED from
    `SHOT_TARGETS`; a mesh/plate-only line is excluded; both sides (direction node and
    MetaBrief) compute the identical set from the same helper; batched P-B output is
    re-sorted into source-line order before hashing (a shuffled batch-completion order
    must not change `semantic_sha256`).
...
**Two corrections to what a live leg can prove (r4 codex M12):**

- **Byte-identity is NOT a live property.** The writer always re-executes
  (`IS_CHANGED -> time.time()`) and local generation samples without a seed
  (nodes/_otr_model_loader.py:1122-1129), so a live control leg can never byte-match a
  committed pre-feature prompt. Byte-identity lives ONLY in the captured-ledger /
  injected tests (9.1 item 8). The live control leg proves something different and
  still worth proving: named-pack PASS-THROUGH (the direction node is inert), NO
  dynamic receipts anywhere on the ledger, and a successful publication.
- **The legs must CONSUME STILLS.** A visual configuration whose visualizers ignore
  stills can publish a perfectly green episode without ever rendering a story-directed
  pixel -- i.e. the smoke would prove nothing about this feature. Control and dynamic
  legs run the SAME explicit still-consuming configuration.

1. **Control leg:** 30-word episode, `visual_style="sci_fi_radio"`, still-consuming
   config -- pass-through proven, no dynamic receipts, published.
2. **Dynamic legs:** the 9.2 ladder, same still-consuming config. Each asserts: the
   artifact is on the DISK ledger with full receipts; the attempt journal shows the
   rungs; a still was actually GENERATED and its prompt carries the artifact-authored
   clauses per the 7.4 matrix; **no local LLM is resident when the Dispatcher's GPU work
   begins** (which requires MetaBrief's and ShotLock's teardown too, 5.5); assets exist
   at `otr\episodes\<ep>\` (Test-Path); `obs_publish OK` **AND** the final file exists
   under `otr\obs\` (Lesson 7 -- `obs_publish OK` alone is not proof).
3. **Stale-source and replay:** NOT live legs (1.6, 7.5) -- proven deterministically over
   a captured frozen ledger.

Operational gate for every leg (Lesson 8): selective CIM reset, port 8000 empty, baseline
VRAM confirmed, the UTF-8 canonical launcher, one server log + one leg log, the watchdog
on long renders, queue/history/runner verdicts checked, and ONE variable changed per
comparison.
...
### 9.4 Build sequence and sprint receipt

**Build sequence (r4 codex M13).** Order matters, because two of the gates cannot be
reconstructed after the fact:

1. CAPTURE and COMMIT the pre-feature baselines FIRST (9.1 item 8) -- prompts,
   prompt_hashes, dispatcher request keys, serialized ledger, per registered pack.
   Once the code changes there is no immutable "before".
2. Land the node + the shared helpers + the three consumers + registration + the tests
   + the canonical JSON + the regenerated variants as ONE activation chunk. Code that is
   not wired is dead (CLAUDE.md section 0); a node wired without its consumers is worse.
3. Run every gate: focused tests, full Windows suite, Bug Bible, AST/JSON/no-BOM/
   no-zero-byte, workflow validator + round-trip + widget/link audit. Commit AND push.
4. Qualify 30w, then 120w, on the 9.2 ladder. Record 720w explicitly as RUN or NOT RUN
   -- silence is not a receipt.
5. Verify `HEAD == origin`.

**PASS is legal only when:** every gate in step 3 is green; the 30w AND 120w legs are
green on both local families and the declared cloud lane (or that lane is explicitly
recorded as NOT RUN with a reason); a still was generated and carries artifact-authored
clauses; the published file exists under `otr\obs\`; and `HEAD == origin`. A feature that
ships with a family that could not be qualified ships with a DECLARED supported-model
list -- it never silently degrades to a named pack.
...
authoritative_writers:    OTR_DynamicStoryDirection -> meta.visual_direction
                          OTR_ImageGenDispatcher   -> image-row direction provenance
                          OTR_ShotLock             -> video creative-sidecar provenance
...
canonical_workflow_hash:  <after the node 96 / link 284 delta; 24 nodes, 58 links>
variant_stamps:           <regenerated + verified>
focused_tests:            <9.1 items 1-25>
full_suite:               <passed/skipped/xfail>
bug_bible:                <result>
model_pairings:           mistral-nemo | gemma-4-E4B | <declared cloud lane, constrained?>
30_word_receipts:         <ledger + still + asset paths per pairing>
120_word_receipts:        <ledger + still + asset paths per pairing>
720_word_receipts:        RUN <paths> | NOT RUN <reason>
live_ledgers:             <paths>
...
CLOSED by r4: `is_dynamic` is runtime-only, never a persisted pack key (2.3); the shots
universe is a projection minus the synthetic open and minus mesh/plate-only lines (2.6);
the talking-safety law stated precisely (5.4a); no truncation anywhere (2.5); the Google
lane runs UNCONSTRAINED (4.3); `order = 4` with shifts (8.3); `source_recheck` and
`artifact_meta` cut; byte-identity moved off the live leg and the legs made
still-consuming (9.3).

**Standing panel dissent, overruled (recorded, not relitigated):** codex asked in r3 and
again in r4 to CUT the unwired `done` and `direction_report` outputs. Overruled: both have
exact unwired precedents in the same live graph (ShotLock ships `shot_report` and `done`
with `links: []` today), they cost zero JSON and zero code, `direction_report` is the only
human-readable surface for a taste feature, and `done` is the affordance the I4 reopen
trigger needs -- with it, adding the ordering edge is a one-link change rather than a
...
**Smaller first cut, if the operator wants one.** The natural v0 is P-A ONLY: an
episode-specific PACK, no `shots[]`, no per-kind matrix, no dispatcher provenance. That
removes roughly half of section 8 and half of the test plan. This design does NOT
recommend it -- per-shot direction is the stated product intent -- but the fallback is
named here so the choice is explicit rather than discovered mid-build.

---

## 11. VERIFY-AT-BUILD checklist (Codex owns; r4 residue)

These are items the design NAMES but that must be measured or read off the real system
before the first line of code. They are not open design questions.

- [ ] **The brief's real storage shape and `episode_id`'s real home** (2.5). Read them
      off a live frozen ledger; build `DirectionSourceV1` on the real paths, with one
      canonical representation and no duplicated lighting field. Pin `cast[].traits`
      str-vs-list.
- [ ] **Token facts per model** (4.3, 4.4): advertised/native cap, configured safe cap,
      exact-vs-conservative counting, provider output floor, constrained mode available,
      and the ACTUAL repair-message size -- for Mistral-Nemo, gemma-4-E4B, and the
      declared cloud lane. Fail if any is unknown.
- [ ] **Every bound** (4.4): each authored string's maximum length, each list's maximum
      cardinality, the `mood` register vocabulary, the P-A/P-B reserves, the deterministic
      batch partition, the artifact maximum.
- [ ] **The full wrong-depth / unknown-key / duplicate / conflicting-candidate battery**
      for BOTH passes (4.1, 4.5), proving a successful deterministic repair runs the
      COMPLETE validator and spends NO additional LLM call.
- [ ] **Canonical JSON after the delta** (8.3): 24 nodes, 58 links, ids 96/284, exact
      `62 -> 96 -> {89, 90}` topology, zero widget slots on 96, reconciled output fan-outs,
      JSON round-trip, computed receipt hash, regenerated + verified variant stamps.
      Read node 63's stamp state before assuming the master-hash step is a no-op.
- [ ] **Cold package import** (8.2, 9.1.23): both mappings, the concrete class, the exact
      label, the full node contract.
- [ ] **Named-pack byte baselines re-run** (9.1.8) and the immutable-input cache policy
      tested (7.5).
- [ ] **Every prompt kind exercised** (7.4): scene-character, scene-beat, still-word,
      mesh-fodder, plate, portrait, talking -- proving each authored field is consumed
      exactly as the matrix says, and that a hostile dynamic tail cannot contaminate the
      talking lane (9.1.22).
- [ ] **Scheduler reality for I4:** measure the actual node execution order, peak VRAM,
      the unload result, and model-cache residency at Dispatcher entry. Only a REPRODUCED
      OOM reopens the ordering edge.
- [ ] **Live gates** (9.3, 9.4): reset / launcher / watchdog / queue / history / log
      receipts, full suite, Bug Bible, AST + JSON + no-BOM + no-zero-byte, a PBUG entry per
      live failure, pushed green commits, `HEAD == origin`.

---

END. Docs-only deliverable; no code, tests, prompts, registries, or
workflows/otr_canonical.json were touched. Codex owns everything in sections 8 and 11.
Kibitz artifacts: kibitz-runs/2026-07-12-dynamic-story-visual/{r1,r2,r3,r4}/
(driver_anchor.md, codex.md, antigravity.md, judgment.md, final.md). Panel: codex
gpt-5.6-sol @ ultra + antigravity gemini-3.5-pro; 6 agent calls across r2-r4; Cowork
Claude as grounded anchor panelist and sole judge throughout.
