...
on the mesh_fodder seam; r4 = contract tightening only, no new seams).
Judgments: kibitz-runs/2026-07-05-multimodal-stage3/{r1..r4}/final.md.

R4 CONTRACT PINS (final):
- ONE era_tail contract: composer entry points call `get_visual_style(meta)`
  ONCE (fail-loud); `get_era_tail` takes the RESOLVED style (or its era_tail)
  and does NO loader lookup itself -- its never-raises character is preserved;
  a legacy no-style call defaults to sci_fi_radio only when
  meta["visual_style"] is absent.
- Pack era_tail replaces ERA_TAIL_DEFAULT EXACTLY -- including empty string
  (anime/cartoon ship era_tail:""); "never empty" is a SCI-FI-pack property,
  not a helper invariant; test_brief_prompt_finishing :36-39 +
  test_still_spine_helpers :137-139 docs updated accordingly (default lane
  unchanged).
- Hash tests are FORCED-META, producer-side: for every object source assert
  prompt_hash == _content_hash(prompt) AFTER finishing (producers at
  otr_meta_brief_image_prompt :1556-1558, :1766-1769, :1781-1783,
  :1802-1804) and two styles => different prompt_hash/dispatcher keys (the
  dispatcher TRUSTS producer hashes, :482-535 -- it never recomputes).
- ONE loader vocabulary: `get_visual_style(meta)` (meta-driven) +
  `resolve_visual_style(style_id)` (id-driven, used by the 3C gate);
  no `get_style` name anywhere.
- forbidden_terms lint = case-insensitive substring over positive_tail /
  image_grade_tail / broadcast_tail / era_tail.
- list_style_ids(): deterministic-order test + sci_fi_radio-valid-choice test.
- 3C baseline stated: node-1 wv is 26 today, source_bank stays slot 25,
  visual_style = NEW slot 26.
