...
NEW str (11): portrait_look, portrait_look_talking, portrait_instruction_look,
scene_instruction_look, announcer_subject_face, announcer_subject_ltx_mouth (template
{form}x1 + mouth-vocab lint), announcer_subject_object, radio_object_look, plate_look,
non_character_emblem_fallback (template {base}x1), still_word_title_mood_style.
NEW dict (4): open_subjects (keys {synthetic, announcer, default}; templates {form}x1;
role "announcer_visual" maps to key "announcer" in Python -- r4 AG M2), motion_registers
(keys {announcer, music_open, music_close, music_inter}; 240-char load budget),
still_word_typography + still_word_backdrop (EXACT case-sensitive keys
{"noir","sci-fi","western","pulp","default"}).
Non-empty rule: all NEW str fields non-empty EXCEPT `scene_instruction_look` (EXEMPT --
`_build_char_scene_request` has NO existing look text, so sci_fi ships "" and the
composer appends only-when-non-empty; byte-identity by construction -- r4 AG M1).
PROVENANCE: additive `visual_style` + `prompt_field_source` keys on BOTH the image prompt
objects (:1731/:1771/:1787/:1808) AND the render-request observability (+trace allowlist
:2033) -- r4 codex M3. "B7" = tests/test_b7_forbidden_sweep.py (in-suite). A v1 pack
fails load naming the path + "upgrade to v2". Both agents' r4 verify-at-build checklists
are MERGED into section 5 by reference (kibitz-runs/.../r4/{codex,antigravity}.md).
Chunk-B delta tests must assert every authored field CHANGED from the A1 sci-fi defaults.
