# OTR GO-FORWARD -- ARCHIVED STEP HISTORY

Older CURRENT/PRIOR/SUPERSEDED step narrative split out of `GO_FORWARD_PLAN.md`
on 2026-06-28 to keep the live plan lean. Read-only history; the live current
step + durable sections (hard rules / forward order / runway / open tickets /
parked) stay in `GO_FORWARD_PLAN.md`.
...
>   the dark NotImplementedError scaffolds (triposr / triposg_talk / hunyuan3d_talk / trellis_talk / hidream_i1 /
>   sd35_large); DELETED `VALIDATED_ENGINES` + `validated_engine_names()` (dropdown == full registry); decoupled the
>   gpu-smoke / coverage / dep-pilot harnesses from the flag; removed the VOICE + LLM opt-in gates (OpenRouter gated
>   on creds only) with `test_audio_byte_identical` GREEN; docstring cleanup + `tests/test_registry_is_the_menu_guard.py`.
>   Suite 5728-5750/0, Bug Bible 16/3xfail, B7 green. The interim option-B (drive flag from dropdown) was REVERTED `cf4487a6`.
> - **Still-engine renames** (`c1132196`, `967c0a07`): `flat_still->still_flat`, `flux_still->still_pan` (de-fluxed the
>   registry row -> cpu/0/[] pure-ffmpeg), `still_kenburns->still_motion` (universal floor kept; all "Ken Burns" refs
>   scrubbed -> "pan"). Legacy aliases in `_engine_id_from_pick` so old saved picks resolve.
> - **Soak duration-gate fix `3991c019`** (this session, pushed): `assert_obs_final_playable` was comparing the obs
>   final's CONTAINER duration (= the longer video track, incl. the silent rolling-credits tail, BUG-410) to the master
>   audio -> false-failed every credits-tail episode. Now asserts AUDIO stream == master (within 2s) AND video >= audio.
>   Suite 5729/0.
> - **HuMo-14B character routing `1572861b`** (operator commit): `16gb_full.json` + node 87 character video sub-slot ->
>   `humo_14B_169` (so character beats get motion, not visualizer blinking); guard test + offline-catalog env-leak fix.
>   The hardcode is a production DEFAULT; the soak harness drives `character_visual` DYNAMICALLY per leg (the pin is not
>   a regression -- no revert). 
> - **Coverage-soak harness built** (this session, throwaway scripts): `_otr_cov_runner.py` (every video+image engine
>   x 3 slots, local-gemma writer, still_flat filler so visualizer never dominates, resumable + merged matrix) +
>   `_otr_build_dash.py` (embedded-snapshot dashboard; sandboxed artifacts can't call Desktop Commander -> 400 on live
>   read). The live coverage soak is the CURRENT STEP in GO_FORWARD_PLAN.md; its findings (ltx_audio_in VRAM regression,
>   broken image contract -> murky HuMo portraits, gemma normalize_length wrapper drift) are the post-soak sprints.

---

> **>>> PRIOR STEP -- 2026-06-28 ROUTE-A 14B PROMOTION SHIPPED + LIVE-VALIDATED + EXTENDED. HEAD `a919d2ad` ==
