...
- **Fix (Phase A — simple-pillarbox composite per Jeffrey's REVISED spec, 2026-05-03 EVENING):**
  ...
  - **Procgen visual fill is Phase B (separate next commit):** the visible HuMo black pillarbox bars are intentional. Procgen renders at native 1920x1080 (currently 832x480) and a new `OTR_PostUpscaleProcgenBlend` node overlays it on the RTXUpscale output AT delivery res. Per Jeffrey: "proc gen 1920x1080 ... then a final ffmpeg w/ the proc gen mix for final 1080p." The post-upscale blend turns the visible black surround into audio-reactive CRT scanlines -- the SIGNAL LOST visual signature -- without going through the AI upscaler (which would smear synthetic patterns) and without touching the per-clip-mux mode (which would risk C7 byte-identity).
