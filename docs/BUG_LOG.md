...
- **Fix (Phase A — landscape layered composite per Jeffrey's final spec):**
  - **HuMo render dims:** `nodes/batch_humo_render.py` widget defaults width=480→1280, height=832→720. HuMo now renders at 1280x720 LANDSCAPE @ 25 FPS (highest official trained resolution). VRAM cost ~2.3x more pixels per frame; smoke-test before long soak.
  - **LTX render dims:** `nodes/batch_ltx_render.py` module constants `LTX_WIDTH` 832→1216, `LTX_HEIGHT` 480→704 (preferred higher-detail per Jeffrey's research, divisible by 32, native to newer LTX-2.x distilled models).
  - **Composite canvas:** `nodes/video_composite.py` INPUT_TYPES defaults canvas_width 832→1472, canvas_height 480→832, humo_target_height 480→832. Workflow JSON node 52 widget values updated to match. New widget `humo_pillar_width` default 512 (HuMo center pillar crop width).
  - **New helper `_resolve_episode_background(episode_dir)`:** picks the freshest FLUX env still from the per-episode workspace (`<ep>/stills/full_env_*.png`), falls back to radio bookend, falls back to None. One backdrop per episode for Phase A; per-scene env still mapping deferred.
  - **New helper `_layered_per_clip_silent(...)`:** detects clip kind via `speaker_role`. Character clips with a backdrop available get a layered composite (background scaled-cropped to 1472x832 fills full canvas + HuMo scaled to 832 tall + center-cropped to 512x832 + overlaid centered on background via ffmpeg `-filter_complex`). Non-character clips (announcer/sfx/music_*) and character clips with no backdrop available fall through to a simple scale-crop fill of the canvas (LTX 1216x704 → 1472x832 fill with thin 19px side bars). Both paths drop audio (`-an`); master mix attaches at the final mux step downstream so C7 byte-identity holds.
  - **`_render_master_mix_per_clip_mux_mode` rewired:** both call sites (in-loop + post-loop tail-pad re-pillarbox) switched from `_pillarbox_humo_silent` to `_layered_per_clip_silent`. The legacy helper is kept for back-compat with smoke tests + non-layered standalone use cases. Background image resolved ONCE per episode at the top of the loop and passed to every per-clip call.
  - **Math sanity:** HuMo 1280x720 → scale to 832 tall = ~1480x832 → crop center 512x832 pillar. LTX 1216x704 → scale-cover 1472x832 = ~1472x852 → crop to 1472x832 (drops 10px top + 10px bottom, ~3% loss). Final RTXUpscale 1472x832 → 1920x1080 is clean (16:9 source → 16:9 delivery, 1.30x scale).
...
- **Tags:** video-composite, layered-composite, landscape-canvas, humo-orientation, ffmpeg-overlay, qa-soak-2026-05-03
- **Related:** BUG-LOCAL-027 (dialogue wipe — orthogonal); BUG-LOCAL-028 (per-episode FLUX save paths — provides the env stills used as backdrop here); BUG-LOCAL-029 (ULTRA_SMOKE format normalize — orthogonal). Round-robin consult was SKIPPED per direct user override; AST + invariant audits + targeted regression + Bug Bible regression all green pre-commit. **HuMo at 1280x720 is OUT-OF-DISTRIBUTION** (trained at 480x832 portrait, with 640x640 as the only verified alternative) — Jeffrey accepted the smoke-test risk. Documented fallback in widget tooltip if VRAM gets tight: drop to 832x480 @ 25 FPS.
