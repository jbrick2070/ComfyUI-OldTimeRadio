...
- **Headless soak status (2026-05-03 EVENING):** DEFERRED. ComfyUI Desktop was terminated mid-session by an autonomous-pass cleanup `taskkill /F /IM python.exe` (originally to clear a hung pytest from BUG-LOCAL-006 dropdown_guardrails). One-shot soak script written and committed at `scripts/soak_bug027_028.py` — Jeffrey runs `scripts/run_comfyui.cmd` in the morning, waits for ComfyUI to register on `:8000` with the f1467a2 code (script auto-checks for `OTR_SaveToEpisodeWorkspace` in `/object_info`), then runs the soak script. Acceptance signatures and per-episode workspace expectations are printed at the end of the script's output for the live runtime-log tail.
