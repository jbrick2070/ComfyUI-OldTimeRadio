...
**Status: implementation and local qualification complete. The first high-RAM
RunPod LTX 2.5 attempt produced a negative 24 GB GPU-VRAM receipt; a clean
48 GB L40S qualification and a fresh local-5080 episode are in progress.**
...
| **LTX 2.5** | The provisioner pins ComfyUI-GGUF and ComfyUI-LTXVideo, applies the in-repo Gemma-4/BF16 patch, and the engine semantically verifies the registered loader. `MANUAL_TIERS["ltx25"]` in `scripts/otr_provision.py` is the authoritative five-file weight manifest. After the Lightricks terms click, `docs/RUNPOD_INSTALL.md` is the sole copy/paste procedure; an executable parity test prevents it drifting from the manifest. | **PROVEN on the RTX 5080 Laptop only.** The shipped Foley path is 832x480 source, 1664x960 two-stage output. The prior Ampere pod reached decode and was SIGKILLed at a 57.7 GiB cgroup limit: a negative receipt for that RAM cap, not for Ampere or RunPod generally. A later high-RAM RTX 4090 pod passed CUDA, provisioning, all weights, node loading, canonical validation, and queue submission, then GPU-OOMed at node 92 `decode` after 39:56; its cgroup recorded no OOM event. That is a negative receipt for the exact 24 GB tuple at the shipped canvas, not a global RunPod refusal. Reserve/clamp experiments are not physical-card qualifications. The physical-4060 LTX 2.5 plan has no completed receipt, so physical 8 GB remains **UNKNOWN/unqualified**. |
...
5090, L40S, 3090, 3080 Ti, 3080, and other rental configurations are
**CANDIDATES**, not refusals and not yet promotions. The exact high-RAM RTX 4090
attempt is the 24 GB negative receipt described above. The later physical-4060
pass should rerun the canonical still and
...
## Integration and file-ownership receipt

The other coder's branch was fetched before overlapping files were edited, and
the matrix/config/provisioner files were clean at that boundary. The stale
machine-matrix claims about an unpublished LTX patch and a missing HuMo lane
are now corrected in their executable owners and regenerated documents.
`config/profiles/otr_runpod_starter.json` and `README.md` remain untouched.
Staging is by explicit filename only so unrelated scratch work cannot enter the
commit.

## Local verification receipt

The definitive Windows suite completed with **12,612 passed, 121 skipped, 1
expected xfail, and zero failures** in 533.76 seconds. The separate Bug Bible
completed with **22 passed, 27 skipped, and 3 expected xfails**. Ordered
