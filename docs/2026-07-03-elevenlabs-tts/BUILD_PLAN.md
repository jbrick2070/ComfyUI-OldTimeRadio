...
estimate per line would trip the $10 cap ~line 9). Hidden auth is BRIDGE-LEVEL
and already proven on the image lane: `_inject_hidden_inputs`
(`cloud_media_invoke.py:363-384`) fills from `session.auth`, `resolve_auth`
(`cloud_media_backend.py:135-154`) takes `OTR_COMFY_API_KEY` as precedence #1.
No per-adapter auth work — but headless S7 REQUIRES `OTR_COMFY_API_KEY` set.
...
- **S1 — cloud voice adapter:** register the `elevenlabs` adapter
  ...
  **FIX-1 (Fable final):** `_engine_by_node_key()`
  (`test_cloud_partner_conformance.py:50-59`) iterates only `ireg`+`vreg`, so
  removing the xfail makes `test_billed_row_has_adapter_or_explicit_xfail[cloud_elevenlabs_tts]`
  raise at `:88-90` (adapter exists but the map can't see it). SAME sprint:
  extend `_engine_by_node_key()` to the AUDIO registry (iterate `areg._REGISTRY`
  / add `all_engine_names()`) and give the cloud audio adapters `node_key` +
  `_partner_inputs` so the kwargs-conformance test checks them. The
  adapter-registration + CAPABILITIES row (C1) must land in the SAME COMMIT (the
  parity test `:213` is bidirectional set-equality).
...
- **S6 — workflow JSON: DROPDOWN VALUES, not graph nodes (Fable F-MF7):** the JSON
  ...
  cloud image with `flux_gen1`). **FIX-2 (Fable final):** do NOT flip the MASTER
  workflow's engine widget values to the cloud engines —
  `test_capability_profiles.py:173-202` asserts the master's engine values EQUAL
  `16gb_full.json`'s slot_overrides, and flipping master (or profile) contradicts
  the "defaults never change" invariant + S7's profile-less rationale. S6 ships
  NO engine-value edit to the master — only `OTR_WorkflowValidator` +
  link/widget audit; the cloud pick is applied PER-RUN by the acceptance harness
  / operator dropdown. Graph-wiring the real partner nodes would bypass
  `invoke_partner_node` (no budget/canonicalize/ledger/gate) — do NOT.
...
## 5b. Verify-at-build (runtime-only; NOT plan defects — Fable final gate GO)
Fable's final grounded pass confirmed all seven cross-cutting fixes are correctly
integrated and found no remaining build-breaker. Three items are only
confirmable at build time:
1. Whether `ElevenLabsVoiceSelector` accepts stable voice IDs or only mutable
   display labels (decides the C4 capture-test design).
2. Whether the local lane exposes a single explicit loudness constant to point
   `LOUDNESS_REFERENCE_SOURCE` at, or it is implicit in the assembler — budget
   S0 time either way (C8).
3. The S0 V3 re-pin regenerates ALL of `partner_nodes.yaml`; diff it for
   unrelated image/video row drift, and land adapter+CAPABILITIES in one commit
   (the parity test is bidirectional set-equality).
