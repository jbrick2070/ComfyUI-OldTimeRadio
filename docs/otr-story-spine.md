...
| canonical workflow JSON | `workflows/otr_scifi_16gb_full.json` (29 nodes; writer = node id 1) |
...
- **Headless, any session (venv python `C:\Users\jeffr\Documents\ComfyUI\.venv\Scripts\python.exe`):** the module's `__main__` self-test; Bug Bible regression (`comfyui-custom-node-survival-guide/tests/bug_bible_regression.py`, 26 tests, ~0.02s, no GPU); core + touched unit suites; the forbidden-sweep (`docs/_s28_forbidden_sweep.py`). Subagents validate units only.
