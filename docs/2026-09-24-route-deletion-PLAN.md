...
HEAD to build on: `e8960496333d4e8f00ed55b29a9eebaf40e208c9`.
...
- Dated archive, same commit as the policy deletion:
  `docs/2026-09-24-lemmy-listen-archive.md`. Quote the 2026-08-18 listen
  path, the winning clone ids, and the hashes that lived on the approved
  IndexTTS2 / chatterbox / dia records. Optional: a JSON snapshot beside
  it that nothing imports. Add a supersession line under
  `docs/OTR_STANDING_RULINGS.md:1923` -- the listen stands; the receipt
  machinery is retired; identity is `RECURRING_CHARACTER_VOICES` +
  `reserved_for`.
- README "Known failures" (`:746-762`): drop the portable-bank route-test
...
- `docs/TTS_VOICE_PREFLIGHT.md`: retire Gate 4 entirely (P4.1-P4.6). Fix
...
- `docs/2026-09-19-ship-regression.md`: APPEND a dated correction naming
  the nodeids this work deletes. Do not rewrite the historical 2.3.0
  receipt body.
- `docs/OTR_STANDING_RULINGS.md:41-50` and `docs/known-failures.md`:
...
1. Capture the failing nodeid set at `e8960496` before touching anything.
   The `e67b2904` worktree is the wrong baseline.
