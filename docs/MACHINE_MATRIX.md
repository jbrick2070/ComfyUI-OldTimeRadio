...
Apple Silicon is `otr_mac_mps`, PROVEN on a named physical system -- a Mac mini M4 / 16 GB published episodes to `otr/obs/` on 2026-09-07 and 2026-09-08, including local `sd15` stills and local `ltx_8gb` video diffusion. It is not promoted to a machine key: a machine key implies a measured VRAM tier, and one 16 GB Mac is one data point, not a tier. Read `apple/MAC.md` before starting -- it ships with the pack, unlike `docs/`. CPU-only is the `otr_cpu_low` graph, PROVEN 2026-09-13: ComfyUI launched with `--cpu` on an x86 laptop, the GPU present and unused, published a one-act episode to `otr/obs/` in 34.5 minutes -- Kokoro voices ran at 0.12x realtime on the CPU. One system, one episode, one act: a receipt, not a tier.
