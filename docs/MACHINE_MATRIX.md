...
> **Apple Silicon:** the canonical renders end to end and publishes to `otr/obs/`
> (proven 2026-09-07, Mac mini M4 16 GB). Setup, manual steps and the engines that
> cannot work locally: **`MAC_PORTABILITY_GUIDE.md`**. There is no local image engine
> and no local video-diffusion engine on that platform -- the visualizer lanes are the
> only fully-local video path, which is why the shipped canonical uses them.
