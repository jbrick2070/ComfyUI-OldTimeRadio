...
| 9-C3 | **Macbeth safety probe (item 9 chunk 3)** -- r1-r4 kibitz arc CLOSED 2026-08-08, **LOCKED spec at `kibitz-runs/2026-08-08-macbeth-safety-probe/r4/final.md`** (gitignored; framing doc `docs/2026-08-08-PROBLEM-STATEMENT-macbeth-safety-probe.md` is tracked). Implementation opens directly against that spec. Scope LOCKED to **B1 (Macbeth II.ii) x A1-A4 = 4 cells** (~$0.72) -- both profile gates say "ONE deliberately violent adaptation beat", so the 3-beat ladder was over-scoped. **Discharge is CONDITIONAL:** freeze the provider-bound prompts, inspect them, and if they do not retain explicit violence the harness REFUSES to discharge and escalates to the operator. All cloud creds live + funded | `scripts/otr_macbeth_probe.py` (new) | coder + 1 live run | nothing -- unblocked |
