...
| obs publish OSError ends the prompt | `_publish_to_obs` does an unguarded `os.makedirs` and raises OSError on a failed re-encode, inside `mux()`'s blanket `except (ValueError, OSError)` which logs and re-raises. Real trigger: `OTR_OBS_DIR` on an unmounted share -- and the 4060's obs link IS a mapped drive over direct Ethernet. | **REFUSED after grounding, and the audit agent missed why.** That blanket handler exists because swallowing it once produced a PHANTOM GREEN on 2026-07-14: "the graph completed, ComfyUI logged Prompt executed, the harness recorded RESULT SUCCESS -- and no file existed." Degrading it re-opens exactly that. There IS a narrow true core -- the archival master has already been written by that point, and the node already has a "withheld, still success" path for a deliberate non-publish -- but separating an I/O failure from a correctness gate here needs the operator's call on whether a run with no obs file may ever report success. Blocked on that ruling, not on code. |
...
**NEW 2026-09-11 -- may a run with NO obs file ever report success?** This is the one
question the obs-publish row is blocked on, and it was previously "blocked on a ruling"
without the ruling being asked anywhere. The case: the archival master is already
written and correct, and only the COPY to `otr/obs/` fails on I/O. *"Only an out of
memory should fail"* says complete the run and say so loudly. Against that: the blanket
handler that would have to change exists because swallowing it once produced a PHANTOM
GREEN on 2026-07-14 -- *"the graph completed, ComfyUI logged Prompt executed, the
harness recorded RESULT SUCCESS -- and no file existed"* -- and seeing episodes in
`obs/` is how success is read at all. One word unblocks it.
