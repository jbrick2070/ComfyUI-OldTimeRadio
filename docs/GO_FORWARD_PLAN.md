...
| **voice-pool concentration -- MEASURED 2026-08-18, cause found, NOT yet fixed** | **OPEN.** Operator's concern was *"many voice models were blocked and we're still limited to a handful"*. **Nothing is blocked** -- 206 bank entries, **zero** quality-rejects, only the 3 reserved Lemmy clones removed (98.5% castable), and no caller requests `require_commercial_clean`. **But he is right about the outcome.** Measured over **1351 episodes / 2011 voiced rows**: ANNOUNCER rows 654, only **10 distinct, top-5 = 99%**, `bm_george` alone **82.3%** -- expected, the announcer has its own pinned path (`announcer_voice_ref`), not the character pool. **CHARACTER rows 1357, 56 distinct, top-5 = 66%**, `vz_bill_boerst` **23.7%** + `vz_caro_davy` **18.8%** = 42.5% between two voices. ~~**THE CAUSE IS THE MATCH LADDER, NOT A BLOCK.**~~ **STRUCK 2026-08-18 NIGHT -- THAT CAUSE DOES NOT REPRODUCE AND THE SUBSYSTEM IT NAMES IS NOT THE ONE THAT RUNS.** Measured by calling the real `assign_voice_for_slot` across 400 seeds x 3 char_ids: male+`warm` draws **13 distinct at top-5 44%** (uniform over 13 would be 38%), female+`warm` 22 at 28%, female+`bright` 17 at 32%. **The scorer is flat**, tier-1 pools are healthy (13/17/12, not thin), and `vz_bill_boerst` does not reach the top 6 of its own ask. **THE REAL CAUSE IS THE HYBRID LLM VOICE-FIT, WHICH SITS IN FRONT OF THE SCORER.** `hybrid_voice_fit_enabled()` (`_otr_casting.py:896`) is **default-ON**: the LLM is shown `build_voice_cards()`, proposes one id, and `cast_lock.py:884-906` stamps it and `continue`s -- never reaching `:909-926` at all. Measured over 1711 ledgers: **1871 rows from accepted proposals vs 82 fallbacks**, so the scorer handles ~4% of casting and tuning it would change almost nothing. **TWO STACKED NARROWINGS, NEITHER THE LADDER:** (1) `build_voice_cards` (`_otr_voice_bank.py:667-672`) sorts **alphabetically** and takes the **first 12** -- a fixed prefix, the same voices every episode forever, leaving **17 of 40 castable voices (42%) permanently unreachable**, including every LibriVox narrator and both LJSpeech rows; (2) the LLM then collapses the survivors -- **1877 proposals, 18 distinct, top-5 = 95%**, with 9 of the 26 ever-offered voices never proposed once. `vz_bill_boerst` and `vz_caro_davy` are the **first male and first female card**; primacy on a fixed list is the whole 42.5%. **THE OPERATOR WAS LITERALLY RIGHT ON BOTH HALVES** -- voices ARE effectively blocked (by alphabet, which no quality-reject audit could see) and the result IS "a handful". The evening's "nothing is blocked, 98.5% castable" measured the 4% path. **A 2026-08-04 kibitz round already named this exact layer** (`kibitz-runs/2026-08-04-continuity-ultracode/input_voice-variety.json:174` -- *"whose 12-card truncation is a harder variety cap ... measures a path production can bypass entirely"*) and was never acted on. Full grounding: `docs/2026-08-18-ANCHOR-voice-pool-concentration.md`. **ARC RAN AND STEP 1+2 ARE DONE 2026-08-18. The pass is OFF and the scorer is
...
| PBUG-20260817-08 | **RE-OPENED AND RE-FIXED 2026-08-18 NIGHT -- the 08-17 fix covered ~4% of production casting.** `reserved_voice_ref_ids()` was applied only inside `assign_voice_for_slot`; the default-ON hybrid LLM path (`build_voice_cards` -> `validate_voice_proposal` -> `cast_lock.py:884-906`) never knew reserved ids existed, and because cards are alphabetical-capped-at-12 **Lemmy's `idx_lemmy_algenib_cockney_v1` was CARD #1 on every male slot**. Corpus: the LLM proposed a reserved id **21 times, accepted 21 times** -- DON PEDRO, MARCELLUS, BANQUO, FLETCHER CORBEN, STARBUCK, FERDINAND, MOE GORDON, Dr. Alexei Petrov. **`signal_lost_rivers_embrace_20260817_233013` was NOT the resident-server trap** -- that explanation is struck in `PROD_BUG_LOG.md`; the path was simply unguarded, reproduced live at HEAD on a freshly imported module. **All three 08-18 verifications were sound and all blind to it:** the 480 draws swept the 4% path, and the clean-corpus window was a 1.1% leak rate not firing. **A SONNET QA PASS THEN FOUND A THIRD POOL, and it was a BLOCKER:** `gender_agnostic_fallback_ref` (3 call sites) drew uniformly with no reserved filter, and canonical gender `other` is **20% of every roll** with zero bank rows, so every such row lands there -- measured **7-9 of 200 draws per engine** returning Lemmy's clone, which is exactly the 2 non-hybrid leaks in the corpus. Now **0/200**. The helper was hardened too (fail-soft on any exception, not just `ImportError` -- it has four callers and two promise never to raise). Fixed at all three pools + `tests/test_lemmy_reserved_on_hybrid_path.py` (18 tests). **A pre-existing test had the defect as an EXPECTATION** -- `test_hybrid_voice_fit.py::_real_male_indextts2_id` returned the alphabetically-first male id, which IS Lemmy's, so it asserted CastLock stamps his Cockney onto a character named BOB, and passed. ~~**CLOSED 2026-08-18 -- the row was STALE, the fix shipped 2026-08-17 (`8f3c7615`).**~~ Lemmy's own clone refs are reserved out of the `char_voice` pool, derived from the policy rather than a hand-kept list. Verified three ways: corpus (the only two leaking episodes are both from 08-17; everything from 08-18 is clean), selector (`tests/test_lemmy_voice_stays_reserved.py`, 480 seeded draws, zero reserved refs reach a slot), and the operator by eye -- *"i do feel we have been seeing the right amount of lemmy"*. **The 10.2% figure never meant 10.2% wrong** -- it was that ref's share of all cast rows over 40 episodes SPANNING the fix, most of it legitimate cameos. **The trap worth keeping:** the leak was seen 16h AFTER the fix commit because the soak harness never tears its server down, so an evening leg still ran the module loaded that morning. A post-fix sighting from a resident server reads exactly like a failed fix |
| PBUG-20260817-07 | stage directions in captions -- **WILL-NOT-FIX** (operator ruling) |
| 24 stale PBUGs | **SWEPT 2026-08-18. 35 open -> 18.** 29 rows triaged against HEAD and **18 closed with the evidence a reviewer needs**, previous status preserved on every row. Eight were ALREADY FIXED (several firing in ledgers published the same day -- `protected_fact_component` in `signal_lost_the_16mm_ransom`, `MALVOLIO.gender == "male"` at tier `exact`, `cast_contract.lemmy_hit=True` on a forced-cameo leg). Ten were OBSOLETE: the scifi_news/Codex rip (`dae1fb3c`) deleted the modules nine of the July-11 rows describe, and the Ollama daemon they assume returns **zero hits** in `nodes/`. **The trap worth keeping: two of that block were NOT bulk-closed** -- `PBUG-20260711-05`/`-06` look like siblings but their fixes GENERALIZED into shared infrastructure (`extract_first_json_block` in `_otr_json.py`, `schema_required_paths` in `_otr_structured_call.py`) that every live lane still calls. Closing them as "module deleted" would have been wrong, and a bulk sweep would have done exactly that. The remaining 18 are correctly open, several by explicit operator ruling |
...
| media_archive coda shape fix | **DONE AND LIVE-VERIFIED 2026-08-18 NIGHT.** `signal_lost_the_16mm_ransom_20260818_145217`, published to `otr/obs/` (RESULT SUCCESS, 1920x1080 h264+aac, 3:11, 30 MB). The coda landed the shipped contract exactly: *"From tonight's hidden vault of celluloid: **The Library of Congress** is hosting a screening of 'The Andromeda Strain' at the Mary Pickford Theater on August 20, 2026. **It is a testament to the enduring power of film when we treat the past with such careful stewardship.**"* -- names what was found, names WHO found it (an institution), two sentences, ends on one reflective thought, no explainer. **The scifi twin's worst failure mode is absent too:** the OPENING announcer stayed pure fiction, so there is no fact/fiction/fact sandwich -- one clean pivot at the end. Leg ran ~26 min (per-segment video is the long pole, by design) |
