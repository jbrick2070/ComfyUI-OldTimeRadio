...
fix, the operator's five deletes, the fingerprint-recipe narrowing, the
-14 LUFS master and the LTX 2.5 recipe module):** suite
**11100 passed / 110 skipped / 1 xfailed** -- MEASURED by a full run, never derived. Chain this window, each
delta exactly the tests added and no regressions at any step: **11080** at open
-> **11094** with the 14 title-anchor tests -> **11104** with the 10
durable-headline tests -> **11070** after four of the operator's deletes -> **11072** after the fifth
(the stale-ledger voice guard: -3 guard tests, +5 pinning the removal and the
...
| **LTX 2.5 INTEGRATION -- CHUNK A BUILT AND GREEN; G8 SMOKE IS ALL THAT IS OWED** | **CHUNK A IS CODE-COMPLETE, NOT YET PROVEN ON PIXELS.** The lab shipped a spec, a Final QA, and an executable golden recipe (`vram-recipe-lab/recipes/ltx_2_5_golden_i2v_foley.json`). Reviewed against all 8 gates of `VIDEO_LANE_PREFLIGHT.md`; full arc record in `kibitz-runs/2026-08-19-ltx25-integration/`. **THE RULING: SPLIT.** (A) a silent video lane is buildable now; (B) the foley bed is NOT, because video renders FOUR topological stages AFTER the master freezes (order 16 vs 12, driver-verified) -- foley does not exist when the master is assembled. **The lab's ffmpeg late-mux is IMPOSSIBLE here, not merely unwise:** four independent `-an` strip points (`otr_silent_composite.py:771`, `:1231`, `:1239-1242`, `otr_caption_burn.py:213`) and `otr_master_audio_mux.py:278` never maps `0:a`, so their filtergraph would error "Stream specifier ':a' matches no streams"; it would also break the mux's `audio_pcm_sha` byte-identity assert. **AND CHUNK B IS STRICTLY HARDER THAN MIME** -- mime REPLACES dialogue so it needs no line positions, while a bed under dialogue needs positions only finalised post-freeze. Mime's 2026-08-10 ruling gives the ROUTE (audio enters the master pre-freeze) but not the mechanism, which it calls "sacred-path surgery with its own arc". **SHIPPED THIS SESSION:** `nodes/_otr_video_engines/ltx25_recipe.py` (the locked recipe as data) + `tests/test_ltx25_recipe_matches_lab_golden.py` (14 tests, a drift gate against the lab's actual file). **BUILT 2026-08-19: `nodes/_otr_video_engines/eng_ltx25.py`** -- internal id `ltx25_video`, public `ltx25_high_video`, 23 declarative nodes translated from the golden 26. Suite **11141 / 111 / 1** (+42 = 26 new tests + 16 registry-walk instances, every one itemised, no regressions); preflight matrix 17/17 with this lane in it; ENGINE_MATRIX regenerated; variants still 50/0. **NEXT ACTION: THE G8 SOLO SMOKE**, which is the ONLY thing between this lane and done -- `scripts/_otr_single_engine_smoke.py --engine ltx25_video`, then a canonical leg published to `otr/obs/`. Until it runs the lane has no cost row, no envelope key and NO `compatible_boot_contracts` (declaring one would be a guess), and the evidence manifest says admission-unenforced in words. **SEVEN DEFECTS WERE CAUGHT BEFORE ANY RENDER, none by the preflight checklist:** `ResizeImageMaskNode` is a V3 DynamicCombo whose API-format dotted keys (`resize_type.width`) CANNOT bind through `run_graph`'s `fn(**kwargs)` -- proved both ways against the installed class, nested dict returns (1,480,832,3) and the dotted form raises; the decode knobs 512/64/33/4 were bare literals and are now homed in `ltx25_recipe.py` so nobody inherits `eng_ltx_av`'s env-driven 4096/8 by resemblance; `session_identity()` was missing on a lane `can_split` says is splittable (the exact gap that once refused a live leg 730 s in); `_clip_from_raw` was inlined so the frame-receipt registry walk could not reach it; `native_frame_count` stamped the pre-trim RUNG, advertising 97 real frames in a 50-frame clip -- an over-claim of the shape a padding lane makes, invisible to the conformance stub; and a `docs/` path in a comment was scraped by `tools/engine_matrix.py` as this lane's FRAME-CAP evidence, printing H3's licence attestation in the cap column (twice -- the elided form matched too). **AGY r3 REVIEWED THE FINISHED DIFF (scoped single-lane pass, NOT a full arc; receipts `kibitz-runs/2026-08-19-ltx25-chunk-a/r3/`)** and killed a mechanism: the driver had SPLIT the video VAE in two copying `eng_ltx_av`, and it buys nothing -- `_topo_order` is Kahn with ties on sorted node id, a `VAELoader` has no deps, so the decode copy is scheduled in the FIRST batch regardless and held to the end; the lab also puts both VAEs at 0.0 GiB at peak. Collapsed to ONE loader, which now matches golden node 2 exactly. It also SETTLED the continuity question the driver could not: `strict_first_frame` is EARNED, because `render_driver` hands segment N's terminal frame to N+1 and `coverage_plan` gives every later segment `drop_head=1`, so the VAE-round-tripped frame 0 is DROPPED at assembly and never reaches the screen -- `soft_reference` would force a JUMP join that hard-cuts. One agy claim was REJECTED with the line: it wanted `img_compression` 0 -> 0.0 for "schema parity", but the schema is `io.Int.Input` (`nodes_lt.py:727-729`), so the int is correct and the JSON's float is a UI artifact. **FOUR FACTS THE DRIVER ESTABLISHED LOCALLY -- do not re-derive them:** (1) the anchor's `strength: 1.0` is the NODE'S UNTOUCHED DEFAULT (`comfy_extras/nodes_lt.py:142`), so it is an inherited value and NOT evidence of a measurement -- our sibling lane's deliberate 0.7 remains the only measured opinion in the tree, and 1.0-vs-0.7 wants OUR ear, not a lab question. (2) `img_compression: 0` IS deliberate -- the node default is 35 (`:728`) and `preprocess()` returns the image untouched at 0 (`:705`), so they knowingly made it a pass-through, which is defensible for a clean synthetic still. (3) **The lab's input still was ALREADY 832x480** (`vram-recipe-lab/fixtures/scene_still.png`), so the graph's resize/centre-crop path NEVER RAN in any of their tests -- OTR's wide scene stills will be the first thing to exercise it, and that is the most likely place a first render surprises us. (4) `LTXVAudioVAEDecode` is ALREADY WIRED as node 34 -- the golden recipe IS the foley variant, so **Chunk A is a SUBTRACTION** (drop node 34 and the audio input to `CreateVideo`), not a later addition, which de-risks Chunk B because the foley path is already proven wired. |
| **LTX 2.5 -- the three lanes, and what is still owed** | **NAMES SETTLED, ONE TOKEN OPEN.** Operator wants all three registered: silent video, `mime` (clip's own foley+music IS the beat audio, no TTS), `foley_plus` (TTS primary, foley bed under). Per lesson L5 that is **three internal engines, never one id with a switch** -- two public ids on one internal id collapses `_INTERNAL_TO_PUBLIC` and trips the bijection assert AT IMPORT, emptying most of the ComfyUI menu. Internal ids `ltx25_video` / `ltx25_mime` / `ltx25_foley_plus`. **TOKEN SETTLED: `high`. THE 4060 IS OFF THE TABLE** (operator, 2026-08-19: *"forget LTX 2.5 on 4060, that is not happening, so don't waste anything on that"*). Public ids are **`ltx25_high_video` / `ltx25_high_mime` / `ltx25_high_foley_plus`**. No clamp test, no `OTR_HEADLESS_RESERVE_VRAM_GB` experiment, no lab question -- the lane is 5080-only and `high` is exactly what that token means. Labels stay SHORT per the operator: what the lane does, no VRAM archaeology; the number goes in the lane receipt.
...
| **LTX 2.5 -- two things the golden JSON corrected, do not re-read the prose** | **PROSE vs FILE, and the file that RAN wins.** (1) The QA describes the first-frame anchor as `SetLatentNoiseMask`; the recipe actually uses **`LTXVImgToVideoInplace` at strength 1.0** -- a node this repo ALREADY wires (`eng_ltx_av.py:822`, `eng_ltx_video.py:1233`). (2) The QA says the scheduler's `latent` connects to `EmptyLTXVLatentVideo`; the JSON connects it to the **ImgToVideoInplace OUTPUT**, a different tensor on the I2V path -- wiring it the documented way hands the scheduler a latent with no still baked in, a wrong-but-running failure. **THIRD FACT WORTH KEEPING:** the audio VAE is required even by a SILENT lane (`LTXVEmptyLatentAudio` mints the audio latent, `LTXVConcatAVLatent` needs it), so "we discard the audio" never meant "we avoid paying for it". **AND ONE FOR THE OPERATOR'S EYE:** the golden recipe anchors at strength **1.0 (hard pin)** while the sibling audio lane deliberately uses **0.7 (soft)** -- adopted as measured, flagged not reconciled. |
...
| **LOUDNESS: he chose D (-9.0 dBFS) BY EAR, so `normalize_dbfs` MUST be wired** | **OPEN -- decided question, undecided implementation.** Blind A/B of four arms built on CPU from a real master (`signal_lost_under_the_enchanted_moon_20260819_062006`), varying ONLY the peak ceiling: he picked **D, the quietest at -9.0 dBFS**, against a shipped hardcode of **-1.0**. **That settles wire-it-vs-remove-it** -- he wants a value the slider cannot deliver at any position today. Note he chose the QUIETEST arm, the opposite of the usual loudness bias in blind listening, so it reads as a real preference rather than noise. **WHY IT IS NOT DONE YET:** `_master_loudness(waveform, ceiling_dbfs=-1.0)` (`nodes/scene_sequencer.py:171`) already TAKES the ceiling and the call site (`:1362`) simply passes nothing -- but the widget lives on `OTR_AudioEnhance` while the work happens in the Assembler, so wiring it is cross-node plumbing that touches the canonical workflow, which the 2026-08-17 amendment makes an explicit **full-arc** trigger. **Do NOT paper over it with an env var:** the defect is that an exposed widget lies, and an env var leaves it lying. **One thing to put to him before changing the shipped default:** the arms differed by LEVEL ALONE, so a listener's volume knob undoes the difference -- the number matters chiefly because episodes sit in `otr/obs/` next to other content, where a -9.0 dBFS master reads as noticeably quieter |
| **the five STATIC findings from that sweep** | **OPEN, awaiting live observation. Do NOT promote these to PBUGs on the strength of the audit alone.** (1) **`cloud_media_cache` has zero production callers** -- `nodes/_otr_shared/cloud_media_cache.py` is exported and implemented, but production goes straight through reserve->submit->bill at `cloud_media_invoke.py:797-809`, so identical cloud requests are re-billed with no dedup. LOW priority: cloud is opt-in and the project is offline-first. (2) **`announcer_only_fallback` is read but never written** -- the empty-cast escape hatch at `_otr_ledger_freeze.py:438-445,528-531` has no producer, so the documented announcer-only case cannot pass freeze. (3) **`assert_registry_ledger_has_voice_ref_id` has zero production callers** (`_otr_audio_cache.py:216`) -- a stale cast-locked ledger missing `voice_ref_id` silently enters render-time reassignment instead of being rejected for recasting. (4) **`audit_post_freeze_writeback` has zero production callers** (`_otr_ledger_consumers.py:246`, exported at `:310`) -- its own docstring describes a "soft-rollout phase" where "consumers log violations to batch_log"; **the rollout reached no consumer**, so the §6.16 null/enum drift check never runs on a real episode. (5) **`closing_audio` is an unwired canonical socket** -- node 12 `OTR_SignalLostVideo`'s `closing_audio` input has `link: None` in `workflows/otr_canonical.json`, and `video_engine.py:2421-2430` would discard it anyway. **A caution for whoever picks these up:** the reviewer named finding (4) in PROSE as "post_freeze_writeback_audit"; the real symbol is `audit_post_freeze_writeback`, and grepping the prose form returns ZERO and reads exactly like a refuted claim. Grep the symbol, not the description |
