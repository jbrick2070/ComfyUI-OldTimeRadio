...
| **voice-pool concentration -- MEASURED 2026-08-18, cause found, NOT yet fixed** | **OPEN.** Operator's concern was *"many voice models were blocked and we're still limited to a handful"*. **Nothing is blocked** -- 206 bank entries, **zero** quality-rejects, only the 3 reserved Lemmy clones removed (98.5% castable), and no caller requests `require_commercial_clean`. **But he is right about the outcome.** Measured over **1351 episodes / 2011 voiced rows**: ANNOUNCER rows 654, only **10 distinct, top-5 = 99%**, `bm_george` alone **82.3%** -- expected, the announcer has its own pinned path (`announcer_voice_ref`), not the character pool. **CHARACTER rows 1357, 56 distinct, top-5 = 66%**, `vz_bill_boerst` **23.7%** + `vz_caro_davy` **18.8%** = 42.5% between two voices. **THE CAUSE IS THE MATCH LADDER, NOT A BLOCK.** Swept with gender only (both `role="char_voice"` as production passes and `role="character"`): **40/40 voices drawn, top-5 = 18%** -- perfectly flat. The difference is that production also passes the writer's voice-fit `timbre` + `age_band` (`cast_lock.py:909-926`), and `g+t+r+age` scoring narrows so hard that a couple of references win most slots before the ladder ever drops a term. **The pool is open; the MATCHING concentrates it.** Fix is a design fork (loosen the timbre weight, spread within a score tier, or track recent-use across episodes) -- takes a panel, and it is pure CPU to measure |
