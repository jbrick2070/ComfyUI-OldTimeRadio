...
media_archive fix, HEAD `741a0e11`):** suite **11080 passed / 110 skipped /
...
| PBUG-20260815-06 media_archive feed selection | **SELECTION FIXED 2026-08-19 (`3be1c1e1`); OPEN for the durable-headline half.** The lane adapted the newest post forever (index defaulted to 0, feeds are newest-first, zero tests on the path). Now reuses the SCIENCE lane's shared news history rather than growing a second one -- the operator's framing: both RSS lanes should share the logic. 14 tests. Bible **12.115**. **STILL OPEN:** the selected headline is not stamped in durable episode meta, so an episode still cannot NAME what it adapted -- recording into the dedup file is not the same thing |
| PBUG-20260729-03 | **STILL OPEN, confirmed at HEAD by the 2026-08-18 sweep.** `_otr_structured_call.py:1300-1305` raises a bare `ValueError` -- which is NOT in `_ATTEMPT_ERRORS`, so it bypasses the ladder-advance fix from `41683fc9` -- when the repair context exceeds `repair_context_max_bytes`. Same shape at `_otr_scifi_p0_contract.py:436`. Both reachable through the live `_otr_scifi_news_pro.py`. A repair REFUSES instead of degrading, which is the opposite of THE LAW. Fix is small and specified |
| PBUG-20260815-05 | **STILL OPEN, fix specified and NOT landed** (confirmed at HEAD). `_generate_title_from_script` in `OTR_LedgerScriptWriter.py` still takes no `source_meta` / `play_title` / `source_ref` parameter, so the episode title can name a DIFFERENT play from the one it actually adapted |
