...
| 3. SHAKESPEARE | Rows 3.6a/3.6b, named by the operator as their own phase rather than folded into general coding. | A verbatim Shakespeare episode is buildable. |
...
| **3.6a THE VERBATIM LANE NEVER SEES ITS SOURCE** | `LineRequest.source_block` defaults to `""` (`_otr_line_composer.py:292`) and the prompt includes source text only `if req.source_block:` (`:908`). Of the two `LineRequest(` sites in the tree, the writer's main one (`OTR_LedgerScriptWriter.py:5179`) never sets it, so the model writes Shakespeare from intent and mood. Wire a DETERMINISTIC source splice there. **Do NOT reach for `compose_exchange` because it has a working `source_block` parameter:** it is still an LLM asked to CARRY the words, not a splice, so it would not satisfy the 2026-08-03 keystone and could reproduce the defect it looks like it fixes. Fidelity is a correctness defect, not story quality. |
| **3.6b `_otr_passage_selector.py` is built and unwired** | `select_passage()` is a deterministic, seeded verbatim window selector, beat-topology aware (spans a long speech across consecutive beats against `BEAT_WORD_HARD_MAX`) and `cast_ceiling`-bound, so it also answers "the cast must follow from the cut". It has its own tests and ZERO production callers. Wire it into the 3.6a seam after verifying its contract against that seam -- built-and-never-run is not the same as working. |
...
| Row | What to write |
|---|---|
| 3.1 Ghost Half-B, LLM tier | The deterministic tier is in and its measured ceiling is 26.3% of beats, so what remains is the LLM tier the operator chose: extend the batched Ghost author so a beat naming no listed `key_object` still gets a physical-artifact subject. **Its arc settled the shape:** a NEW additive optional sibling field carried NEXT TO the closed `ghost_prompt` object, so `GHOST_PROMPT_FIELDS` and the schema/author versions stay untouched and a frozen replay bundle simply lacks the key and no-ops. Rules: photographable thing only; the beat RANKS from the episode's own `key_objects`, never sources a new noun; a noun named only inside a negation is not an artifact; no second model call, no retry loop, no gate. Prompt additions are character-budgeted and must be justified in characters. |
...
| 3.4 clean install | Work from the full r1 review (`kibitz-runs/2026-09-11-arc-cleaninstall/r1/codex.md`), not a one-line gloss -- the gloss omits a real durability point. Keep the existing download scope; the early-tool-check proposal needs narrowing. Low priority, non-crash. |
