...
**CURRENT STEP = STAGE 1b (first live consumer).** STAGE 1 CHUNK 1 (dormant foundation) SHIPPED this
session: `nodes/_otr_story_pack.py` (stdlib fail-loud loader: strict validator, dup-key + whitespace
reject, typed error hierarchy, load-once cache, get_pack_prompt[_or_none]) + `nodes/story_packs/
science_news/science_news_default.json` (12 seams byte-identical to the live constants, generated by
importing the real modules) + `tests/test_story_pack_stage1.py` (18 tests: byte-identity via
runtime-import, fail-loud matrix, accessor semantics, DORMANCY guard). Sub-plan of record:
`docs/multimodal-story-schema/STAGE1_SUBPLAN.md` (v3; kibitz r1+r2 codex+antigravity converged +
2 Sonnet-5 consistency subagents clean). Suite 6198/0, Bug Bible 16. Zero production behavior change
(dormant). NEXT = STAGE 1b: wire the FIRST consumer (pilot seam `line_composer_system`, sent directly,
no identity logic). PRECONDITION: migrate the object-identity `is <const>` checks to `==` at
test_creative_prompt_router.py:62/:103 + test_audio_c7_clamp_counter.py:52 + _otr_outline.py:1847,
plus a repo_id->pack-coordinates map in _otr_creative_prompt_router.py. Stage 1b gets its OWN kibitz +
the RESERVED Fable §9 gate (the make-or-break, production-touching wiring). Then Stage 2 (banks/
pipelines routing + new lanes), Stage 3 (visual_style), Stage 4 (asserts->JSON enforcer).
Kibitz artifacts: `kibitz-runs/2026-07-04-multimodal-stage1/`.
