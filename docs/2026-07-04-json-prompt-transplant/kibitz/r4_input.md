# KIBITZ R4 INPUT -- PHASE A JSON PROMPT EXTRACTION (convergence)

## Round focus: r4 -- CONVERGENCE / RESIDUAL DEFECTS

r1 -> r2 -> r3 have delivered a hardened 7-chunk plan (Chunk 0 mirror
refresh -> Chunk 1 contracts+profiles plumbing -> Chunk 3 extractor ->
Chunk 5 byte-identity harness -> Chunk 4 extractor coverage tests ->
Chunk 6 anchor doc rewrites -> Chunk 7 full regression + rollback).

r4 is the FINAL gate. Return NO-GO only for a genuinely new must-fix
issue that would block a coder from shipping this end-to-end. If the
plan is code-ready, say GO.

Specific r4 checks:

1. Any file:line drift between pass03 and OTR HEAD (still a7bdc42d;
   docs tip 6f1c7ce2)?
2. Any test-name collision with existing sibling tests (grep
   `tests/` in sibling)?
3. Does the Chunk 5 identity pytest at OTR-side depend on
   `conftest.py` setup (OTR_TEST_MODE, CUDA_VISIBLE_DEVICES='')
   that isn't obviously satisfied by a plain pytest invocation?
4. Are the regression commands in Chunk 7 actually runnable as-is
   (venv path, PYTHONUTF8, pytest flags)?
5. Any Phase A invariant still at risk that r1-r3 missed?
6. Any circular-import risk introduced by adding an extractor.py
   module to the lab that imports from contracts + egistry?
7. Is the Chunk 1 back-compat alias TEMPLATE_SEAMS = ALL_TEMPLATE_SEAMS
   safe -- does anything import TEMPLATE_SEAMS from the top level
   that would break under the tuple-union?

Panel: Codex + Fable + Sonnet. Antigravity dropped.

## Repos to grep

- Production OTR: `C:\Users\jeffr\Documents\ComfyUI\custom_nodes\ComfyUI-OldTimeRadio` at `a7bdc42d` (docs tip `6f1c7ce2`).
...
# CURRENT PLAN STATE (input to r4 review)

Source: `docs\2026-07-04-json-prompt-transplant\kibitz\pass03_plan.md`
