...
2. ~~**`meta.voice_cast_decision`** ... This is the gold seam for "Cast & Voices."~~ **THIS SEAM IS GONE AS OF 2026-08-18 — DO NOT BUILD ON IT.** The hybrid LLM voice-fit that populated `voice_cast_decision` was ripped, so the field is now stamped EMPTY (`{}`) on every episode. It is kept only so CastLock's `meta.get("voice_cast_decision") or {}` keeps one stable shape; it carries no data. Everything the original text said about it was true at the time — it was verified present, it was in the wire ledger before the freeze — and none of it is true now. **The replacement seam for "Cast & Voices" is the CastLock stamp in item 4 below** (`voice_ref_id` / `voice_engine` per row), which is what actually names the voice that read each character; the standing blocker there is unchanged, namely that CastLock's stamp does not reach the singleton. Also note `meta.voice_cast_mode` now records WHICH caster ran (`"scorer"`), which is provenance rather than per-character credit copy.
