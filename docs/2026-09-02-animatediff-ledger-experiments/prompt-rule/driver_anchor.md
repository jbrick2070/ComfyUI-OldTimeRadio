...
## 12. r3 fold (Cursor) -- the coat is in the leaf, and the operator had already said so

Full review: `kibitz-runs/2026-09-02-prompt-v3-crux/r3/cursor.md`.
Driver judgment: `kibitz-runs/2026-09-02-prompt-v3-crux/r3/judgment.md`.
Verdict returned: yes-with-fixes. All seven must-fixes accepted.

**The finding that mattered.** Half A as written dropped the costume MOTIF but
kept the authored LEAF -- and the stored leaves carry the costume themselves:
motif *"a lean figure in a charcoal coat, carrying a satchel"*, leaf *"a figure
in a charcoal coat holds a satchel out"*. The leaf is the motif with a verb.
Shipped as planned, the A/B would have changed the prompt and not the picture,
and read as a null result for the wrong reason.

**The operator's rewrites decide it, and they go further than either reviewer.**
Across all ten he kept none of the three composed slots: no costume (his rule
9), no authored leaf (rule 3, replaced by the world's own motion), and no
framing law -- not one rewrite carries *"mid-shot or wider, whole figure
legible, one clear action, unbroken shot"*, which is 45% of every prompt today.
What he wrote is one shape, four things: **style, subject, the subject's motion,
the place.** Three of them are already in the ledger.

**So Half A composes `pack cue + crux kernel + world motion + setting`,** with a
vantage clause mapped from the stored mode, and it reads no `motif_cue`, no
`drawable_beat` and no mode law. The stored fields stay on the row, unread and
unrenamed, so replay stays byte-stable and Half B keeps its inputs.

**Three other confirmed corrections:**

* `build_request_from_shot` is ALSO ShotLock's cast-time preflight
  (`otr_shot_lock.py:1708`) and only `DeferredImageGapError` is caught there, so
  a budget refusal would kill plan build after the writer already ran. v3 gets a
  NEW finalizer, drops to fit, and never raises on a frozen row.
* `OTRVideoRenderBatch` has **no `IS_CHANGED`** (grep is empty), so a resident
  session could serve cached v2 clips under v3 code. Fresh server per arm, and
  the version constant goes into `IS_CHANGED`.
* The plate is DEFERRED. `GHOST_PROMPT_MAX_CHARS` is shared by the video prompt,
  the plate and the engine budget, and landing both changes in one GPU A/B would
  confound item 2's plate isolation.

**And one correction to the driver's own item 3b audit:**
`include_appearance=False` on the foley and mime lanes exists because the joint
latent SPEAKS the prompt, not because I2V lanes have a general identity rule.
Item 3b is amended -- add a crux clause beside `appearance` on the silent I2V
lanes rather than dropping it there on redundancy alone. The `wan_ti2v` 100-word
cap finding stands on its own.

**The governing contract is now W1-W7 in the r3 judgment, section 9.**
Where V1-V6 and R1-R10 disagree with W1-W7, W1-W7 win.

## 13. What r4 (convergence) must confirm

1. Does `cue + kernel + motion + setting + vantage` still say enough for a
   picture on a beat whose `key_objects` are thin -- and is the vantage map the
   right way to keep the three stored modes from collapsing into one shot?
2. Is a DETERMINISTIC world-motion pool acceptable for Half A given the operator
   wants fewer variables, or does per-beat motion variety require Half B's LLM
