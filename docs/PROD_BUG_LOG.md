...
- why it matters beyond tidiness: the repo already has this exact ruling for benches -- the isolated-bench carve-out (GO_FORWARD section 0A) says bench output goes to `otr/episodes/_bench_4arm/` only, **"never `otr/obs/`, never quotable as a published episode."** Soak, banksweep, probe and acceptance legs are the same category of artifact and were never given the same boundary, so they land beside real deliverables where a viewer or a future window cannot tell them apart.
- fix: NONE YET. Two defensible shapes and they are not equivalent -- (a) harness legs stop publishing to `otr/obs/` at all, matching the bench rule; (b) they publish but under a diagnostics prefix/subfolder that the eyeball and any "pick a good episode" pass skips. There is also a question of whether a run label belongs in `episode_title` at all versus a separate `run_label` field. That is a design fork, so it takes a panel rather than a guess.
- verify idea: assert no artifact in `otr/obs/` has `meta.title_source == "user"`, or more precisely that no published episode's title matches the harness-label vocabulary. Deterministic, runs over the existing corpus, and it would have caught all 16 the day they landed.
...
- fix: NONE YET. The defect is diagnosed, not repaired -- the repair is a design fork (reconcile the brief before it enters the prompt / state precedence in the prompt / post-hoc ensemble-foreign gate / all three) and is queued in `GO_FORWARD_PLAN.md` as item **I**, which takes a full kibitz arc before code.
...
- status: PROMOTED BUG-11.61 -- the RULE is promoted; the CODE FIX is open as queue item I
