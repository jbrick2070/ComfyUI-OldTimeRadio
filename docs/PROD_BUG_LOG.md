...
- root cause: NOT diagnosed. The cameo reference is reachable from the ordinary voice-selection pool rather than being reserved to the cameo decision. `meta.voice_cast_decision` records the per-character pick, so the selection is auditable.
- verify idea: assert no cast row may be assigned a `voice_ref_id` matching the cameo family (`idx_lemmy_*`) unless the Lemmy cameo is actually present in that episode's cast/decision. Deterministic; measure the corpus rate first -- earlier logs show at least one other episode assigning the same reference.
- bible-worthy: LIKELY -- "a reserved character asset is reachable from the general pool" is a reusable class. Do not promote before a fix.
- promotion: PENDING
- status: OPEN
