...
- fix: NONE YET, and the shape is now CONSTRAINED rather than open. Fix it at the SOURCE -- stop reusing the production `episode_title` field as the harness's scratch label (a separate `run_label` that the card does not read is the obvious candidate) -- and **never** by suppressing, relocating or gating the publish. Any proposal that reduces how many episodes land in `otr/obs/` tomorrow is the wrong fix regardless of tidiness. That still leaves a real design question (where the label lives, and whether the card shows it for a harness leg), so it takes a panel.
