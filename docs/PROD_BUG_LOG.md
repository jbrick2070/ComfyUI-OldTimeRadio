...
- **WHY THE LEAK WAS SEEN 16 HOURS AFTER THE FIX, and it is not a code defect.** `rivers_embrace` rendered 2026-08-17 23:30, the fix committed 07:21 the same day. The reservation was already correct at that commit -- rebuilt from `8f3c7615:config/cast_pools.py`, all three clone ids present. The soak harness boots ONE server and never tears it down (CLAUDE.md section 5), so an evening leg was still executing the module Python loaded that morning. **A stale resident process explains a post-fix sighting without the fix being wrong** -- and it is exactly why the selector-level test above now exists: the pre-existing tests all checked the reserved LIST, and none exercised the POOL where the bug actually lived.
- bible-worthy: LIKELY -- "a reserved character asset is reachable from the general pool" is a reusable class, and the companion trap ("a post-fix sighting from a resident server reads as the fix failing") is arguably the more reusable half. Promotion deferred to the next operator fan-out.
- promotion: PENDING (candidate; fix is live and verified)
- status: **FIXED 2026-08-17 (`8f3c7615`), VERIFIED AND CLOSED 2026-08-18**
