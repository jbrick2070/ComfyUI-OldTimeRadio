...
| `sd15` | image | manual, ungated | 2.0 | The only image engine whose CAPABILITIES row declares mps on purpose, and it has published real episodes on this exact M4/16 GB — 1.99 GB single CheckpointLoaderSimple file, no split loaders, no TE download, long side clamped to 768 (`OTR_SD15_MAX_SIDE`). |
