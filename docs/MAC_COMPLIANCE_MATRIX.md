...
**GENERATED 2026-09-09** by a 41-agent audit of all 61 registered engines, each verdict grounded in the registries and adapter code, and every load-bearing verdict (PROVEN / WILL NOT RUN / OOM RISK) adversarially re-checked. **5 were overturned** and are marked below. Hand-corrected after generation on 2026-09-09; the list of what changed is at the end, under "Edits after generation".
...
The sections below run in that order -- the two that can cost you a machine first, then what is known to work, then what is predicted to.
...
| engine | ns | weights | ~GB | why |
|---|---|---|---|---|
| `indextts2` | audio | manual | 11.1 | **[OVERTURNED from WILL NOT RUN]** The first pass called its sidecar a cu128-pinned, PowerShell-only install. The re-check refuted the cu128 half from the installer itself (`scripts/_otr_indextts2_install.ps1` lines 4-6: cu128 on Windows/Linux, Metal-capable default wheels on Mac via `[tool.uv.sources]`, plus a `bin/python` venv fallback for the Mac layout). What remains is size and friction: the checkpoints alone are 11.06 GB against a ~11.8 GiB Metal working set, behind an installer written as a PowerShell script. |
| `flux_gen1` | image | manual | 13.0 | One all-in-one FLUX.1-dev fp8 checkpoint the adapter itself sizes at ~13 GiB (17.3 GB per the engine survey) with no split/offload structure, and its default request is 832x1216 — z_image's 12.3 GB checkpoint already needed ~20.4 GiB in the KSampler here. |
| `z_image_turbo` | image | auto, ungated | 19.3 | Measured on this box: bf16 (12.31 GB) loads fully on Metal and then dies in the KSampler needing ~20.4 GiB of a 20.13 GiB ceiling; the int8_convrot build hits NotImplementedError aten::_int_mm (no int8 matmul on Metal at all); the nvfp4 build is Blackwell-only. |
| `animatediff15_v3_haunted_video` | video | manual | 3.8 | Only 3.77 GB of weights and no Metal blocker (its Lightning sibling published on this M4 with the same node pack and SD1.5 substrate), but it declares adaptive_hold_for_memory=False, so it has NO guard against the 136-latent ceiling that rebooted this M4 (PBUG-20260909-01). |
...
| `humo` | video | manual | 26.7 | HuMo renders FULLY RESIDENT by contract (no free_after_use -- forcing eviction fragmented the allocator into an OOM), so its 17.89 GB fp8 UNET, 6.27 GB umt5, 2.88 GB whisper, VAE and LoRA are co-resident at 26.74 GiB against an ~11.8 GiB Metal ceiling. |
| `humo_1.7B` | video | manual | 12.6 | Its own UNET is only 3.24 GB, but HuMo17BEngine._loader_names overrides ONLY unet and lora, so it still inherits the 6.27 GB umt5_xxl_fp8 encoder plus whisper 2.88 GB and the VAE, all fully resident -- about 12.6 GB against a ~10.3 GiB accelerator budget. |
...
| `flux2_klein` | image | manual | 11.0 | It actually rendered on this M4 — klein_run.log shows Flux2TEModel_ (7672.25 MB) and Flux2 (2591.64 MB) both loaded and sampling, and it minted a clean 1472x832 still at ~8 min/still — but it survived on swap (20 GB phys_footprint, 22 GB peak, on a 16 GB machine -- PROD_BUG_LOG). `sd15` is the practical Mac image engine. |
| `sd15` | image | auto, ungated | 2.0 | The only image engine whose CAPABILITIES row declares mps on purpose, and it has published real episodes on this exact M4/16 GB — 1.99 GB single CheckpointLoaderSimple file, no split loaders, no TE download, long side clamped to 768 (`OTR_SD15_MAX_SIDE`). |
| `off` | upscale | none | - | Pure pass-through that never resolves or loads a device — the composite only calls resolve_device when engine.name != "off" — and it is the shipped widget value ('off','cpu') on node 84 of the Mac lightning variant that published on 2026-09-09. |
...
| engine | ns | weights | ~GB | why |
|---|---|---|---|---|
| `chatterbox` | audio | manual | 3.0 | **[OVERTURNED from WILL NOT RUN]** There is no macOS install path: the only installer is PowerShell and the adapter's default sidecar interpreter is the Windows `.venv/Scripts/python.exe`, while the Mac profile sets allow_sidecars false. Packaging, not Metal -- see the overturned list. |
| `dia` | audio | manual | 6.0 | **[OVERTURNED from WILL NOT RUN]** Its installer force-reinstalls a nightly cu128 torch as the final step, a wheel index that publishes nothing for macOS, and the worker never accepts a device at all - it picks fp16 only when torch.cuda is available and float32 otherwise (`scripts/_otr_dia_worker.py`). A missing Mac installer, not a Metal gate -- see the overturned list. |
...
| `stable_audio_music` | audio | auto, GATED | 4.5 | Never run on a Mac and declares only ["cuda"], but the adapter's sole device code is `.to(dev)` plus `torch.Generator(device=dev)`, and a seeded torch.Generator("mps") is recorded as returning finite noise on this exact host - so the missing row is untested policy, not a blocker. |
| `cloud_flux_pro` | image | none | - | No local compute at all — the provider renders and the only host-side work is a PIL sRGB-PNG canonicalize; the row already declares mps with practical_without_gpu True and assert_usable checks nothing but Pillow and a healthy partner pin row. |
...
| `cloud_nano_banana_2` | image | none | - | Same CPU-side partner-API shape as every cloud still row — mps declared, practical_without_gpu True, zero model_requirements, and the only structural check is Pillow plus the cloud_nano_banana_2 pin row; untested here purely because a test would measure a credential, not Metal. |
...
## Cloud lanes: local plumbing PROVEN, the POST untested (2026-09-09)

Nineteen of the 27 LIKELY rows are cloud lanes (4 audio, 7 image, 8 video). For
the eight cloud VIDEO rows the verdict is more precise than LIKELY suggests, and
the distinction saved a paid test.

**Six of the eight cloud VIDEO lanes inherit one base**, `_CloudVideoBase`
(`eng_cloud_video.py:472`), which owns `assert_usable`, `render_clip` and
`canonicalize`; the subclasses differ only in endpoint and parameters. The
other two, `google_omni_video` and `google_veo_video`, are standalone BYO-key
adapters with the same ffmpeg/ffprobe gate. So the
platform-sensitive code is one class, shared -- and it splits cleanly:

| half | where it runs | status on Metal |
|---|---|---|
| the ffmpeg/ffprobe gate, the 1080p conform, the provider-audio strip, sha256, temp paths | **this Mac** | **PROVEN** |
| `invoke_partner_node` -- the POST | the provider's GPU | untested |

The local half was exercised with a real episode mp4 standing in for a provider
response, so it cost nothing:

```
resolve_ffmpeg  -> ~/Library/Application Support/ffmpeg-downloader/ffmpeg/ffmpeg
resolve_ffprobe -> .../ffprobe                     (assert_usable's own gate)
cloud_delivery_wh  1472x832 -> 1920x1080 | 832x1472 -> 1080x1920 | 0x0 -> default
canonicalize_video -> 1920x1080 @ 25 fps, 136.24 s, sha256 896b9df7...
post-strip proof   -> ZERO audio streams in the output
```

The audio strip is the part that had to be checked: master audio is frozen
upstream and the mux is last, so a provider audio track surviving would corrupt
the episode. It does not survive.

**Why the POST is not worth chasing on this machine.** Comfy Cloud auth comes
from a LOGGED-IN ComfyUI DESKTOP session, not from a key on disk -- the base's own
comment says so, and warns that a resolve-time env check "would wrongly block
logged-in desktop users". Every Mac render in this campaign ran through a
headless `main.py`, where those hidden inputs do not exist. So a Comfy cloud call
cannot be exercised here regardless of credits; it needs the Desktop app. Google
lanes take a BYO env key instead and would work headless.

Neither is a Metal question. What remains untested is a credential and a provider
endpoint, and a Mac cannot break either differently from Windows.
...
**2026-09-12.** `sd15` and `spandrel_esrgan` both said "auto, ungated" in the weights column; both are **manual, ungated**. Neither adapter fetches anything -- `nodes/_otr_image_engines/sd15.py` raises `EngineUnusable(MISSING_MODEL)` with an `hf_hub_download` line to run and a file to copy into `models/checkpoints/`, and `nodes/_otr_upscale_engines/eng_spandrel_esrgan.py` does the same for a 67 MB file in `models/upscale_models/`. The word came from `scripts/otr_provision.py` filing them under `hf_cache`, whose own contract is "the engine's own loader fetches it", and it propagated into the README's dropdown matrix as well. Both are corrected at that source, so the generated matrix now agrees. **SUPERSEDED THE SAME DAY, and the fix is better than the correction:** `sd15`'s checkpoint was added to `nodes/_otr_visual_assets.py`'s manifest on 2026-09-12, so `OTR_WorkflowValidator` now downloads it at queue time when a selected lane needs a still. Its row reads **auto** again, this time truthfully. That makes the Mac stills lane AND the `ltx_8gb` lane one click each -- LTX 0.9.8 is image-to-video, so its own two self-fetching weights were never enough on their own. `spandrel_esrgan` stays manual: 67 MB, no manifest row yet. Verdicts and counts are unchanged.
