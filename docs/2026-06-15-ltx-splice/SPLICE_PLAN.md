...
gate. Operator decisions (2026-06-15): **(A) sampler = distilled** (what the mini uses), gated on a
motion-acceptance smoke; **(B) per-beat = scene/b-roll** (HuMo keeps character faces — no role change).
...
- Sampler: `_sampler_mode()` default → `distilled` (decision A; the mini's chain). KEEP `ksampler` as the
  `OTR_LTX_SAMPLER` env fallback. Update the stale "ksampler DEFAULT / distilled GATES motion" comment to note
  the 22B+distilled-LoRA reverses that (proven on the mini) — **gated by the §7 motion smoke**.
