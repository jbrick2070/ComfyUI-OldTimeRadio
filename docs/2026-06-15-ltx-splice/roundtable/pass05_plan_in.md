...
## 11. PASS-04 FOLDS (precise wiring — these win over looser wording above)
1. **Sigmas:** the mini's `ManualSigmas` == the engine's existing `_SigmasFromValues` injector carrying the SAME 9
   values (`1.0,0.99375,0.9875,0.98125,0.975,0.909375,0.725,0.421875,0.0`). "Exact graph" = same sigma VALUES via
   that injector, NOT a new `ManualSigmas` node. The §6 test checks the VALUES, not a node class.
2. **Pin the mini values in distilled mode.** Grounded code lets env override them: `_sampler_name()`←`OTR_LTX_SAMPLER_NAME`,
   cfg←`OTR_LTX_CFG`, LoRA strength←`OTR_LTX_DISTILLED_LORA_STRENGTH`, i2v←`OTR_LTX_I2V_STRENGTH`. In `distilled`
   production mode these MUST equal the mini (`euler_cfg_pp`, cfg `1.0`, `0.70`, `0.75`): hardcode in distilled mode,
   or the motion smoke asserts the four env vars are unset/at-mini-values and FAILS if they differ. A/B knobs → ksampler-only.
3. **`_distilled_lora_file()` returns a `(name, path)` TUPLE.** Split into `_distilled_lora_name()` (the Comfy
   `lora_name` string) + `_distilled_lora_path()` (install/floor). Wire `LoraLoaderModelOnly{lora_name:_distilled_lora_name()}`;
   fix every existing tuple call site.
4. **Re-wire the text-encode clip:** the `pos`/`neg` `CLIPTextEncode` nodes' `clip` input → `W("te",0)` (was `W("encoder",0)`).
5. **Add the new candidates to the BASE `_node_candidates()` ONLY** — `_node_candidates_sampling`/`_i2v` INHERIT it.
   Do NOT add to both (redundant + desync risk).
6. **Node-class gate lives in `load()`, NOT `assert_usable()`** (Invariant V-12: `assert_usable` runs at validate-time
   on a CPU box with NO ComfyUI nodes importable). `assert_usable()` checks only Sage + the 5 weight files + floors.
   `load()` resolves the FULL active candidate set (the 5 loaders/decode PLUS the distilled sampler chain + i2v nodes:
   `KSamplerSelect/RandomNoise/CFGGuider/SamplerCustomAdvanced/EmptyLTXVLatentVideo/LTXVConditioning/LTXVImgToVideoConditionOnly/LoadImage/CLIPTextEncode`).
7. **VRAM keep-set = `{"lora", self._TERMINAL}`** (the LoRA-wrapped unet IS the resident patcher); `model =
   results.get("lora",(None,))[0]`. Unit test asserts that exact retained key under `free_after_use=True`.
8. **`commercial_clean = True`** (Apache GGUF + LTX-2 Community model; no AGPL/GPL) — set it in this commit.
9. **Motion floor = HARDCODE frame-diff ≥ 2.0 per role** (2B static read 0.84, dynamic 7.85; ≥2.0 proves real motion),
   measured by `otr_ltx_motion_smoke`. State it in the test, not "e.g.".
10. Keep the CLAUDE.md GPU-reset + validator-round-trip as belt-and-suspenders (panel called them redundant; harmless).
