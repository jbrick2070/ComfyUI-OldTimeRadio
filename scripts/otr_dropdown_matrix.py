...
def render_doc(rows: list) -> str:
    L = ["# Dropdown matrix -- what each choice costs, and what runs it\n\n",
         ...
         "`CAPABILITIES` row against the shipped workflow named in its column; "
         "sizes and gating are read out of the fetch manifests. Only the "
         "memory verdicts and receipts are curated, in "
         "`apple/dropdown_matrix.json`.\n\n",
         "You never need all the weights in this workflow. One workflow ships; "
    ...
    if unprov:
        L.append("\n## Hosted lanes no shipped workflow provisions\n\n")
        L.append("These need no weights and no VRAM -- only a credential -- and "
                 ...
                 "shipping workflow selects one, so they were never added to the "
                 "provisioner's remote route, and `scripts/otr_provision.py` "
                 "REFUSES a `--profile` (its own flag name) that names one rather "
                 "than guessing. Using one today means selecting it in the "
                 "canonical workflow by hand, not through a shipped one. Worth "
    ...
    if orphans:
        L.append("These are registered and selectable, but "
                 ...
                 "weights -- so a workflow that selects one fails to "
...
def render_apple(rows: list) -> str:
    ...
    1. Which workflow do I open?  2. Will this engine run here?  3. Where do the
    ...
    L = ["# Which machine runs what\n\n",
         "Every figure here is read from the code that uses it, so this page "
         "cannot drift from what the pack actually does.\n\n",
    ...
    L.append("*This page is generated. To change it, edit "
             "`scripts/otr_dropdown_matrix.py` in the GitHub tree and run it -- "
             "`scripts/` is not part of an installed copy, so there is nothing "
             "here to hand-edit.*\n")
    return "".join(L)
