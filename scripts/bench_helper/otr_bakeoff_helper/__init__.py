...
# THE DMD RESTART SAMPLER MOVED OUT OF THIS PACKAGE (kibitz r2 F2, 2026-08-01).
#
# It now lives at nodes/_otr_video_engines/dmd_sampler.py in the OTR pack and is
# registered there under the SAME node key, "OTR_DMDRestartSamplerSelect", so the
# digest-pinned bench graphs under scripts/bench_graphs/ resolve unchanged.
#
# WHY IT LEFT: this package is a DIAGNOSTIC vendored under the CLAUDE.md s0A bench
# carve-out. A production engine (fastwan_8gb) may not depend on it, and two copies
# of a sampling transition is how a silent divergence ships. There is no import
# shim here on purpose -- re-registering the key from two packs is ambiguous, and
# the bench runs against a server with the OTR pack installed, so the class
# resolves from its one owner.
#
# The production copy also REFUSES (DmdModelSamplingError) where this one used to
# log noise_scaling=FALLBACK-flow and proceed with an inline CONST approximation --
# loud, but it still rendered, and the bench would have graded that as a PASS for a
# transition that was approximated rather than taken from the loaded model.
#
# The VRAM probes above STAY here: they are measurement machinery, the sampler is
# production machinery. That asymmetry is deliberate.
