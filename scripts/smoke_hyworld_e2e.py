...
Six stages:
    ...
    [4] Temp WAV hygiene -- assert no hyworld_audio_*.wav leaked in %TEMP%.
...
def count_leaked_temp_wavs() -> int:
    pattern = os.path.join(tempfile.gettempdir(), "hyworld_audio_*.wav")
...
def run_poll(job_id: str):
    ...
    return HyworldPoll().execute(hyworld_job_id=job_id)
...
def run_renderer(assets_path, shotlist_json, audio, episode_title="SIGNAL LOST -- Smoke Test Episode"):
    ...
    return HyworldRenderer().execute(
...
def main() -> int:
    print("=" * 72)
    print("HyWorld trio smoke test (Bridge -> Poll -> Renderer + contract asserts)")
    print("=" * 72)

    temp_wav_before = count_leaked_temp_wavs()
    print(f"Baseline hyworld_audio_*.wav count in %TEMP%: {temp_wav_before}")
    ...
    print(f"  hyworld_audio_*.wav in %TEMP%: before={temp_wav_before} after={temp_wav_after}")
    ...
    print("\n" + "=" * 72)
    print("ALL 6 STAGES PASSED -- HyWorld wiring smoke test GREEN")
    print("=" * 72)
    return 0
