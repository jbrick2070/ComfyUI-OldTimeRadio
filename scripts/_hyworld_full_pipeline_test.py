"""
HyWorld Full Pipeline Test — runs the complete OTR workflow with HyWorld nodes
wired in, via ComfyUI /prompt API. Supersoaker pattern.

This exercises the REAL pipeline end-to-end:
  ScriptWriter -> Director -> HyworldBridge (fires early, parallel with audio)
                           -> BatchBark -> Kokoro -> AudioGen -> MusicGen
                              -> SceneSequencer -> AudioEnhance -> EpisodeAssembler
  HyworldPoll (waits for sidecar) + EpisodeAssembler audio -> HyworldRenderer -> final MP4

HyWorld Bridge runs RIGHT AFTER Director (no SceneSequencer dependency),
...
# The full OTR pipeline + HyWorld nodes appended
WORKFLOW = {
    "1": {
        "class_type": "OTR_Gemma4ScriptWriter",
        "inputs": {
            "episode_title": "HyWorld Integration Smoke",
    ...
    # ── HyWorld v2.0 nodes (dry run, no sidecar) ──
...
def main():
    print("=" * 65)
    print("HyWorld FULL PIPELINE Test (supersoaker pattern)")
    print(f"ComfyUI: {COMFYUI}")
    print(f"Time: {time.strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"Config: short (3 acts), 350 words, 2 chars, dry-run HyWorld")
    print("=" * 65)
    ...
    try:
        ...
        needed = [
            ...
            "OTR_HyworldBridge", "OTR_HyworldPoll", "OTR_HyworldRenderer",
        ]
    ...
    print("\n[5] Checking HyWorld contract artifacts...")
    import os
    io_dir = os.path.join(
    ...
    if os.path.isdir(io_dir):
        ...
        print(f"    Jobs in io/hyworld_in/: {len(jobs)}")
        for j in jobs:
        ...
        print(f"    io/hyworld_in/ not found (Bridge may not have written)")

    # Summary
    print("\n" + "=" * 65)
    print(f"FULL PIPELINE TEST: status={status}, elapsed={elapsed_min:.1f}min")
    if status == "success":
        print("VERDICT: PIPELINE COMPLETE — audio + video + HyWorld integration OK")
    else:
