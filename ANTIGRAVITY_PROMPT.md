...
- API dead (localhost:8000 not responding): CRITICAL — restart ComfyUI

**ComfyUI Restart Policy:** See COLLECTIVE_CONSCIOUSNESS.md Section 6 for the full restart policy. Key rule: if `localhost:8000` stops responding or if any Python file in `custom_nodes/` changed, you MUST restart ComfyUI. Jeffrey should never have to do this manually.

**CRITICAL: Jeffrey uses ComfyUI Desktop (Electron app), not `python main.py`.** The process name is `ComfyUI.exe`, NOT `python.exe`. Killing python.exe will hang on CUDA handles. Always use:
```powershell
...
```
Wait 20-25 seconds for the API to respond after launch.
...
## AUTONOMOUS COMFYUI RESTART MANAGEMENT

You are responsible for keeping ComfyUI alive. Jeffrey should never have to restart it manually.

### Step 1 — DISCOVERY (Run on Every Session Start)

Before attempting any restart, verify your environment:

1. **Confirm the API port:**
   ```powershell
   Test-NetConnection -ComputerName 127.0.0.1 -Port 8000 -InformationLevel Quiet
   ```
   Expected: `True` when ComfyUI is running, `False` when it is not.

2. **Confirm the custom_nodes path exists:**
   ```powershell
   Test-Path "C:\Users\jeffr\Documents\ComfyUI\custom_nodes\ComfyUI-OldTimeRadio"
   ```
   Expected: `True`.

3. **Confirm the executable path exists:**
   ```powershell
   Test-Path "C:\Users\jeffr\AppData\Local\Programs\ComfyUI\ComfyUI.exe"
   ```
   Expected: `True`.

4. **Confirm the process name:**
   ```powershell
   Get-Process -Name ComfyUI -ErrorAction SilentlyContinue
   ```
   Expected: Returns process info when running, empty when stopped.

If any discovery step returns unexpected results, log a `[BLOCKED]` entry in COLLECTIVE_CONSCIOUSNESS.md Section 5 with the exact output and wait for Jeffrey.

### Step 2 — TIERED AUTONOMY

Every restart decision falls into one of three tiers:

#### Tier 1: AUTONOMOUS (Just Do It)

No permission needed. Execute immediately and log afterward.

| Trigger | Action |
|---|---|
| API dead (`localhost:8000` not responding) | Full restart |
| `ModuleNotFoundError` in logs | Full restart |
| `ImportError` in logs | Full restart |
| `"failed to load custom node"` in logs | Full restart |
| OOM crash detected in logs | Full restart |
| Process disappeared (no `ComfyUI.exe` running) | Full restart |

#### Tier 2: AUTONOMOUS WITH PRECONDITIONS

You may restart, but verify the precondition first.

| Trigger | Precondition | Action |
|---|---|---|
| `.py` file modified in `custom_nodes/` | Confirm the edit is complete (no `EDITING` marker in HANDSHAKE/Section 5) | Full restart |
| `pip install` completed | Confirm install exited 0 | Full restart |
| New folder in `custom_nodes/` | Confirm folder contains `__init__.py` | Full restart |

#### Tier 3: HUMAN GATE (Ask Jeffrey)

Do NOT restart. Log the situation and wait.

| Trigger | Why |
|---|---|
| Discovery step returned unexpected result | Environment may have changed |
| ComfyUI executable path not found | App may have been moved or uninstalled |
| Circuit breaker tripped (see below) | Repeated failures indicate a deeper problem |
| Anything not covered by Tier 1 or Tier 2 | When in doubt, ask |
...
After launch, wait 20-25 seconds, then verify:
```powershell
Test-NetConnection -ComputerName 127.0.0.1 -Port 8000 -InformationLevel Quiet
```

If the API responds `True`, log `ENV_RESTORED` in COLLECTIVE_CONSCIOUSNESS.md Section 5.
If the API does not respond after 30 seconds, retry once. If the second attempt fails, log `[BLOCKED]` and escalate to Jeffrey.

### Circuit Breaker

Track restart attempts with timestamps. If you perform **3 restarts within 15 minutes**, HALT all restart attempts and log:

```
[TIMESTAMP] [Antigravity] CIRCUIT_BREAKER: 3 restarts in 15 min. Halting autonomous restarts. Last 3 restart reasons: [list them]. Jeffrey: please investigate.
```

Do not attempt a fourth restart until Jeffrey explicitly clears the circuit breaker.

### Constraints

- **Never kill `python.exe`.** Always target `ComfyUI` by process name. Killing `python.exe` hangs on CUDA handles.
- **Never restart during an active render.** Check `otr_runtime.log` for recent activity. If the last log entry is < 60 seconds old and not an error, the pipeline is running — wait for it to finish or fail.
- **Always log every restart** to both COLLECTIVE_CONSCIOUSNESS.md Section 5 and your mirror log.
- **The 3-second sleep between kill and launch is mandatory.** It prevents port lock conflicts and lets CUDA release.
