...
**Workstream 1 — SFX dedicated LLM pass [feature; round-robin gated].** Generate SFX (and `music_inter`) cues in a dedicated `technical_model`-slot pass that runs *once over the whole composed script* — a film-score-style "spotting" pass that decides every cue placement at once, NOT a per-line "is a cue appropriate here" check. Cue density and pacing are a global dramatic decision; per-line tends to over-sprinkle or come out uneven. The pass reads the finished script as context and emits clean `speaker_role="sfx"` / `"music_inter"` ledger rows, each with a `start_s` (or an anchor + offset). Scope note: the downstream half is already built — `OTR_SceneSequencer` assembles the timeline by `start_s` and has an `sfx_offset_ms` knob (it reports `sfx_lines_positioned`), and `batch_procedural_sfx` / `batch_audiogen` already render `roles={"sfx"}` rows. Only the spotting pass is new work. Retire the dead `sfx_cue`-on-a-beat field. Mirrors the announcer dedicated-pass pattern (`compose_announcer_intro` / `compose_announcer_outro`). New LLM call → Prime Directive 6 applies. Scope (2026-06-01): this SFX feature is independent of the v2 audio-engine overhaul -- the engine registry covers music + announcer + character voices only and deliberately excludes SFX. Do not batch the two.
...
**Status:** PARKED (independent track). Workstream 2's primary fix (BUG-LOCAL-267) has landed; its no-separator follow-up and Workstream 1 remain parked. The v2 audio-engine overhaul (music + announcer + character voices) does NOT include SFX -- keep the tracks separate.
