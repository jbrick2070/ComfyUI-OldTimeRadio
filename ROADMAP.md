...
- **MusicGen Theme. [IN PROGRESS — node landed, needs end-to-end episode test]** Real instrumental music for opening and closing bookends plus an act-break interstitial. Implemented as `nodes/musicgen_theme.py` (`OTR_MusicGenTheme`, display name "🎺 MusicGen Theme"). Uses `transformers.MusicgenForConditionalGeneration` + `AutoProcessor` with `facebook/musicgen-medium` (~6 GB VRAM) — no `audiocraft` dependency, because audiocraft's pinned `av==11.0.0` and `spacy==3.5.2` create an unsolvable install path on Windows + Python 3.12. `Gemma4Director` now emits a `music_plan` block in `production_plan_json` with three fixed cues (`opening` 12s, `closing` 8s, `interstitial` 4s), each with a generation prompt tailored to the specific episode's tone. The node caches every `(cue_id, prompt, duration, episode_seed)` tuple to `models/musicgen_cache/<cue>_<hash>.wav` so re-runs of the same episode never reload the model or regenerate audio. Model load is gated on at least one cue being uncached; after generation the model is deleted and `torch.cuda.empty_cache()` is called so Bark has its full VRAM window when BatchBark runs next. The opening and closing outputs replace the previous procedural-noise `OTR_SFXGenerator` stubs that were wired into `EpisodeAssembler.opening_theme_audio` / `closing_theme_audio` in all three shipped workflows — those dangling SFX nodes are still present in the workflows but are no longer consumed. Interstitial output is left unconnected pending a scene-transition bus patch in v1.5. **Remaining:** full-episode regression pass on hardware with measured VRAM delta during the Gemma -> MusicGen -> Bark handoff.
