...
**Current gate:** Sprint H §3.7 harness closure is structurally GREEN. **BUG-LOCAL-230 architectural axis PROVEN** by the 2026-05-18 21:10 smoke (gates #1-#4 + #7 PASS: load delta 11.08 GiB, dtype `torch.float8_e4m3fn` w/ bf16 cast). **Runtime axis BLOCKED on BUG-LOCAL-231** -- 3 fix attempts to date (HEAD 36bcfc0 encode-reorder + free_memory; HEAD c1d37fe Option B unload_all_models; HEAD 4c887e0 paired with gc+empty_cache per Bug Bible BUG-07.03) brought the sampler pace from pre-fix 154 s/it to 123 s/it (20% improvement, still 8-10x off the 10-15 s/step target). LHM during sampler entry on all 3 attempts: ~15.5-15.9 GiB GPU Memory Used + 974-1199 MB D3D Shared Memory paging, against expected ~11 GiB resident with `mm.unload_all_models() + gc.collect() + empty_cache()` followed by `mm.load_models_gpu([model])`. **Verdict: encode-before-pin + nuclear-eviction architecture is structurally correct (audio gates remain clean, dtype path remains correct, AST + Bug Bible + audio byte-identical regressions all PASS) but does not close gates #5/#6 alone.** The residual ~5 GiB at sampler entry is the bottleneck. Strongest current hypothesis (i): the audio path's Bark/Kokoro/MusicGen + Gemma writer LLM tensors are NOT comfy-registered models -- `mm.unload_all_models()` only evicts what's in comfy's model_management registry. Need explicit OTR-side teardown (`_otr_model_loader.unload_llm()` + per-audio-node `model.cpu() + del` + gc + empty_cache) BEFORE the FLUX phase starts. Three other candidates (comfy_kitchen fp8 backend, `model.load_device` addressability, sm_120-specific bf16-cast sampler path) documented in BUG_LOG.md BUG-LOCAL-231 next-investigation-surface section. BUG-LOCAL-230 stays at pending-verification posture (no `[FIXED]` marker) until BUG-LOCAL-231 closes and a fresh 7-criteria smoke passes cleanly. §3.8 stays blocked. **Holding for Jeffrey on next investigation direction.**
